...
"""
	...
	sample = tmpl%(savedir, response, variables, method)
...
class UncertaintyDakotaIn:
	...
	'''
	def __init__(self, mofn, start, stop, step, initStep, maxStep, integOrder
			,solver, nls, lv, system, runsolstice=False, peaker=False
			,perf_num=1, perf_i=[1]):
...
class OptimisationDakotaIn:
	...
	def moga(self,seed=10983, max_eval=2500, init_type='unique_random'
			...
			, percent_change=0.05, num_generations = 40, final_solutions=3):
		"""
		Multi-objective optimisation, DAKOTA:
...
output silent
"""
		self.method+=tmpl%(seed, max_eval, init_type, crossover_type
				, num_offspring, num_parents, crossover_rate, mutation_type
				, mutation_rate, fitness_type, percent_change, num_generations
				, final_solutions)
	...
	def soga(self,seed=10983, max_eval=2000, init_type='unique_random', pop_size=48, crossover_type=None,  num_offspring=2, num_parents=2, crossover_multip=2, crossover_rate=0.8, mutation_type='replace_uniform', mutation_rate=0.2, fitness_type='merit_function', percent_change = 0.05, num_generations = 20):
		"""
		Single-objective optimisation, DAKOTA
		`seed`, int: index of seed, to generate repeatable results
		`max_eval`, int: maximum function evaluations
		`init_type`, str: initialisation type, 'unique_random'
			, 'simple_random' or 'flat_file'
...
output silent
"""
		self.method+=tmpl %(seed, max_eval, init_type, pop_size, crossover
			, mutation_type , mutation_rate, fitness_type, percent_change
			, num_generations)
...
def gen_interface_bb(savedir):
	...
	* `perf_n` (list of str): a list of the names of the resulting performance
		, e.g. lcoe, capf, epy, srev	
	* `perf_sign` (list of float): a list of signs for the optimisation
		, e.g. 1 is to minimise, -1 is to maximise; 
		perf_sign=None if it is a study other than optimisation		
	"""
...
# TODO surely there is a cleaner way of writing this part...
...
if __name__=='__main__':
	# FIXME move all test code to tests folder if possible
	'''
	mode='uncertainty'
	sample_type='lhs'
	num_sample=20
