...
void st_linprog_dualtank2(
    ...
    double t0, double um, double* optimalSolution
)
{
    //*************** Printing some stuff
    MSG("Optimisation at time = %f h with time step = %f h", t0/3600, dt);
    ...
    MSG("Maximum hydrogen heat to burner = %.3f", Q_TES_HX_max / etaBurner);
        ...
        */
        ...
        fprintf(stderr,"Q_TES_PB_max = %.3f MWhth\n", PB_size/etaPB);
   ...
   for(int i=1; i<=N; i++){
        ...
        if (PB_size/etaPB > 1e-3){
            glp_set_col_bnds(
                P, i_Q_TES_PB(i), GLP_DB, 0, PB_size/etaPB
            );
        }else{
            glp_set_col_bnds(
                P, i_Q_TES_PB(i), GLP_FX, 0.0, 0.0
            );
        }
   ...
   */
    ...
    for (unsigned i = 1; i <= N; i++)
    {
        ...
        0. TES energy balance reversed
                -E_TES[i] + E_TES[i-1] + Q_SF_TES Δt + P_PV_TES etaResistive Δt/um - Q_TES_PB Δt - Q_TES_HX Δt = 0
                    -1         +1             +dt              +etaResistive dt/um           -dt         -dt
        ...
        if (i == 1){
            ...
            /*
                -E_TES[i]  + Q_SF_TES Δt + P_PV_TES etaResistive Δt - Q_TES_PB Δt - Q_TES_HX Δt = -E_TES_init
                    -1           +dt            +etaResistive dt/um           -dt         -dt   
            */
            glp_set_mat_row(
                P, TES_EB_REV(i), 5,
                   (int[]){0 , i_E_TES(i), i_Q_SF_TES(i),  i_P_PV_TES(i)            , i_Q_TES_PB(i)  ,i_Q_TES_HX(i)},
                (double[]){0 ,     -1    ,      +dt     ,  etaResistive * +dt/um      ,     -dt        ,      -dt     }
            );
            ...
            */
            glp_set_mat_row(
                ...
                (double[]){0 ,     -1    ,     +1      ,       +dt     ,etaResistive * +dt/um   ,     -dt        ,     -dt     }
            );
        ...
        */
        ...
        if (i == 1){
            //****************************** Deal with the LHS of the equation
            /*
                E_TES[i] - Q_SF_TES Δt - P_PV_TES etaResistive Δt/um + Q_TES_PB Δt + Q_TES_HX Δt = E_TES_init
                    +1         -dt            -etaResistive dt/um           +dt         +dt   
            */
            glp_set_mat_row(
                P, TES_EB(i), 5,
            ...
            */
            glp_set_mat_row(
                P, TES_EB(i), 6,
                   (int[]){0 , i_E_TES(i), i_E_TES(i)-1,  i_Q_SF_TES(i), i_P_PV_TES(i)      , i_Q_TES_PB(i) , i_Q_TES_HX(i)},
                (double[]){0 ,     +1    ,     -1      ,       -dt     ,-dt *etaResistive/um,     dt        ,       dt     }
            );
        ...
        */
        ...
        6. Electrolyser energy balance reversed
                -Q_TES_PB etaPB Δt * um - P_PV_AEL Δt + P_AEL Δt = 0
        ...
        */
        ...
        10. Heater energy balance reversed
                (Q_H2_reactor/LHV * -c_ratio) * Δh_H2 dt + Q_H2_burner * etaBurner dt + Q_TES_HX dt * um = 0
        ...
        */
        glp_set_mat_row(
            ...
            (double[]){0,  -c_ratio/LHV*delta_h_H2*dt       ,   +dt * etaBurner             ,       +dt*um      }
        );
        ...
        */
        ...
        12. Long term TES energy balance
                E_TES[N] = E_TES_init
                 Σ(Q_TES_PB + Q_TES_HX - Q_SF - P_PV_TES/um) = 0
