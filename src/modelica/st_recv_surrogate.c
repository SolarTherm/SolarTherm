...
typedef struct{
	// FIXME use lower case for struct members
	TF_Session* Session;
	TF_Graph* Graph;												
	TF_Status* Status;																
	TF_SessionOptions* SessionOpts;
	TF_Buffer* RunOpts; // FIXME remove members that don't need to be here
...
double run_surrogate(const Session_Props_Static_ANN *sess
		, const double raw_input[], int inputsize
		, const double X_max[], const double X_min[], const double Y_max
		, const double Y_min
){

	//*********************Read input struct
	// FIXME remove all this, see st_recv_surrogate_multi.c
	...
	//********* Run the session
	TF_SessionRun(Session, NULL, Input, InputValues
		,NumInputs, Output, OutputValues, NumOutputs, NULL, 0,NULL, Status);
