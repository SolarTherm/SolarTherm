...
model Reference_1_fluid
  ...
  // System components
  // *********************
  Models.Sources.SolarModel.Sun sun(lon = data.lon, lat = data.lat, t_zone = data.t_zone, year = data.year, redeclare function solarPosition = Models.Sources.SolarFunctions.PSA_Algorithm) annotation(
    Placement(transformation(extent = {{-82, 60}, {-62, 80}})));
  Models.CSP.CRS.HeliostatsField.HeliostatsField heliostatsField(n_h = n_heliostat, lon = data.lon, lat = data.lat, ele_min(displayUnit = "deg") = 0.13962634015955, Wspd_max = 15, use_wind = true, he_av = 0.99, use_on = true, use_defocus = true, A_h = A_heliostat, nu_defocus = 1, nu_min = 0.3, Q_design = 330000000, nu_start = 0.6, redeclare model Optical = Models.CSP.CRS.HeliostatsField.Optical.Table(angles = SolarTherm.Types.Solar_angles.elo_hra, file = file_optics)) annotation(
    Placement(transformation(extent = {{-88, 2}, {-56, 36}})));
  Models.CSP.CRS.Receivers.ReceiverFluid receiver(em = 0.88, redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, H_rcv = H_receiver, D_rcv = D_receiver, N_pa = 20, t_tb = 1.25e-3, D_tb = 40e-3, ab = 0.94) annotation(
  ...
  Models.Storage.Tank.Tank tankHot(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, T_start = from_degC(574), L_start = 30, alpha = 3, use_p_top = true, enable_losses = true, use_L = true, T_set = from_degC(500)) annotation(
    Placement(transformation(extent = {{16, 54}, {36, 74}})));
  Models.Fluid.Pumps.PumpSimple pumpHot(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph) annotation(
    Placement(transformation(extent = {{66, 38}, {78, 50}})));
  Models.Storage.Tank.Tank tankCold(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, T_start = from_degC(290), L_start = 70, alpha = 3, use_p_top = true, enable_losses = true, use_L = true, T_set = from_degC(280)) annotation(
    Placement(transformation(extent = {{64, -28}, {44, -8}})));
  Models.Fluid.Pumps.PumpSimple pumpCold(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, k_loss = 0.15e3) annotation(
    Placement(transformation(extent = {{10, -30}, {-2, -18}})));
  Models.Sources.DataTable.DataTable data(t_zone = 9.5, file = file_weather) annotation(
    Placement(transformation(extent = {{-138, -30}, {-108, -2}})));
  Models.Fluid.Sensors.Temperature temperature(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph) annotation(
  ...
  Models.Control.PowerBlockControl controlHot(m_flow_on = 682.544, L_off = 5, L_df_on = 120, L_df_off = 123, L_on = 10) annotation(
    Placement(transformation(extent = {{48, 72}, {60, 58}})));
  Models.Control.ReceiverControl controlCold(T_ref = from_degC(574), m_flow_max = 1400, y_start = 1000, L_df_on = 5, L_df_off = 7, L_off = 0, Ti = 0.1, Kp = -1000) annotation(
    Placement(transformation(extent = {{24, -10}, {10, 4}})));
  Models.PowerBlocks.PowerBlockModel powerBlock(W_des = P_gross, enable_losses = true, redeclare model Cycle = Models.PowerBlocks.Correlation.Rankine, nu_min = 0.5, external_parasities = true, W_base = 0.0055 * 111e6, p_bo = 10000000, T_des = 316.15, redeclare model Cooling = Models.PowerBlocks.Cooling.SAM(T_des = 316.15)) annotation(
  ...
  Models.Analysis.Market market(redeclare model Price = Models.Analysis.EnergyPrice.Constant) annotation(
    Placement(transformation(extent = {{126, 12}, {146, 32}})));
...
equation
  ...
  connect(powerBlock.W_net, market.W_net) annotation(
    Line(points = {{115.18, 22.05}, {119.59, 22.05}, {119.59, 21.8}, {126, 21.8}}, color = {0, 0, 127}));
  connect(receiver.on, controlCold.sf_on) annotation(
  ...
  annotation(
    Diagram(coordinateSystem(extent = {{-140, -120}, {160, 140}}), graphics = {Text(extent = {{4, 92}, {40, 90}}, lineColor = {217, 67, 180}, fontSize = 9, textString = "defocus strategy"), Text(extent = {{-50, -40}, {-14, -40}}, lineColor = {217, 67, 180}, fontSize = 9, textString = "on/off strategy"), Text(extent = {{-52, 8}, {-4, -12}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Receiver"), Text(extent = {{-110, 4}, {-62, -16}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Heliostats
Field"), Text(extent = {{-80, 86}, {-32, 66}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Sun"), Text(extent = {{0, 58}, {48, 38}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Hot Tank"), Text(extent = {{30, -24}, {78, -44}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Cold Tank"), Text(extent = {{80, 12}, {128, -8}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Power Block"), Text(extent = {{112, 16}, {160, -4}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Market"), Text(extent = {{-6, 20}, {42, 0}}, lineColor = {0, 0, 0}, fontSize = 8, textString = "Receiver
Control"), Text(extent = {{30, 62}, {78, 42}}, lineColor = {0, 0, 0}, fontSize = 8, textString = "Power Block
Control"), Text(extent = {{-146, -26}, {-98, -46}}, lineColor = {0, 0, 0}, fontSize = 10, textString = "Data Source")}),
