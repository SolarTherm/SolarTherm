...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch_TEST
  ...
  parameter Real PV_fraction = 7.254694329 "Fraction of the hybrid system that is PV nameplate";
  parameter SI.Power P_hybrid_system = 2543311240 "Hybrid system nameplate [W] --> allow to be oversized";
  parameter Real t_storage_H2(unit = "h") = 182.0087346 "[ST] H2 Storage capacity";
  parameter Real t_storage(unit = "h") = 68.61480013 "[ST] Storage capacity";
  parameter Real PB_fraction = 0.0;
  parameter Real upstreammultiplier_change = 12.02637029 "The multiplier controlled by the optimisation, and passed";
  ...
  parameter SI.MassFlowRate H2_mdot_target = H2_mdot_numberup "Hydrogen production per second in that has to be supplied by the tank [kg/s]";
  ...
  parameter SI.Energy E_max = t_storage * 3600 * ((m_flow_blk + m_dot_pcl_DP_H2_HX) * (h_hot_set - h_cold_set)) "Maximum tank stored energy [J]";
  ...
  parameter Real c2 = 0 "Penalty for dumping heat from solar field";
  parameter Real c3 = 1 "Penalty for dumping electricity from PV";
  ...
  parameter Real A_sf = A_field * upstreammultiplier "Total area of the solar field [m2] -- has to be multiplied by upstreammultiplier";
  parameter Real PB_size = P_name / 1e6 * upstreammultiplier "PB name plate [MWe] -- has to be multiplied by upstreammultiplier";
  parameter Real TES_capacity = E_max * 2.77778e-10 * upstreammultiplier "TES capacity [MWhth] -- has to be multiplied by upstreammultiplier";
  parameter Real Q_TES_HX_max = Q_burner_emergency * eta_burner / 1e6 "Maximum dispatchable thermal power from the TES to the TES-heater to heat-up H2_reactor + H2_recycle [MWth] (Q_burner_emergency is already multiplied by upstream multiplier)";
  parameter Real E_TES_min = hot_tnk_empty_lb / 100 * TES_capacity "Minimum TES level [MWhth] (TES_capacity is already multiplied by upstream multiplier)";
...
*/
  if time > 3.15359e7 then
    ...
    if on_CSP then
      ...
      eta_pb_gross := if PB_fraction_final < 1e-3 then 0 else E_pb_gross / E_pb_input;
      eta_pb_net := if PB_fraction_final < 1e-3 then 0 else E_pb_net / E_pb_input;
...
//************************************ Dual tank dispatch optimiser variables
  E_TES_init = tankHot.L / 100 * TES_capacity "Initial level of TES [MWhth] -- TES_capacity has been multiplied by upstreammultiplier";
//E_TES_init = tankHot.L / 100 * m_max * upstreammultiplier * 2.77778e-10 * (Utils.h_T(tankHot.medium.T) - Utils.h_T(T_cold_set));
...
//******************** Obtain the DNI for the next horizon
        dni_dual_tank[i] = SolarTherm.Utilities.DNI_horizon(t_forecast_dual_tank[i], wea_table);
...
// PV solutions
    if PVArray.P_net > P_hybrid_system_final then
      ...
      fraction_P_PV_TES = if use_TES==false then 0 else (PVArray.P_net - P_hybrid_system_final) / PVArray.P_net "If no thermal, then no TES";
    else
      fraction_P_PV_AEL = 1;
      fraction_P_PV_TES = 0;
    end if;
...
// TES solutions
    fraction_Q_TES_PB = if use_TES==false or PB_fraction_final < 1e-3 then 0 else 1;
    fraction_Q_TES_HX = if use_TES==false then 0 else 1 "If no thermal, then no TES, no indirect particle gas HX";
// H2 stg solutions
...
end Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch_TEST;