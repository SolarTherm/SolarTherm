...
model SimpleSystemOptimalDispatch
    ...
    parameter STMotab.STMotab wea_motab = STMotab.STMotab(wea_file);
    parameter STMotab.STMotab pri_motab = STMotab.STMotab(pri_file);

    parameter nSI.Angle_deg lon(fixed=true) = STMotab.getMetaLon(wea_motab);
    parameter nSI.Angle_deg lat(fixed=true) = STMotab.getMetaLat(wea_motab);
    parameter nSI.Time_hour t_zone(fixed=true) = STMotab.getMetaTimezone(wea_motab);
    parameter Integer year = 1996 "Meteorological year (if needed for sunpos calcs)";
...
equation
    ...
    elseif blk_state == 2 then
        ...
        //Q_flow_dis = if ramp_order == 0 then if dispatch_optimiser == true
        //  then optimalDispatch*1e6 else Q_flow_sched else fr_ramp_blk * Q_flow_sched;
    ...
    der(TOD_W) = P_elec*pri.price * (1e6*3600) 
        "Multiplied by 3.6e9 to convert back into TOD";
