...
model PhysicalParticleCO21D_1stApproach_SurrogateReceiver_NewPB
  ...
  // Power block
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.recompPB_Modified_JPidea powerBlock(redeclare package MedRec = Medium, P_nom = P_net, T_HTF_in_des = T_in_ref_blk, T_amb_des = blk_T_amb_des, T_low = T_low, pinch_PHX = pinch_exchanger, external_parasities = external_parasities, nu_min = nu_min_blk, N_exch = N_exch_parameter "PG", N_LTR = N_LTR_parameter, f_fixed_load = f_fixed_load, PR = PR, pri_recuperator = pri_recuperator, pri_turbine = pri_turbine, pri_compressor = pri_compressor, pri_cooler = pri_cooler, pri_generator = pri_generator, pri_exchanger = pri_exchanger, eta_motor = eta_motor, T_HTF_out = T_cold_set, pinch_recuperator = pinch_recuperator, par_fr = par_fr, p_high = p_high) annotation(
...
initial equation
  Q_flow_des = powerBlock.Q_HX_des;
...
//PowerBlock connections
  ...
  connect(tankCold.L, particleReceiver.level) annotation(
    Line(points = {{44, -14}, {16, -14}, {16, 24}, {-20, 24}, {-20, 24}}, color = {0, 0, 127}));
  connect(particleReceiver.m_flow_out, controlHot.m_flow_in) annotation(
    Line(points = {{-22, 28}, {42, 28}, {42, 62}, {48, 62}, {48, 62}}, color = {0, 0, 127}));
  annotation(
