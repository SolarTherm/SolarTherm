within examples;
model NaSaltsCO2SystemOperation "High temperature Sodium-sCO2 system"
	...
	// Solar field
	SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatFieldOperation heliostatsField(
		n_h = n_heliostat,
		lon = data.lon,
		lat = data.lat,
	...
	parameter Integer horizon = integer(nu_start_sf*3600/dt) + 1 "Forecast horizon of the receiver dispatch algorithm";
	parameter Real dt = 150 "Forecast time step, in seconds";
	...
	Real time_simul "Current simulation second";
	SI.Time t_forecast;
...
equation
	heliostatsField.t_forecast = t_forecast;
	der(counter) = 1;
	when counter> 0 then
