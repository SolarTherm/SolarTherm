...
model Reference_1
  ...
  SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatsField heliostatsField(n_h = n_heliostat, lon = data.lon, lat = data.lat, ele_min(displayUnit = "deg") = 0.13962634015955, Wspd_max = 15, use_wind = true, he_av = 0.99, use_on = true, use_defocus = true, A_h = A_heliostat, nu_defocus = 1, nu_min = 0.3, Q_design = 330000000, nu_start = 0.6, redeclare model Optical = SolarTherm.Models.CSP.CRS.HeliostatsField.Optical.Table(angles = SolarTherm.Types.Solar_angles.elo_hra, file = file_optics)) annotation(
    Placement(transformation(extent = {{-88, 2}, {-56, 36}})));
  SolarTherm.Models.CSP.CRS.Receivers.ReceiverSimple receiver(em = 0.88, redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, H_rcv = H_receiver, D_rcv = D_receiver, N_pa = 20, t_tb = 1.25e-3, D_tb = 40e-3, ab = 0.94) annotation(
  ...
  SolarTherm.Models.Storage.Tank.Tank tankHot(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, T_start = from_degC(574), L_start = 30, alpha = 3, use_p_top = true, enable_losses = true, use_L = true, T_set = from_degC(500)) annotation(
    Placement(transformation(extent = {{16, 54}, {36, 74}})));
  SolarTherm.Models.Fluid.Pumps.PumpSimple pumpHot(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph) annotation(
    Placement(transformation(extent = {{66, 38}, {78, 50}})));
  SolarTherm.Models.Storage.Tank.Tank tankCold(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph, T_start = from_degC(290), L_start = 70, alpha = 3, use_p_top = true, enable_losses = true, use_L = true, T_set = from_degC(280)) annotation(
  ...
  SolarTherm.Models.Fluid.Sensors.Temperature temperature(redeclare package Medium = Media.MoltenSalt.MoltenSalt_ph) annotation(
  ...
  SolarTherm.Models.Control.PowerBlockControl controlHot(m_flow_on = 682.544, L_off = 5, L_df_on = 120, L_df_off = 123, L_on = 10) annotation(
    Placement(transformation(extent = {{48, 72}, {60, 58}})));
  SolarTherm.Models.Control.ReceiverControl controlCold(T_ref = from_degC(574), m_flow_max = 1400, y_start = 1000, L_df_on = 5, L_df_off = 7, L_off = 0, Ti = 0.1, Kp = -1000) annotation(
    Placement(transformation(extent = {{24, -10}, {10, 4}})));
  SolarTherm.Models.PowerBlocks.PowerBlockModel powerBlock(W_des = P_gross, enable_losses = true, redeclare model Cycle = SolarTherm.Models.PowerBlocks.Correlation.Rankine, nu_min = 0.5, external_parasities = true, W_base = 0.0055 * 111e6, p_bo = 10000000, T_des = 316.15, redeclare model Cooling = SolarTherm.Models.PowerBlocks.Cooling.SAM(T_des = 316.15)) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y = 1) annotation(
    Placement(visible = true, transformation(extent = {{-50, 38}, {-24, 58}}, rotation = 0)));
...
equation
  ...
  connect(heliostatsField.Q_incident, controlCold.Q_input) annotation(
    Line(points = {{-54, 20}, {10, 20}, {10, 6}, {10, 6}}, color = {0, 0, 127}));
  connect(realExpression.y, controlCold.eta_rec_in) annotation(
    Line(points = {{-22, 48}, {24, 48}, {24, 6}, {24, 6}}, color = {0, 0, 127}));
  connect(True.y, heliostatsField.on_hopper) annotation(
    Line(points = {{-116, 10}, {-112, 10}, {-112, 20}, {-88, 20}, {-88, 20}}, color = {255, 0, 255}));
  annotation(
    Diagram(coordinateSystem(extent = {{-140, -120}, {160, 140}}, initialScale = 0.1), graphics = {Text(lineColor = {217, 67, 180}, extent = {{4, 92}, {40, 90}}, textString = "defocus strategy", fontSize = 9), Text(lineColor = {217, 67, 180}, extent = {{-50, -40}, {-14, -40}}, textString = "on/off strategy", fontSize = 9), Text(extent = {{-52, 8}, {-4, -12}}, textString = "Receiver", fontSize = 10), Text(extent = {{-110, 4}, {-62, -16}}, textString = "Heliostats
Field", fontSize = 10), Text(extent = {{-80, 86}, {-32, 66}}, textString = "Sun", fontSize = 10), Text(extent = {{0, 58}, {48, 38}}, textString = "Hot Tank", fontSize = 10), Text(extent = {{30, -24}, {78, -44}}, textString = "Cold Tank", fontSize = 10), Text(extent = {{80, 12}, {128, -8}}, textString = "Power Block", fontSize = 10), Text(extent = {{112, 16}, {160, -4}}, textString = "Market", fontSize = 10), Text(extent = {{-6, 20}, {42, 0}}, textString = "Receiver
Control", fontSize = 8), Text(extent = {{30, 62}, {78, 42}}, textString = "Power Block
Control", fontSize = 8), Text(extent = {{-146, -26}, {-98, -46}}, textString = "Data Source", fontSize = 10)}),
