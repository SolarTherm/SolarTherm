...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch3
  ...
  parameter Real PV_fraction = 1.49937772041 "Fraction of the hybrid system that is PV nameplate";
  parameter SI.Power P_hybrid_system = 1968485792.43 "Hybrid system nameplate [W] --> allow to be oversized";
  parameter Real t_storage_H2(unit = "h") = 50.1263624096 "[ST] H2 Storage capacity";
  parameter Real t_storage(unit = "h") = 30.182264329 "[ST] Storage capacity";
  parameter Real PB_fraction = 0.2;
  parameter Real upstreammultiplier_change = 3 "The multiplier controlled by the optimisation, and passed";
  ...
  parameter SI.Mass m_max = if PB_fraction > 1e-3 then (m_dot_pcl_DP_H2_HX * 6 + m_flow_blk) * t_storage * 3600 else m_dot_pcl_DP_H2_HX * t_storage * 3600 "Max particles mass in tanks [kg]";
  ...
  parameter Real PB_size = if PB_fraction < 1e-3 then 0 else P_name / 1e6 "PB name plate [MWe] -- has to be multiplied by upstreammultiplier";
