...
//	replaceable package Medium = SolarTherm.Media.ChlorideSaltPH.ChlorideSaltPH_ph "Medium props for Chloride Salt";
	...
	parameter SI.Temperature T_hot_set = from_degC(800) "Hot tank target temperature";
	parameter SI.Temperature T_cold_start = T_cold_set "Cold tank starting temperature";
	parameter SI.Temperature T_hot_start = from_degC(800) "Hot tank starting temperature";
	parameter SI.Temperature T_cold_aux_set = from_degC(500) "Cold tank auxiliary heater set-point temperature";
	parameter SI.Temperature T_hot_aux_set = from_degC(700) "Hot tank auxiliary heater set-point temperature";
	...
	parameter SI.Area A_land = 0 "Land area"; //parameter SI.Area A_land = land_mult*A_field + 197434.207385281 "Land area";
	...
	parameter SI.Area SA_storage = pi*D_storage*H_storage "Storage tank surface area";
	...
	parameter FI.PowerPrice pri_bop = if currency == Currency.USD then 340*0/1e3 else 340*0/1e3/r_cur "Balance of plant cost per gross rated power"; // Based on downselection criteria criteria
	...
	parameter FI.Money C_receiver = C_fpr + C_tower "Total receiver cost";
//	parameter FI.Money C_bins = FI.particleBinCost(T_hot_set)*SA_storage + FI.particleBinCost(T_cold_set)*SA_storage "Cost of cold and hot storage bins";
	...
	parameter FI.Money C_storage = /*C_bins +*/ C_particles + f_loss * t_life * pri_particle * 1.753e10 "Total storage cost";
	...
	parameter FI.Money C_block (fixed=false) "Power block cost"; // TODO Should be updated based on Eq. 17 in Albrecht et al's ASME conference paper draft
