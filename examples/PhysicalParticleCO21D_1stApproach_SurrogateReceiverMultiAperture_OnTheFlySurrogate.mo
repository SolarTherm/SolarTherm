...
model PhysicalParticleCO21D_1stApproach_SurrogateReceiverMultiAperture_OnTheFlySurrogate
  ...
  parameter Real etaC = metadata_list[1];
  ...
  // Receiver
  parameter Real ar_rec_1 = 1 "Height to diameter aspect ratio of receiver aperture";
  parameter Real ar_rec_2 = 1 "Height to diameter aspect ratio of receiver aperture";
  parameter Real ar_rec_3 = 1 "Height to diameter aspect ratio of receiver aperture";
  
  parameter SI.Length H_rcv_1(fixed = false) "Receiver aperture height";
  parameter SI.Length H_rcv_2(fixed = false) "Receiver aperture height";
  parameter SI.Length H_rcv_3(fixed = false) "Receiver aperture height";
  
  parameter SI.Length W_rcv_1 = H_rcv_1 / ar_rec_1 "Receiver aperture width";
  parameter SI.Length W_rcv_2 = H_rcv_2 / ar_rec_2 "Receiver aperture width";
  parameter SI.Length W_rcv_3 = H_rcv_3 / ar_rec_3 "Receiver aperture width";
  
  parameter SI.Area A_rcv_1 = H_rcv_1 * W_rcv_1 "Receiver 1 aperture area";
  parameter SI.Area A_rcv_2 = H_rcv_2 * W_rcv_2 "Receiver 2 aperture area";
  parameter SI.Area A_rcv_3 = H_rcv_3 * W_rcv_3 "Receiver 3 aperture area";
  ...
  parameter FI.AreaPrice pri_receiver = 37400;
  ...
  parameter Real C_target = helio_refl * 100 "annual reflectivity target";
  ...
  parameter FI.Money C_tower = 
      if set_SAM_tower_cost then 
        ...
        /*C_tower = Tim Harvey structure only cost [USD]- SBP Materials [USD]+ Sam Tower Cost [USD]- Piping Cost [in Euro] + Ducting cost [USD]*/
            ...
            pri_tower_fix_SAM * Modelica.Math.exp(pri_tower_scalar_exp_SAM * (H_tower + 0.5 * H_helio - (H_rcv_1+H_rcv_2+H_rcv_3)/3 / 2)) - //==========> SAM Cost v2020.2.29
  ...
  parameter FI.Money C_fpr = pri_receiver * (A_rcv_1 + A_rcv_2 + A_rcv_3) "Falling particle receiver cost";
  parameter FI.Money C_lift_rec = pri_lift * dh_liftRC * m_flow_fac "Receiver lift cost - for n number for apertures";
  ...
  Modelica.Blocks.Sources.RealExpression parasities_input(
      y = heliostatsField.W_loss + liftHX.W_loss + 
  ...
  SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatsFieldSolstice_3Apertures_1stApproach heliostatsField(
      ...
      nu_min = nu_min_sf, 
      Q_design_1 = Q_flow_defocus_calculated1, 
      Q_design_2 = Q_flow_defocus_calculated2, 
      Q_design_3 = Q_flow_defocus_calculated3, 
      nu_start = nu_start, 
      Q_in_rcv_1 = Q_in_rcv_1, //speak to Ye, this one is not applicable in this case since the system already calculates the Q_in_rcv for each aperture
      Q_in_rcv_2 = Q_in_rcv_2, //speak to Ye, this one is not applicable in this case since the system already calculates the Q_in_rcv for each aperture
      Q_in_rcv_3 = Q_in_rcv_3, //speak to Ye, this one is not applicable in this case since the system already calculates the Q_in_rcv for each aperture
      H_rcv_1 = H_rcv_1, 
      H_rcv_2 = H_rcv_2, 
      H_rcv_3 = H_rcv_3, 
      W_rcv_1 = W_rcv_1, 
  ...
  //************************* Surrogate Receiver
  SolarTherm.Models.CSP.CRS.Receivers.ParticleReceiver_3Apertures particleReceiver(
      H_drop_design = H_rcv_1, 
  ...
  SolarTherm.Models.CSP.CRS.Receivers.ParticleReceiver1DCalculator particleReceiver1DCalculator1(
      Q_in = Q_in_rcv_1, 
  ...
  //*************************  Receiver Design Condition Sizing Calculator - Using expensive model
  SolarTherm.Models.CSP.CRS.Receivers.ParticleReceiver1DCalculator particleReceiver1DCalculator2(
      Q_in = Q_in_rcv_2, 
      T_out_design = T_in_ref_blk, 
      T_in_design = T_in_rec, 
      T_amb_design = T_amb_des_rcv, 
  ...
  //*************************  Receiver Design Condition Sizing Calculator - Using expensive model
  SolarTherm.Models.CSP.CRS.Receivers.ParticleReceiver1DCalculator particleReceiver1DCalculator3(
      Q_in = Q_in_rcv_3, 
      T_out_design = T_in_ref_blk, 
      T_in_design = T_in_rec, 
      T_amb_design = T_amb_des_rcv, 
  ...
  //************************* Hot tank
  SolarTherm.Models.Storage.Tank.Tank tankHot(redeclare package Medium = Medium, D = D_storage, H = H_storage, T_start = T_hot_start, L_start = (1 - split_cold) * 100, alpha = alpha, use_p_top = set_tnk_use_p_top, enable_losses = set_tnk_enable_losses, use_L = true, W_max = W_heater_hot, T_set = T_hot_aux_set, U_value = U_value_hot_tank, packing_factor = packing_factor) annotation(
    Placement(transformation(extent = {{16, 54}, {36, 74}})));
  
  //************************* Cold tank
  SolarTherm.Models.Storage.Tank.Tank tankCold(redeclare package Medium = Medium, D = D_storage, H = H_storage, T_start = T_cold_start, L_start = split_cold * 100, alpha = alpha, use_p_top = set_tnk_use_p_top, enable_losses = set_tnk_enable_losses, use_L = true, W_max = W_heater_cold, T_set = T_cold_aux_set, U_value = U_value_cold_tank, packing_factor = packing_factor) annotation(
...
initial equation
  ...
  H_rcv_1 = particleReceiver1DCalculator1.particleReceiver1D.H_drop;
  H_rcv_2 = particleReceiver1DCalculator2.particleReceiver1D.H_drop;
  H_rcv_3 = particleReceiver1DCalculator3.particleReceiver1D.H_drop;
...
//PowerBlock connections
  ...
  connect(Wspd_input.y, wspdScaler.WspdGround) annotation(
    Line(points = {{-112, 30}, {-102, 30}, {-102, 44}, {-66, 44}, {-66, 44}}, color = {0, 0, 127}));
  connect(wspdScaler.WspdOut, particleReceiver.Wspd) annotation(
    Line(points = {{-44, 44}, {-40, 44}, {-40, 60}, {-26, 60}, {-26, 42}, {-26, 42}}, color = {0, 0, 127}));
  annotation(
...
end PhysicalParticleCO21D_1stApproach_SurrogateReceiverMultiAperture_OnTheFlySurrogate;