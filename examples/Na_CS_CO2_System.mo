...
model Na_CS_CO2_System "High temperature Sodium-sCO2 system"
  ...
  parameter SI.HeatFlowRate Q_flow_defocus = 1.3 * Q_flow_des "Solar field thermal power at defocused state"; // This only works if const_dispatch=true. TODO for variable disptach Q_flow_defocus should be turned into an input variable to match the field production rate to the dispatch rate to the power block.
  ...
  parameter Real Kp = -750 "Gain of proportional component in receiver control";
  parameter Real Ti_CS = 0.1 "Time constant for integral component of receiver control";
  parameter Real Kp_CS = Kp+8.5 "Gain of proportional component in receiver control";
  ...
  parameter FI.EnergyPrice pri_storage = if currency == Currency.USD then 37 / (1e3 * 3600) else 37 / (1e3 * 3600) / r_cur "Storage cost per energy capacity"; // SAM 2018 cost data: 22 / (1e3 * 3600)
  ...
  parameter FI.Money C_receiver = if currency == Currency.USD then 71708855 * (A_receiver / 879.8) ^ 0.7 else 71708855 * (A_receiver / 879.8) ^ 0.7 / r_cur "Receiver cost"; // SAM 2018 cost data: 103e6 * (A_receiver / 1571) ^ 0.7
  ...
  parameter FI.Money C_EPC = 0.11 * C_cap_dir_tot "Engineering, procurement and construction(EPC) and owner costs"; // SAM 2018 cost data: 0.13
  ...
  SolarTherm.Models.Sources.DataTable.DataTable data(lon = lon, lat = lat, t_zone = t_zone, year = year, file = wea_file) annotation(
    Placement(visible = true, transformation(extent = {{-136, -82}, {-106, -54}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression Tamb_input(y = data.Tdry) annotation(
    Placement(visible = true, transformation(extent = {{144, 98}, {124, 118}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression Pres_input(y = data.Pres) annotation(
    Placement(visible = true, transformation(extent = {{140, -20}, {120, 0}}, rotation = 0)));
  ...
  SolarTherm.Models.Fluid.Pumps.PumpSimple pumpCold1(redeclare package Medium = Medium1, k_loss = k_loss_cold) annotation(
    Placement(visible = true, transformation(extent = {{-12, -42}, {-24, -30}}, rotation = 0)));
  ...
  SolarTherm.Models.Fluid.HeatExchangers.buffer_tank SodiumBufferTank(replaceable package Medium = Medium1) annotation(
...
initial equation
  ...
  if H_tower > 120 then // then use concrete tower
    C_tower = if currency == Currency.USD then 3117043.67 * exp(0.0113 * H_tower) else 3117043.67 * exp(0.0113 * H_tower) / r_cur "Tower cost"; //SAM 2018 cost data: 3e6 * exp(0.0113 * H_tower) in USD
  else // use Latticework steel tower
    C_tower = if currency == Currency.USD then 1.09025e6 * exp(0.00879 * H_tower) else 1.09025e6 * exp(0.00879 * H_tower) / r_cur "Tower cost"; // SAM 2018 cost data: 1.09025e6 * (603.1/318.4) * exp(0.00879 * H_tower)
  end if;
...
equation
  ...
  connect(Pres_input.y, SodiumBufferTank.p_top) annotation(
    Line(points = {{120, -10}, {42, -10}, {42, -26}, {7, -26}, {7, -28}}, color = {0, 0, 127}));
  connect(Pres_input.y, tankCold.p_top) annotation(
    Line(points = {{120, -10}, {84, -10}, {84, -22}, {84, -22}}, color = {0, 0, 127}));
  ...
  connect(Tamb_input.y, tankCold.T_amb) annotation(
    Line(points = {{123, 108}, {116, 108}, {116, 42}, {108, 42}, {108, 20}, {92, 20}, {92, -22}}, color = {0, 0, 127}));
  connect(Tamb_input.y, powerBlock.T_amb) annotation(
    Line(points = {{123, 108}, {116, 108}, {116, 34}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(Tamb_input.y, tankHot.T_amb) annotation(
    Line(points = {{123, 108}, {54, 108}, {54, 74}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(Pres_input.y, tankHot.p_top) annotation(
    Line(points = {{119, -10}, {42, -10}, {42, 84}, {62, 84}, {62, 74}}, color = {0, 0, 127}));
  ...
  connect(hX_Control.m_flow_rec, pumpCold1.m_flow) annotation(
    Line(points = {{35, -44}, {34, -44}, {34, -22}, {-18, -22}, {-18, -31}}, color = {0, 0, 127}));
  ...
  connect(pumpCold1.fluid_a, SodiumBufferTank.fluid_b) annotation(
    Line(points = {{-12, -36}, {-1, -36}, {-1, -36.5}, {4, -36.5}}, color = {0, 127, 255}));
  ...
  annotation(
    Diagram(coordinateSystem(extent = {{-140, -120}, {160, 140}}, initialScale = 0.1), graphics = {Text(lineColor = {217, 67, 180}, extent = {{4, 92}, {40, 90}}, textString = "defocus strategy", fontSize = 9), Text(origin = {0, -18}, lineColor = {217, 67, 180}, extent = {{-50, -40}, {-14, -40}}, textString = "on/off strategy", fontSize = 9), Text(origin = {-8, 2}, extent = {{-52, 8}, {-4, -12}}, textString = "Receiver", fontSize = 3, fontName = "CMU Serif"), Text(origin = {6, 2}, extent = {{-110, 4}, {-62, -16}}, textString = "Heliostats Field", fontSize = 3, fontName = "CMU Serif"), Text(origin = {-26, 6}, extent = {{-80, 86}, {-32, 66}}, textString = "Sun", fontSize = 3, fontName = "CMU Serif"), Text(origin = {34, 2}, extent = {{0, 58}, {48, 38}}, textString = "Hot Tank", fontSize = 3, fontName = "CMU Serif"), Text(origin = {36, -14}, extent = {{30, -24}, {78, -44}}, textString = "Cold Tank", fontSize = 3, fontName = "CMU Serif"), Text(origin = {16, 2}, extent = {{80, 12}, {128, -8}}, textString = "Power Block", fontSize = 3, fontName = "CMU Serif"), Text(origin = {14, 2}, extent = {{112, 16}, {160, -4}}, textString = "Market", fontSize = 3, fontName = "CMU Serif"), Text(origin = {22, -88}, extent = {{-6, 20}, {42, 0}}, textString = "HX Control", fontSize = 3, fontName = "CMU Serif"), Text(origin = {48, 38}, extent = {{30, 62}, {78, 42}}, textString = "Power Block Control", fontSize = 3, fontName = "CMU Serif"), Text(origin = {0, -50}, extent = {{-146, -26}, {-98, -46}}, textString = "Data Source", fontSize = 3, fontName = "CMU Serif"), Text(origin = {48, 22}, extent = {{-52, 8}, {-4, -12}}, textString = "Heat Exchanger", fontSize = 3, fontName = "CMU Serif")}),
