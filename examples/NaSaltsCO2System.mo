...
model NaSaltsCO2System "High temperature Sodium-sCO2 system"
  ...
  parameter SI.RadiantPower R_des(fixed = if fixed_field then true else false) = 619.77e6 "Input power to receiver at design point";
  ...
  parameter Real SM = 547.03 / (111 / eff_blk) "Solar multiple";
  ...
  parameter SI.Efficiency eff_opt = 619.77e6 / (6764 * A_heliostat * he_av_design * dni_des) "Field optical efficiency at design point";
  ...
  parameter FI.AreaPrice pri_field = if currency == Currency.USD then 75 else 75 / r_cur "Field cost per design aperture area"; //75 $/m2 from SAM 2020 High Temperature Salts
  parameter FI.AreaPrice pri_site = if currency == Currency.USD then 10 else 10 / r_cur "Site improvements cost per area"; //10 $/m2 from SAM 2020 High Temperature Salts
  parameter FI.EnergyPrice pri_storage = if currency == Currency.USD then 40 / (1e3 * 3600) else 40 / (1e3 * 3600) / r_cur "Storage cost per energy capacity"; //40 $/kWh from SAM 2020 High Temperature Salts
  parameter FI.PowerPrice pri_block = if currency == Currency.USD then 900 / 1e3 else 900 / r_cur "Power block cost per gross rated power"; //900 $/kW from SAM 2020 High Temperature Salts
  parameter FI.PowerPrice pri_bop = if currency == Currency.USD then 350 / 1e3 else 350 / 1e3 / r_cur "Balance of plant cost per gross rated power"; //UPDATE ME!
  parameter FI.AreaPrice pri_land = if currency == Currency.USD then 10000 / 4046.86 else 10000 / 4046.86 / r_cur "Land cost per area"; //10000 $/acre from DownSelection Criteria NREL
  parameter Real pri_om_name(unit = "$/W/year") = if currency == Currency.USD then 40 / 1e3 else 40 / 1e3 / r_cur "Fixed O&M cost per nameplate per year"; //40 $/kW-yr from target value DownSelection Criteria NREL
  parameter Real pri_om_prod(unit = "$/J/year") = if currency == Currency.USD then 3 / (1e6 * 3600) else 3 / (1e6 * 3600) / r_cur "Variable O&M cost per production per year"; //3 $/MWh from target value DownSelection Criteria NREL
  ...
  parameter FI.Money_USD C_receiver = if currency == Currency.USD then 133396250.56 * (A_receiver / 1571) ^ 0.7 else 133396250.56 * (A_receiver / 1571) ^ 0.7 / r_cur "Receiver cost";
  ...
  // Pump cold1
  SolarTherm.Models.Fluid.Pumps.Pump_PressureLosses pumpCold1(redeclare package Medium = Medium1, k_loss = k_loss_cold) annotation(
