...
model NaSaltsCO2System "High temperature Sodium-sCO2 system"
  ...
  Modelica.Blocks.Sources.RealExpression PressureLosses_CS_loop(y = Shell_and_Tube_HX.Dp_shell) annotation(
    Placement(visible = true, transformation(extent = {{-22, 52}, {-2, 72}}, rotation = 0)));
  ...
  //HX
  SolarTherm.Models.Fluid.HeatExchangers.HX Shell_and_Tube_HX(replaceable package Medium1 = Medium1, replaceable package Medium2 = Medium2, T_Na2_input = T_Na2_input, Q_d_des = Q_rec_out, optimize_and_run = optimize_HX_design, d_o_input = d_o_input, N_p_input = N_p_input, layout_input = layout_input, ratio_cond = ratio_cond, ratio_max = ratio_max, L_max_cond = L_max_cond, L_max_input = L_max_input) annotation(
  ...
  // Hot tank
  SolarTherm.Models.Storage.Tank.Two_Tanks tankHot(redeclare package Medium = Medium2, D = D_storage, H = H_storage, T_start = T_hot_start_CS, L_start = (1 - split_cold) * 100, alpha = alpha, use_p_top = tnk_use_p_top, enable_losses = tnk_enable_losses, use_L = true, W_max = W_heater_hot, T_set = T_hot_aux_set) annotation(
    Placement(visible = true, transformation(extent = {{50, 52}, {70, 72}}, rotation = 0)));
  // Pump hot
  ...
  // Cold tank
  SolarTherm.Models.Storage.Tank.Two_Tanks tankCold(redeclare package Medium = Medium2, D = D_storage, H = H_storage, T_start = T_cold_start_CS, L_start = split_cold * 100, alpha = alpha, use_p_top = tnk_use_p_top, enable_losses = tnk_enable_losses, use_L = true, W_max = W_heater_cold, T_set = T_cold_aux_set) annotation(
    Placement(visible = true, transformation(extent = {{100, -42}, {80, -22}}, rotation = 0)));
  // Pump cold 2NaS
...
equation
  ...
  connect(tankCold.fluid_a, powerBlock.fluid_b) annotation(
    Line(points = {{100, -27}, {100, 14}, {110, 14}}, color = {0, 127, 255}));
  ...
  connect(temperature2.fluid_a, tankCold.fluid_b) annotation(
    Line(points = {{74, -6}, {74, -40}, {80, -40}, {80, -39}}, color = {0, 127, 255}));
  ...
  connect(Shell_and_Tube_HX.port_b_out, tankHot.fluid_a) annotation(
    Line(points = {{14, 6}, {6, 6}, {6, 68}, {50, 68}, {50, 67}}, color = {0, 127, 255}));
  ...
  connect(receiver.Tamb, tankHot.T_amb) annotation(
    Line(points = {{-36, 36}, {-36, 80}, {56, 80}, {56, 72}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  ...
  connect(Tamb_input.y, tankHot.T_amb) annotation(
    Line(points = {{157, 100}, {56, 100}, {56, 72}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(Tamb_input.y, tankCold.T_amb) annotation(
    Line(points = {{157, 100}, {116, 100}, {116, 42}, {108, 42}, {108, 20}, {94, 20}, {94, -22}}, color = {0, 0, 127}));
  connect(Pres_input.y, tankHot.p_top) annotation(
    Line(points = {{119, -12}, {42, -12}, {42, 84}, {64.5, 84}, {64.5, 72}}, color = {0, 0, 127}));
  connect(Pres_input.y, tankCold.p_top) annotation(
    Line(points = {{119, -12}, {85.5, -12}, {85.5, -22}}, color = {0, 0, 127}));
...
//controlHot connections
  ...
  connect(tankHot.L, controlHot.L_mea) annotation(
    Line(points = {{70, 66}, {72, 66}, {72, 76.5}, {98, 76.5}}, color = {0, 0, 127}));
...
//HX_control connections
  connect(hX_Control.L_mea, tankCold.L) annotation(
    Line(points = {{38, -66}, {38, -74}, {70, -74}, {70, -28}, {80, -28}}, color = {0, 0, 127}));
...
//Fluid connections
  connect(tankHot.fluid_b, pumpHot.fluid_a) annotation(
    Line(points = {{70, 55}, {72, 55}, {72, 48}, {78, 48}}, color = {0, 127, 255}));
