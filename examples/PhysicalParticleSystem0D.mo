...
model PhysicalParticleSystem0D
  ...
  parameter Real he_av_des = 1.00 "Helisotats availability ";
  ...
  SolarTherm.Models.Control.ReceiverControl controlCold(Kp = Kp, L_df_off = cold_tnk_defocus_ub, L_df_on = cold_tnk_defocus_lb, L_off = cold_tnk_crit_lb, L_on = cold_tnk_crit_ub, T_ref = T_hot_set, Ti = Ti, m_flow_max = m_flow_rec_max, m_flow_min = m_flow_rec_min, y_start = m_flow_rec_start) annotation(
...
//PowerBlock connections
  ...
  annotation(
    ...
    experiment(StopTime = 10000, StartTime = 0, Tolerance = 0.0001, Interval = 0.570776),
