...
model PhysicalParticleCO21D_2ndApproach_CurveFitRcv
  ...
  //Cooler parameters
  parameter SI.ThermodynamicTemperature T_low = 41 + 273.15 "Inlet temperature of the compressor, default from Simon is 45 degC";
  ...
  parameter SI.Temperature blk_T_amb_des = 39 + 273.15 "Ambient temperature at design for power block, default from Simon is 43 degC";
  ...
  parameter Real Kp = -1 "Gain of proportional component in receiver control";
  ...
  parameter SI.MassFlowRate m_flow_rec_start = 0.1 * m_flow_fac "Initial https://pubs.acs.org/doi/pdf/10.1021/jp206115por guess value of mass flow rate to receiver in the feedback controller";
  ...
  parameter FI.Money C_storage = C_bins + C_particles + C_lift_hx + C_lift_cold + C_insulation + f_loss * t_life * pri_particle * 1.7132e10 "Total storage cost";
  //PB-subsystem cost
  ...
  //WindDirection_input
  Modelica.Blocks.Sources.RealExpression Wdir_input(y = data.Wdir) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression parasities_input(y = heliostatsField.W_loss + liftHX.W_loss + liftRC.W_loss + tankHot.W_loss + tankCold.W_loss) annotation(
    Placement(visible = true, transformation(origin = {110, 59}, extent = {{-12, -8}, {12, 8}}, rotation = -90)));
  ...
  // Receiver control
  SolarTherm.Models.Control.ReceiverControl receiverControl(T_ref = T_hot_set, m_flow_min = m_flow_rec_min, m_flow_max = m_flow_rec_max, y_start = m_flow_rec_start, L_df_on = cold_tnk_defocus_lb, L_df_off = cold_tnk_defocus_ub, L_off = cold_tnk_crit_lb, L_on = cold_tnk_crit_ub, Ti = Ti, Kp = Kp) annotation(
...
equation
  ...
  der(E_recv_net) = particleReceiver1D.Q_rcv;
...
//PowerBlock connections
  connect(parasities_input.y, powerBlock.parasities) annotation(
    Line(points = {{110, 46}, {110, 40.85}, {109.6, 40.85}, {109.6, 34.4}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
...
end PhysicalParticleCO21D_2ndApproach_CurveFitRcv;