...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch2
  ...
  parameter Real upstreammultiplier_change = 6.1 "The multiplier controlled by the optimisation, and passed";
  ...
  parameter SI.Power P_hybrid_system = 6e8 "Hybrid system nameplate [W] --> allow to be oversized";
  ...
  parameter Real PV_fraction = 2.0 "Fraction of the hybrid system that is PV nameplate";
  ...
  parameter Real PB_fraction = 1.0;
  ...
  parameter Real t_storage(unit = "h") = 10 "[ST] Storage capacity";
  ...
  parameter Real t_storage_H2(unit = "h") = 10 "[ST] H2 Storage capacity";
  ...
  //********************* Temperature sensor
  //********************* PowerBlockControl
  SolarTherm.Models.Control.PowerBlockControl_PVCSP_Particle_H2O2Storage controlHot(m_flow_on = m_flow_blk, L_on = hot_tnk_empty_ub, L_off = hot_tnk_empty_lb, L_df_on = hot_tnk_full_ub, L_df_off = hot_tnk_full_lb, logic.dispatch_optimiser = set_dispatch_optimiser, logic.set_scheduler = set_scheduler, P_net = P_hybrid_system_final / upstreammultiplier, CSP_name_plate = P_net, Q_HX_des = Q_flow_des, eta_gross_base = eff_blk, t_storage = t_storage, logic.P_net = P_hybrid_system_final / upstreammultiplier, optimal_dispatch_dual_tank = set_dispatch_optimiser_dual_tank) annotation(
  ...
  //********************* PV Array
  //********************* Electrolyser
  SolarTherm.Models.Electrochemical.Simple_Electrolyser electrolyser(P_electro_requested = P_hybrid_system_final, with_storage = true) annotation(
...
//******************** If dispatch optimiser dual tank is ON then populate the array
    ...
    when counter_dual_tank > 0 then
      time_simul_dual_tank = floor(time) "Rounding down the time";
