...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch2
  ...
  parameter Real upstreammultiplier_change = 0.7022336869 "The multiplier controlled by the optimisation, and passed";
  ...
  parameter SI.Power P_hybrid_system = 527959370.5 "Hybrid system nameplate [W] --> allow to be oversized";
  ...
  parameter Real PV_fraction = 1.207844243 "Fraction of the hybrid system that is PV nameplate";
  ...
  parameter Real PB_fraction = 0.2321161957;
  ...
  parameter Real t_storage(unit = "h") = 7.956219312 "[ST] Storage capacity";
  ...
  parameter Real t_storage_H2(unit = "h") = 74.76488016 "[ST] H2 Storage capacity";
...
// PV solutions
      fraction_P_PV_AEL = max(0, optimalSolution[1]);
      fraction_P_PV_TES = max(0, optimalSolution[2]);
      fraction_P_PV_dumped = max(0, optimalSolution[3]);
// TES solutions
      fraction_Q_TES_PB = max(0, optimalSolution[4]);
      fraction_Q_TES_HX = max(0, optimalSolution[5]);
// H2 stg solutions
      fraction_Q_H2_reactor = max(0, optimalSolution[6]);
      fraction_Q_H2_burner = max(0, optimalSolution[7]);
// SF solutions
      fraction_Q_SF_TES = max(0, optimalSolution[8]);
      fraction_Q_SF_dumped = max(0, optimalSolution[9]);
...
protected
  annotation(
    ...
    experiment(StopTime = 72000, StartTime = 0, Tolerance = 1e-06, Interval = 3600),
