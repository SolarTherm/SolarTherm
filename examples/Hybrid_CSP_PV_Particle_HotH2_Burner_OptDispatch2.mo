...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch2
  ...
  parameter Real t_storage(unit = "h") = 15.78370254 "[ST] Storage capacity";
  parameter Real PB_fraction = 0.6317058233;
  parameter Real PV_fraction = 2.588274595 "Fraction of the hybrid system that is PV nameplate";
  parameter SI.Power P_hybrid_system = 1111675531 "Hybrid system nameplate [W] --> allow to be oversized";
  parameter Real t_storage_H2(unit = "h") = 43.1407654 "[ST] H2 Storage capacity";
  ...
  parameter SI.Power P_net = if PB_fraction_final < 1e-3 then P_net_default_value else PB_fraction_final * P_hybrid_system_final / upstreammultiplier;
  /*
...
//********************************* Cost of the indirect particle-gas-HX between particle and H2
  C_exchanger_indirect = if f_TES < 1e-3 then 0 else (pri_PHX_per_area * A_HX_H2 + pri_PHX_BOP_s * heatExchanger_ParticleGasIndirectH2.designPoint.m_dot_pcl_DP + pri_PHX_BOP_CO2 * heatExchanger_ParticleGasIndirectH2.designPoint.m_dot_gas_DP) * upstreammultiplier;
...
// LP unsolved
        if PVArray.P_net > electrolyser.P_electro_requested then
          ...
          controlHot.logic.fraction_Q_TES_PB = min(1, (electrolyser.P_electro_requested - PVArray.P_net) / (upstreammultiplier * P_net));
        end if;
...
// TES solutions
    fraction_Q_TES_PB = if set_no_thermal then 0 else 1;
