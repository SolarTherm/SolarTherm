...
model Hybrid_CSP_PV_Particle_HotH2_Burner_OptDispatch2
  ...
  //********************* PV Array
  //********************* Electrolyser
  SolarTherm.Models.Electrochemical.Simple_Electrolyser electrolyser(P_electro_requested = P_hybrid_system_final, with_storage = true, upstreammultiplier = upstreammultiplier) annotation(
...
//****************** Call the PV function to populate array of PV net power in
        P_PV_in_z[i] = SolarTherm.Utilities.PVArray_func(N_parallel_final_PV, N_series_final_PV, dhi_dual_tank[i], albedo_horizon_dual_tank[i], Wspd_horizon_dual_tank[i], Tdry_horizon_dual_tank[i], PV_Target, azi_s, ele_s, lat, ele_min, dni_dual_tank[i], dec_horizon_dual_tank[i], if abs(time - 43200) < 1e-3 then 1e-6 else hra_horizon_dual_tank[i], zen_horizon_dual_tank[i], ele_horizon_dual_tank[i], azi_horizon_dual_tank[i]) / 1e6 "Net PV power production over the next forecast horizon in MWe";
...
// PV solutions
    if PVArray.P_net > P_hybrid_system_final then
      ...
      fraction_P_PV_TES = (PVArray.P_net - P_hybrid_system_final) / PVArray.P_net;
    else
      fraction_P_PV_AEL = 1;
      fraction_P_PV_TES = 0;
    end if;
...
//fraction_P_PV_dumped;
    controlHot.logic.fraction_Q_TES_PB = 1;
    heatExchanger_ParticleGasIndirectH2.fraction_Q_TES_HX = fraction_Q_TES_HX;
    tank_H2.fraction_Q_H2_reactor = 1;
    tank_H2.fraction_Q_TES_HX = 1;
    tank_H2.fraction_Q_H2_burner = 1;
