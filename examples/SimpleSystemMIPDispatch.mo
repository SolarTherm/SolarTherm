...
model SimpleSystemMIPDispatch
	...
	parameter SI.Area A_field = 700 "Area of collector";
	parameter SI.Area A_rec = 1 "Area of receiver aperture";
	parameter Real C = 0.65*A_field/A_rec "Concentration ratio";
	...
	parameter SI.HeatFlowRate Q_flow_sched = P_name/eff_blk "Discharge schedule";
	...
	parameter FI.MoneyPerYear C_year = 10*A_field "Operation and maintenance cost per year";
	...
	Real eff_opt_horizon[horizon] "The forecasted optical efficiency";
	Real eff_rec_horizon[horizon] "The forecasted receiver efficiency";
	Real dni_horizon[horizon] "The forecasted DNI";
	Real tod_horizon[horizon] "The forecasted TOD";
	...
	Modelica.Blocks.Types.ExternalCombiTable1D tod_table = Modelica.Blocks.Types.ExternalCombiTable1D(
		tableName = "prices",
		fileName = pri_file,
		table = fill(0.0, 0, 2),
		columns = 1:2,
...
equation
	...
	der(counter) = 1;
    SLinit = CV.to_kWh(E)/1e3 "Initial stored energy at TES [MW]"; 	
    SLmax = CV.to_kWh(E_max)/1e3 "Maximum stored energy at TES [MW]";
    SLminrel = E_low_l/E_max "Lowest energy fraction in the tank [-]";
    
    for i in 1: horizon loop
        eff_opt_horizon[i] = 0.65;
        eff_rec_horizon[i] = eff_rec;
        dni_horizon[i] = SolarTherm.Utilities.horizon_function(if time_simul + i*dt*3600 < 31536000 then time_simul + (i-1)*dt*3600 else time_simul + (i-1)*dt*3600 - 31536000, 3, wea_table);
        tod_horizon[i] = SolarTherm.Utilities.horizon_function(if time_simul + i*dt*3600 < 31536000 then time_simul + (i-1)*dt*3600 else time_simul + (i-1)*dt*3600 - 31536000, 2, tod_table);
    end for;
    
    when counter > 0 then
        time_simul = floor(time);
        optimalDispatch = SolarTherm.Utilities.MIPProgFunc(
            tod_horizon,
            horizon, 
            dt, 
            time_simul, 
