model test_HX_plus_Rec
  ...
  parameter SI.Temperature T_cold_set = CV.from_degC(670) "Cold Sodium target temperature"; //Update with design Tna2
  ...
  SolarTherm.Models.Sources.DataTable.DataTable data(lon = lon, lat = lat, t_zone = t_zone, year = year, file = wea_file) annotation(
    Placement(visible = true, transformation(extent = {{-134, -58}, {-104, -30}}, rotation = 0)));
  //DNI_input
  ...
  // Receiver
  SolarTherm.Models.CSP.CRS.Receivers.ReceiverSimple_2 receiver(em = em_rec, redeclare package Medium = Medium1, H_rcv = H_receiver, D_rcv = D_receiver, N_pa = N_pa_rec, t_tb = t_tb_rec, D_tb = D_tb_rec, ab = ab_rec) annotation(
  ...
  //HX
  SolarTherm.Models.Fluid.HeatExchangers.HX Shell_and_Tube_HX(Q_d_des = Q_flow_des*SM, replaceable package Medium1 = Medium1, replaceable package Medium2 = Medium2) annotation(
  ...
  Modelica.Fluid.Sources.MassFlowSource_T MS_inlet(replaceable package Medium = Medium2, T(start = 700, nominal = 700), nPorts = 1, use_T_in = true, use_m_flow_in = true) annotation(
    Placement(visible = true, transformation(origin = {131, 25}, extent = {{7, -7}, {-7, 7}}, rotation = 0)));
initial equation
  if fixed_field then
...
// Fluid connections
  MS_inlet.T_in = T_MS1_des;
  connect(Shell_and_Tube_HX.port_a_out, pumpCold.fluid_a) annotation(
    Line(points = {{82, 46}, {96, 46}, {96, -24}, {46, -24}, {46, -24}}, color = {0, 127, 255}));
  ...
  connect(Shell_and_Tube_HX.port_b_in, MS_inlet.ports[1]) annotation(
    Line(points = {{74, 42}, {74, 25}, {124, 25}}, color = {0, 127, 255}));
    
// controlCold connections
  connect(temperature.T, controlCold.T_mea) annotation(
    Line(points = {{27, 66}, {-10, 66}, {-10, 24}, {38, 24}, {38, 1}, {61, 1}}, color = {0, 0, 127}));
