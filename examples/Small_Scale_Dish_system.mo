...
model Small_Scale_Dish_system
  ...
  //Free Parameters
  //**********************
  parameter Real SM = 2.67 "Solar multiple";
  parameter Real t_storage(unit = "h") = 15.97 "Hours of storage";
  //Packages
  //**********************
  ...
  parameter SI.HeatFlowRate Q_flow_defocus = Q_PB_in_des; //Unused
  ...
  parameter Real r_cur = 0.71 "The currency rate from AUD to USD";//Unused
  ...
  parameter FI.Money C_cap_dir_sub = (1 - f_Subs) * (C_field + C_site + C_receiver + C_storage + C_block + C_bop) "Direct capital cost subtotal";
  ...
  // Receiver
  SolarTherm.Models.CSP.CRS.Receivers.PBS_Receiver_Dish receiver(em = em_rec, redeclare package Medium = Medium, H_rcv = H_receiver, D_rcv = D_receiver, ab = ab_rec, A=A_receiver) annotation(
