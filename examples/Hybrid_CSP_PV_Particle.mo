...
model Hybrid_CSP_PV_Particle
  ...
  parameter SI.Power P_net_default_value = 12345678 "Default value to handle P_net = 0 [W]";
  ...
  //********************* Parasitic inputs
  Modelica.Blocks.Sources.RealExpression parasities_input(y = if set_external_storage then heliostatsField.W_loss + liftHX.W_loss + liftRC.W_loss + tankHot.W_loss + tankCold.W_loss + liftHeater.W_loss else heliostatsField.W_loss + 0 + liftRC.W_loss + tankHot.W_loss + tankCold.W_loss + liftHeater.W_loss) annotation(
  ...
  SolarTherm.Models.UtilitiesComponent.SimpleElectricalHeater simpleElectricalHeater(W_dumped = if (P_CSP + PV_Target - P_hybrid_system) > 0 then (P_CSP + PV_Target - P_hybrid_system) else 0) annotation(
    Placement(visible = true, transformation(origin = {-12, -136}, extent = {{-44, -44}, {44, 44}}, rotation = 0)));
  //********************* Electric heater lift
  SolarTherm.Models.Fluid.Pumps.LiftSimple liftHeater(redeclare package Medium = Medium, cont_m_flow = true, use_input = true, dh = dh_liftHX, CF = 0, eff = eff_lift) annotation(
    Placement(visible = true, transformation(origin = {44, -132}, extent = {{-26, -26}, {26, 26}}, rotation = 0)));
  //********************* Heat exchanger lift
...
//************************************ CapitalCost Calculation
  C_cap_CSP = C_field + C_site + C_receiver + C_storage + C_block + C_bop;
  C_cap_PV = C_PV;
  C_cap_SMR = C_SMR;
...
//************************************ PowerBlock connections
  ...
  connect(heatExchanger_ParticleGas.particle_port_out, tankCold.fluid_a) annotation(
    Line(points = {{248, -2}, {254, -2}, {254, -14}, {64, -14}, {64, -12}}, color = {255, 0, 0}, thickness = 1));
...
//************************************ End of The System Model ********************************************\\
  ...
  connect(tankCold.fluid_b, liftHeater.fluid_a) annotation(
    Line(points = {{44, -24}, {44, -24}, {44, -80}, {80, -80}, {80, -130}, {52, -130}, {52, -130}}, color = {0, 127, 255}));
  connect(liftHeater.fluid_b, simpleElectricalHeater.particle_port_in) annotation(
    Line(points = {{34, -130}, {26, -130}, {26, -122}, {18, -122}, {18, -122}}, color = {0, 127, 255}));
  connect(simpleElectricalHeater.particle_port_out, tankHot.fluid_a) annotation(
    Line(points = {{-42, -122}, {-58, -122}, {-58, -108}, {-22, -108}, {-22, -56}, {10, -56}, {10, 68}, {16, 68}, {16, 70}}, color = {0, 127, 255}));
  connect(electrolyser.W_dumped, simpleElectricalHeater.W_electric) annotation(
    Line(points = {{238, -62}, {256, -62}, {256, -96}, {-12, -96}, {-12, -114}, {-12, -114}}, color = {0, 0, 127}));
protected
  annotation(
