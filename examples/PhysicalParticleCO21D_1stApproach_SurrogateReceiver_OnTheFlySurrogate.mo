...
model PhysicalParticleCO21D_1stApproach_SurrogateReceiver_OnTheFlySurrogate
  ...
  parameter SI.HeatFlowRate Q_flow_defocus = if set_feedforward == true then
  ...
  parameter FI.Money C_lift_hx = if set_external_storage then 
                                      pri_lift * dh_liftHX * m_flow_blk 
                                  else
                                      0
  "Heat exchanger lift cost";
  /******************************************************************************************************
  ...
  //********************* PowerBlockControl
  SolarTherm.Models.Control.PowerBlockControl_particle controlHot(m_flow_on = m_flow_blk, L_on = hot_tnk_empty_ub, L_off = hot_tnk_empty_lb, L_df_on = hot_tnk_full_ub, L_df_off = hot_tnk_full_lb, logic.dispatch_optimiser = set_dispatch_optimiser, logic.set_scheduler = set_scheduler) annotation(
  ...
  when counter > 0 then
    time_simul = floor(time) "Rounding down the time";
      if set_dispatch_optimiser then
          ...
          for i in 1:integer(horizon/dt) loop
            ...
            if set_swaying_optical_eff == true then
                ...
                if slope_error_runtime[i] < slope_error_windy then
                    ...
                    eta_opt_horizon[i]  = (eta_opt_horizon_windy[i] + (eta_opt_horizon_calm[i]  - eta_opt_horizon_windy[i]) 
                                          / (slope_error_windy - slope_error) * (slope_error_runtime[i] - slope_error)) * eta_rcv_assumption;
                else
          ...
          optimalDispatch = SolarTherm.Utilities.LinProgFunc_particle(
              //DNI_file, 
              //price_file, 
              horizon, 
              dt, 
  ...
  der(E_losses_defocus) = 
        if heliostatsField.on_internal then 
            if heliostatsField.defocus_internal then 
                  abs(heliostatsField.Q_net - heliostatsField.Q_raw) 
            else 0 
        else 
            0;
