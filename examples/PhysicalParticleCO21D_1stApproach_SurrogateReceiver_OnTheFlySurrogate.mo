...
model PhysicalParticleCO21D_1stApproach_SurrogateReceiver_OnTheFlySurrogate
  ...
  parameter SI.Power P_net = 20e6 "Power block net rating at design point";
  ...
  //OnTheFlySurrogateParameters
  parameter Real eta_gross_base(fixed = false);
  parameter Real eta_Q_base(fixed = false);
  ...
  parameter Real tolerance_kriging = 1.5e-3;
  ...
  // Power block
  SolarTherm.Models.PowerBlocks.SurrogatesCO2PB_OTF powerBlock(P_net = P_net, T_in_ref_blk = T_hot_set, p_high = p_high, PR = PR, pinch_PHX = pinch_exchanger, dTemp_HTF_PHX = T_hot_set - T_cold_set, load_base = 1, eta_gross_base = eta_gross_base, eta_Q_base = eta_Q_base, T_amb_base = blk_T_amb_des, Q_HX_des = Q_flow_des, m_HTF_des = m_flow_blk, base_path = base_path, SolarTherm_path = SolarTherm_path, inputsize = inputsize_PB, outputsize = outputsize_PB, tolerance_kriging = tolerance_kriging, tolerance_ANN = tolerance_ANN, which_surrogate = which_surrogate, test_mode = false, eta_motor = eta_motor, f_fixed_load = f_fixed_load) annotation(
