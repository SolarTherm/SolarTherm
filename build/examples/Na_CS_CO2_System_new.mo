...
model Na_CS_CO2_System_new "High temperature Sodium-sCO2 system"
  ...
  //Pgross and defocus
  parameter SI.Power P_gross(fixed = if fixed_field then false else true) = 10e6 "Power block gross rating at design point";
  parameter SI.HeatFlowRate Q_flow_defocus = 370 / 294.118 * Q_flow_des "Solar field thermal power at defocused state";
  /*35e6*/
  // Control
  ...
  // Receiver
  SolarTherm.Models.CSP.CRS.Receivers.SodiumReceiver_new receiver(em = em_rec, redeclare package Medium = Medium1, H_rcv = H_receiver, D_rcv = D_receiver, N_pa = N_pa_rec, t_tb = t_tb_rec, D_tb = D_tb_rec, ab = ab_rec, T_in_0 = T_cold_set_Na, T_out_0 = T_hot_set_Na, DNI_SF = data.DNI, Q_flow_def = Q_flow_defocus, SM=SM) annotation(
  ...
  /*, d_o=Shell_and_Tube_HX.d_o, N_p=Shell_and_Tube_HX.N_p, N_sp=Shell_and_Tube_HX.N_p, layout=Shell_and_Tube_HX.layout, N_t=Shell_and_Tube_HX.N_t, L=Shell_and_Tube_HX.L*/
  //HX
  ...
  SolarTherm.Models.Fluid.HeatExchangers.buffer_tank SodiumBufferTank(replaceable package Medium = Medium1, T_start = T_cold_set_Na, D = 1, H = 1) annotation(
