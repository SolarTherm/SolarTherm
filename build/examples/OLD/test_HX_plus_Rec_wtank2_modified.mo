model test_HX_plus_Rec_wtank2_modified
  ...
  SolarTherm.Models.Sources.DataTable.DataTable data(lon = lon, lat = lat, t_zone = t_zone, year = year, file = wea_file) annotation(
    Placement(visible = true, transformation(extent = {{-126, -54}, {-96, -26}}, rotation = 0)));
  //DNI_input
  Modelica.Blocks.Sources.RealExpression DNI_input(y = data.DNI) annotation(
    Placement(visible = true, transformation(extent = {{-112, 60}, {-92, 80}}, rotation = 0)));
  //Tamb_input
  Modelica.Blocks.Sources.RealExpression Tamb_input(y = data.Tdry) annotation(
    Placement(visible = true, transformation(extent = {{62, 84}, {42, 104}}, rotation = 0)));
  //WindSpeed_input
  Modelica.Blocks.Sources.RealExpression Wspd_input(y = data.Wspd) annotation(
    Placement(visible = true, transformation(extent = {{-112, 20}, {-86, 40}}, rotation = 0)));
  //Sun
  SolarTherm.Models.Sources.SolarModel.Sun sun(lon = data.lon, lat = data.lat, t_zone = data.t_zone, year = data.year, redeclare function solarPosition = Models.Sources.SolarFunctions.PSA_Algorithm) annotation(
    Placement(visible = true, transformation(extent = {{-54, 60}, {-34, 80}}, rotation = 0)));
  // Solar field
  SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatsField heliostatsField(n_h = n_heliostat, lon = data.lon, lat = data.lat, ele_min(displayUnit = "deg") = ele_min, use_wind = use_wind, Wspd_max = Wspd_max, he_av = he_av_design, use_on = true, use_defocus = true, A_h = A_heliostat, nu_defocus = nu_defocus, nu_min = nu_min_sf, Q_design = Q_flow_defocus, nu_start = nu_start, redeclare model Optical = Models.CSP.CRS.HeliostatsField.Optical.Table(angles = angles, file = opt_file)) annotation(
    Placement(visible = true, transformation(extent = {{-60, 2}, {-28, 36}}, rotation = 0)));
  ...
  SolarTherm.Models.Control.ReceiverControl controlCold(T_ref = T_hot_set, m_flow_max = m_flow_rec_max, y_start = 17.1174, L_df_on = cold_tnk_defocus_lb, L_df_off = cold_tnk_defocus_ub, L_off = cold_tnk_crit_lb, L_on = cold_tnk_crit_ub, Ti = Ti, Kp = Kp) annotation(
    Placement(visible = true, transformation(extent = {{52, -10}, {38, 4}}, rotation = 0)));
  
  //Fake Tank Level Input
  Modelica.Blocks.Sources.RealExpression L_input(y = L_fake) annotation(
    Placement(visible = true, transformation(extent = {{142, -54}, {122, -34}}, rotation = 0)));
  //HX
  SolarTherm.Models.Fluid.HeatExchangers.HX_wi_noF_modified Shell_and_Tube_HX(replaceable package Medium1 = Medium1, replaceable package Medium2 = Medium2, Q_d_des = Q_flow_des * SM) annotation(
    Placement(visible = true, transformation(origin = {61, 47}, extent = {{-21, -21}, {21, 21}}, rotation = 0)));
  /*, Q_receiver=receiver.Q_rcv*/
  /*Q = receiver.Q_rcv*/
  SolarTherm.Models.Fluid.Sources.FluidSink MS_Sink(replaceable package Medium = Medium2) annotation(
    Placement(visible = true, transformation(origin = {142, 80}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Fluid.Sources.MassFlowSource_T MS_inlet(replaceable package Medium = Medium2, T(start = 700, nominal = 700), nPorts = 1, use_T_in = true, use_m_flow_in = true) annotation(
    Placement(visible = true, transformation(origin = {137, 25}, extent = {{7, -7}, {-7, 7}}, rotation = 0)));
...
equation
  connect(heliostatsField.on, Shell_and_Tube_HX.HF_on) annotation(
    Line(points = {{-44, 2}, {-44, -34}, {-90, -34}, {-90, 10}, {-128, 10}, {-128, 114}, {66, 114}, {66, 57}}, color = {255, 0, 255}));
  connect(loop_breaker.port_a, Shell_and_Tube_HX.port_a_out) annotation(
    Line(points = {{102, -24}, {110, -24}, {110, 46}, {74, 46}}, color = {0, 127, 255}));
  connect(Shell_and_Tube_HX.port_b_in, MS_inlet.ports[1]) annotation(
    Line(points = {{67, 41}, {67, 24.33}, {130.67, 24.33}}, color = {0, 127, 255}));
  connect(Shell_and_Tube_HX.port_b_out, MS_Sink.port_a) annotation(
    Line(points = {{55, 56}, {55, 80.45}, {134.91, 80.45}}, color = {0, 127, 255}));
  connect(temperature.fluid_b, Shell_and_Tube_HX.port_a_in) annotation(
    Line(points = {{24, 71}, {48, 71}, {48, 52}}, color = {0, 127, 255}));
  connect(heliostatsField.on, controlCold.sf_on) annotation(
    Line(points = {{-44, 2}, {-44, -36}, {64, -36}, {64, -8}, {52, -8}}, color = {255, 0, 255}));
  connect(heliostatsField.on, receiver.on) annotation(
    Line(points = {{-44, 2}, {-44, -36}, {-20, -36}, {-20, 6}, {-4, 6}}, color = {255, 0, 255}));
  connect(heliostatsField.heat, receiver.heat) annotation(
    Line(points = {{-28, 27.5}, {-17.68, 27.5}}, color = {191, 0, 0}));
  connect(sun.solar, heliostatsField.solar) annotation(
    Line(points = {{-44, 60}, {-44, 36}}, color = {255, 128, 0}));
  connect(controlCold.defocus, heliostatsField.defocus) annotation(
    Line(points = {{45, -10.98}, {45, -52.98}, {-60, -52.98}, {-60, 9}}, color = {255, 0, 255}));
  connect(Wspd_input.y, heliostatsField.Wspd) annotation(
    Line(points = {{-84.7, 30}, {-60, 30}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(L_input.y, controlCold.L_mea) annotation(
    Line(points = {{121, -44}, {71.5, -44}, {71.5, -3}, {53, -3}}, color = {0, 0, 127}));
  connect(pumpCold.fluid_a, loop_breaker.port_b) annotation(
    Line(points = {{38, -24.12}, {82, -24.12}, {82, -24.12}, {82, -24.12}}, color = {0, 127, 255}));
  connect(Tamb_input.y, receiver.Tamb) annotation(
    Line(points = {{41, 94}, {0, 94}, {0, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(temperature.T, controlCold.T_mea) annotation(
    Line(points = {{19, 65.9}, {17, 65.9}, {17, 15.9}, {63, 15.9}, {63, 1.9}, {51, 1.9}, {51, 1.9}}, color = {0, 0, 127}));
  connect(controlCold.m_flow, pumpCold.m_flow) annotation(
    Line(points = {{37.16, -3}, {32.16, -3}, {32.16, -19}}, color = {0, 0, 127}));
  connect(pumpCold.fluid_b, receiver.fluid_a) annotation(
    Line(points = {{26, -24}, {16, -24}, {16, 6}, {4, 6}, {4, 6}}, color = {0, 127, 255}));
  connect(receiver.fluid_b, temperature.fluid_a) annotation(
    Line(points = {{6.12, 30.64}, {6.12, 70.64}, {14.12, 70.64}}, color = {0, 127, 255}));
  connect(DNI_input.y, sun.dni) annotation(
    Line(points = {{-91, 70}, {-55, 70}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
...
//Solar field connections i.e. solar.heat port and control
  annotation(
    Diagram(coordinateSystem(extent = {{-140, -120}, {160, 140}}, initialScale = 0.1), graphics = {Text(origin = {28, 0}, lineColor = {217, 67, 180}, extent = {{-50, -40}, {-14, -40}}, textString = "on/off strategy", fontSize = 9), Text(origin = {28, 0}, extent = {{-52, 8}, {-4, -12}}, textString = "Receiver", fontSize = 4, fontName = "CMU Serif"), Text(origin = {36, 0}, extent = {{-110, 4}, {-62, -16}}, textString = "Heliostats Field", fontSize = 4, fontName = "CMU Serif", horizontalAlignment = TextAlignment.Left), Text(origin = {12, 10}, extent = {{-80, 86}, {-32, 66}}, textString = "Sun", fontSize = 4, fontName = "CMU Serif"), Text(origin = {28, 0}, extent = {{-6, 20}, {42, 0}}, textString = "Receiver Control", fontSize = 4, fontName = "CMU Serif"), Text(origin = {12, -24}, extent = {{-146, -26}, {-98, -46}}, textString = "Data Source", fontSize = 4, fontName = "CMU Serif")}),
...
end test_HX_plus_Rec_wtank2_modified;