model TestSCO2PB
  ...
  //DNI_input
  //Tamb_input
  Modelica.Blocks.Sources.RealExpression Tamb_input(y = from_degC(25)) annotation(
    Placement(transformation(extent = {{140, 70}, {120, 90}})));
  //WindSpeed_input
  //pressure_input
  Modelica.Blocks.Sources.RealExpression Pres_input(y = 1e5) annotation(
  ...
  // Solar field
  // Receiver
  SolarTherm.Models.CSP.CRS.Receivers.ChlorideSaltReceiver receiver(redeclare package Medium = Medium, em = em_rec, ab = ab_rec, H_rcv = H_receiver, D_rcv = D_receiver, const_alpha = true) annotation(
  ...
  // Receiver lift
  SolarTherm.Models.Fluid.Pumps.PumpSimple pumpCold(redeclare package Medium = Medium, k_loss = k_loss_hot) annotation(
  ...
  // ReceiverControl
  // Power block
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.recompPB powerBlock(redeclare package MedRec = Medium, P_gro = P_gross, T_HTF_in_des = T_in_ref_blk, T_amb_des = blk_T_amb_des, T_low = T_comp_in, external_parasities = external_parasities, nu_min = nu_min_blk) annotation(
  ...
  Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow Heat(Q_flow = Q_flow_des) annotation(
    Placement(visible = true, transformation(origin = {-78, 27}, extent = {{-20, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression m_flow_in(y = m_flow_blk) annotation(
    Placement(visible = true, transformation(origin = {122, 106}, extent = {{5, 10}, {-35, -10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression realExpression(y = m_flow_blk) annotation(
    Placement(visible = true, transformation(origin = {62, -52}, extent = {{5, 10}, {-35, -10}}, rotation = 0)));
initial equation
  if fixed_field then
...
//PowerBlock connections
  ...
  connect(Heat.port, receiver.heat) annotation(
    Line(points = {{-68, 28}, {-46, 28}, {-46, 28}, {-46, 28}}, color = {191, 0, 0}));
  connect(m_flow_in.y, pumpHot.m_flow) annotation(
    Line(points = {{86, 106}, {80, 106}, {80, 56}, {80, 56}}, color = {0, 0, 127}));
  connect(realExpression.y, pumpCold.m_flow) annotation(
    Line(points = {{26, -52}, {13, -52}, {13, -10}, {0, -10}, {0, -20}}, color = {0, 0, 127}));
  annotation(
