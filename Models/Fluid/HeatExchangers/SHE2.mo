...
model SHE2 "Simplified Heat Exchanger"
  ...
  parameter SI.SpecificEnthalpy hl_bo=Modelica.Media.Water.IF97_Utilities.hl_p(p_bo);
  parameter SI.SpecificEnthalpy hv_bo=Modelica.Media.Water.IF97_Utilities.hv_p(p_bo);
  MB.ThermodynamicState state_bo=MB.setState_phX(port_b_in.p,hv_bo,inStream(port_b_in.Xi_outflow));
  SI.Temperature T_bo=MB.temperature(state_bo);
