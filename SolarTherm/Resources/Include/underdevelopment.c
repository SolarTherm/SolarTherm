...
{
    double H_drop = 32;
    double T_HTF_in_des = 550 + 273.15;
    double T_HTF_out_des = 800 + 273.15;
    ...
    Session_Props* rcv_sess =  constructOnTheFlyReceiver(H_drop, T_HTF_in_des, T_HTF_out_des, base_path, SolarTherm_path);

    check_session(rcv_sess,5,1);

    FILE* F = fopen("./training_data/validation_data_dummy.csv","r");

    char line[limitSize];

    fgets(line,limitSize,F);
    ...
    while(!feof(F))
    {
        double Q_in,T_in,T_amb,Wspd,Wspd_dir,eta_rec;
        sscanf(
            line,"%lf,%lf,%lf,%lf,%lf,%lf",
            &Q_in,&T_in,&T_amb,&Wspd,&Wspd_dir,&eta_rec
            );
        
        //*************** The input values must be negative of the raw input since it use the "deviation" logic, which doesn't
        //present in the receiver model
        const double raw_inputs[5] = {
            -Q_in,-T_in,-T_amb,-Wspd,-Wspd_dir
        };

        double eta_rec_pred = predict_ANN(rcv_sess,raw_inputs,0);

        fprintf(stderr,"%lf,%lf\n",eta_rec_pred,eta_rec);
        fgets(line,limitSize,F);
    }
...
void* constructOnTheFlyReceiver(double H_drop, double T_HTF_in_des, double T_HTF_out_des, char* base_path, char* SolarTherm_path)
{
    Session_Props* sess;
    double* X_max = malloc(sizeof(double*)*5);
    double* X_min = malloc(sizeof(double*)*5);
    double* y_max = malloc(sizeof(double)*1);
    double* y_min = malloc(sizeof(double)*1);
    ...
    char* traindir_base = "/training_data/";
    char* config_base = "configReceiver";
    char* trainingdir = build_trainingdir_path(base_path,traindir_base,config_base,match_index);

    fprintf(stderr,"match index and status config = %d %d\n",match_index,status_config);

    int numdata = 3500;

    if(status_config==1) //**************** No configuration exists in the database
    {
        int stop = 0;

        fprintf(stderr,"Configuration doesn't exist. Generating training data!\n");
        
        while(stop==0)
        {
            simReceiver(numdata, H_drop, 
            T_HTF_in_des, T_HTF_out_des, trainingdir, SolarTherm_path, base_path, 
            status_config);

            status_config = 0;
            
            //*************Train ANN and validate it
            char* fndata = concat_training_dir(trainingdir,"/training_data.csv");
            trainingANNReceiver(fndata,trainingdir,0, SolarTherm_path); 

            char* fnscore = concat_training_dir(trainingdir,"/score.txt");
            FILE* h = fopen(fnscore,"r");
            if(h==NULL)
            {
                fprintf(stderr,"Score file doesn't exist! your path: %s\n",fnscore);
                exit(EXIT_FAILURE);
            }
            
            double MAE,R2;

            fgets(line,limitSize,h);
            while(!feof(h))
            {
                sscanf(line,"%lf,%lf",&MAE,&R2);
                fgets(line,limitSize,h); 
            }
            fclose(h);

            fprintf(stderr,"MAE: %lf,R2 : %lf\n",MAE,R2);

            char* ANN_path = concat_training_dir(trainingdir,"/surrogate_model_0");

            /*Load X max and X min*/
            char* fn_max = concat_training_dir(trainingdir,"/max.txt");
            char* fn_min = concat_training_dir(trainingdir,"/min.txt");

            FILE* f = fopen(fn_max,"r");
            FILE* g = fopen(fn_min,"r");

            if(f==NULL)
            {
                fprintf(stderr,"max file doesn't exist. Your path : %s\n",fn_max);
                exit(EXIT_FAILURE);
            }

            if(g==NULL)
            {
                fprintf(stderr,"min file doesn't exist. Your path : %s\n",fn_min);
                exit(EXIT_FAILURE);
            }            
            
            fgets(line,limitSize,f); //get the first line of fnmax, and store it in variable line
            sscanf(
                line,"%lf,%lf,%lf,%lf,%lf,%lf",
                &X_max[0],
                &X_max[1],
            ...
            fgets(line,limitSize,g); //get the first line of fnmax, and store it in variable line
            sscanf(
                line,"%lf,%lf,%lf,%lf,%lf,%lf",
                &X_min[0],
                &X_min[1],
            ...
            sess = load_session(ANN_path, X_max, X_min, y_max, y_min, 5, 1, 0, 0, 0);

            if(R2<0.95)
            {
                fprintf(stderr,"Validation failed with R_squared value: %lf! Increase number of data!\n",R2);
                numdata = 500;
            }
            else
            {
                fprintf(stderr,"Validation done with R_squared value: %lf!\n",R2);
                stop = 1;
            }
    ...
    {
        fprintf(stderr,"Configuration exists!\n");
        DIR* check_dir;
        ...
        if ((check_dir = opendir(ANN_path)) == NULL)
        {
            fprintf(stderr,"Configuration exists, but ANN model doesn't. Generating training data!\n");
            while(stop==0)
            {
                simReceiver(numdata, H_drop, 
                T_HTF_in_des, T_HTF_out_des, trainingdir, SolarTherm_path, base_path, 
        ...
        {
            //*************** Model is ready
            fprintf(stderr,"Model is ready! Load session\n");
            char* ANN_path = concat_training_dir(trainingdir,"/surrogate_model_0");
    ...
    sess->base_values[0] = 0;
    sess->base_values[1] = 0;
    sess->base_values[2] = 0;
    sess->base_values[3] = 0;
    sess->base_values[4] = 0;
