...
//Actually SM*10"
  ...
  replaceable package Filler_B = SolarTherm.Materials.PigIron_Constant "Middle tank filler";
  ...
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction Splitter_top(redeclare package Medium = Medium, T_min = T_min) annotation(
    Placement(visible = true, transformation(origin = {26, 61}, extent = {{-10, -9}, {10, 9}}, rotation = 0)));
  ...
  SolarTherm.Models.Control.PBS_Controller_3 Control(redeclare package HTF = Medium, T_recv_max = T_recv_max, T_PB_min = T_PB_min, T_target = T_max, m_flow_PB_des = m_flow_blk_des, Q_des_blk = Q_flow_ref_blk, T_recv_start = T_recv_start, T_PB_start = T_PB_start) annotation(
...
//  Line(points = {{-113, 48}, {-32, 48}, {-32, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  ...
  connect(Splitter_top.fluid_c, Tank.fluid_a) annotation(
    Line(points = {{26, 61}, {26, 49}}, color = {0, 127, 255}));
  ...
  connect(Control.T_bot_tank, Tank.T_bot_measured) annotation(
    Line(points = {{52, -20}, {38, -20}, {38, 28}, {34, 28}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_top_measured, Control.T_top_tank) annotation(
    Line(points = {{34, 44}, {46, 44}, {46, -16}, {52, -16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
protected
  annotation(
