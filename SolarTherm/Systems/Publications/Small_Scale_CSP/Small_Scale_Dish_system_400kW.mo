within SolarTherm.Systems.Publications.Small_Scale_CSP;

model Small_Scale_Dish_system_400kW
  ...
  //Replaceable Models
  //**********************
  replaceable model Cycle = SolarTherm.Models.PowerBlocks.Correlation.sCO2NREL_400kWe_574C_290C;
  ...
  parameter SI.Power P_name = 400.0e3 "Nameplate rating of power block";
  ...
  SolarTherm.Models.CSP.CRS.HeliostatsField.Dish heliostatsField(n_h = n_heliostat, lon = data.lon, lat = data.lat, ele_min(displayUnit = "deg") = ele_min, use_wind = use_wind, Wspd_max = Wspd_max, he_av = he_av_design, use_on = true, use_defocus = true, A_h = A_heliostat, nu_defocus = nu_defocus, nu_min = nu_min_sf, Q_design = Q_recv_inc_des, nu_start = nu_start, redeclare model Optical = Models.CSP.CRS.HeliostatsField.Optical.Table(angles = angles, file = opt_file), E_start = helio_E_start, W_track = helio_W_track) annotation(
    Placement(visible = true, transformation(extent = {{-84, 4}, {-52, 38}}, rotation = 0)));
  // Receiver
...
//Connections from data
  ...
  connect(Wspd_input.y, heliostatsField.Wspd) annotation(
    Line(points = {{-117, 54}, {-100, 54}, {-100, 32}, {-84, 32}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
...
//Solar field connections i.e. solar.heat port and control
  connect(sun.solar, heliostatsField.solar) annotation(
    Line(points = {{-72, 60}, {-72, 47}, {-68, 47}, {-68, 38}}, color = {255, 128, 0}));
  connect(heliostatsField.heat, receiver.heat) annotation(
    Line(points = {{-52, 29.5}, {-54.82, 29.5}, {-54.82, 27.4}, {-46, 27.4}}, color = {191, 0, 0}));
//PowerBlock connections
  ...
  connect(False.y, heliostatsField.defocus) annotation(
    Line(points = {{-96, 8}, {-92, 8}, {-92, 11}, {-84, 11}}, color = {255, 0, 255}));
  annotation(
...
end Small_Scale_Dish_system_400kW;
