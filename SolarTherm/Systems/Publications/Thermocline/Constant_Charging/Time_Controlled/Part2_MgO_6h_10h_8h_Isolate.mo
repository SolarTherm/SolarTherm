...
model Part2_MgO_6h_10h_8h_Isolate
  ...
  parameter Real[inputdim] X_max_charging = {-9.72353685e+02, 8.35348455e-01, 9.60050100e+05, 3.13143707e+02};
  parameter Real[inputdim] X_min_charging = {-2.41383150e+03, 2.31096910e-01, 9.45905377e+05, 2.53306364e+02};
  parameter Real y_max_charging = 1.02871441;
  parameter Real y_min_charging = 0.81689158;
  parameter Real[inputdim] X_max_discharging = {2.41571026e+03, 8.34176698e-01, 7.11378069e+05, 3.13143707e+02};
  parameter Real[inputdim] X_min_discharging = {9.73624104e+02, 2.30934116e-01, 7.00545084e+05, 2.53306364e+02};
  parameter Real y_max_discharging = 1.02792768;
  parameter Real y_min_discharging = 0.76482371;
  parameter String saved_model_dir_charging = "/home/philgun/solartherm-zeb/SolarTherm/Resources/Include/neural-network/trained-model/ThermoclineTank/Air_MgO/charging";
  parameter String saved_model_dir_discharging = "/home/philgun/solartherm-zeb/SolarTherm/Resources/Include/neural-network/trained-model/ThermoclineTank/Air_MgO/discharging";
  ...
  //Study this
  parameter Integer N_p = 1 "Number of filler CVs  in main tank";
  //Study this
  parameter SI.Energy E_max = 10 * 3600 * (P_name / eff_PB) "Storage capacity (J), t_discharge(s), 100MWe, 50% PB efficiency";
  //parameter SI.Energy E_max = E_max2 "Storage capacity (J), t_discharge(s), 100MWe, 50% PB efficiency";
  ...
  /*
        SolarTherm.Models.Storage.Thermocline.Thermocline_Spheres_SingleTank_LC_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar = ar, eta = eta, d_p = d_p, U_loss_tank = U_loss_tank, Correlation = Correlation) annotation(
          Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
        */
  SolarTherm.Models.Storage.Thermocline.Thermocline_Spheres_SingleTank_LC_Final_SurrogateModel thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar = ar, eta = eta, d_p = d_p, U_loss_tank = U_loss_tank, Correlation = 1, 
    X_max_charging = X_max_charging, 
    X_min_charging = X_min_charging, 
    X_max_discharging = X_max_discharging, 
    X_min_discharging = X_min_discharging, 
    y_max_charging = y_max_charging, 
    y_min_charging = y_min_charging, 
    y_max_discharging = y_max_discharging, 
    y_min_discharging = y_min_discharging, 
    saved_model_dir_charging = saved_model_dir_charging, 
    saved_model_dir_discharging = saved_model_dir_discharging, 
    inputdim = inputdim
  ) annotation(
    Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
  ...
  //************ Combitable 1D to read the mass flow rate and enthalpy data
  Modelica.Blocks.Sources.CombiTimeTable table(tableOnFile = true, tableName = "input", smoothness = Modelica.Blocks.Types.Smoothness.ContinuousDerivative, columns = 2:3, fileName = "/home/philgun/Documents/coolstuff/coolstuff/ML/script/script/effectiveness_based/data/pred_test_data/input.motab") annotation(
    Placement(transformation(extent = {{-90, 50}, {-70, 70}})));
  SI.MassFlowRate mdot_table;
  SI.SpecificEnthalpy h_in_table;
  SI.Temperature T_in_table;
  //Costs
...
//****************** Do the regression by calling Python function via external C-function
  mdot_table = table.y[1];
  h_in_table = table.y[2];
  T_in_table = Fluid_Package.T_h(h_in_table);
  if mdot_table > 0 then
//Discharging
    m_PB_signal = mdot_table;
    m_Recv_signal = 1e-5;
  else
//Charging
    m_PB_signal = 1e-5;
    m_Recv_signal = abs(mdot_table);
  end if;
  Recv_outlet.T_in = T_in_table;
  PB_outlet.T_in = T_in_table;
/*
...
//discharging
    thermocline_Tank.Tank_A.h_out = thermocline_Tank.Tank_A.h_in + epsilon_stg * (h_f_max - thermocline_Tank.Tank_A.h_in);
  elseif thermocline_Tank.Tank_A.m_flow < (-1e-3) then
//charging
    thermocline_Tank.Tank_A.h_out = thermocline_Tank.Tank_A.h_in + epsilon_stg * (h_f_min - thermocline_Tank.Tank_A.h_in);
  else
//standby
    epsilon_stg = 0;
  end if;
...
end Part2_MgO_6h_10h_8h_Isolate;