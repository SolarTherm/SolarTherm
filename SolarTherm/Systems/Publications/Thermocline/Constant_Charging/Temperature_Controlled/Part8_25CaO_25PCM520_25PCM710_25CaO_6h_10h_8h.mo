...
model Part8_25CaO_25PCM520_25PCM710_25CaO_6h_10h_8h
  ...
  package Filler_Package_A = SolarTherm.Materials.CaO_Constant;
  package Filler_Package_B = SolarTherm.Materials.PCM_520;
  package Filler_Package_C = SolarTherm.Materials.PCM_710;
  package Filler_Package_D = SolarTherm.Materials.CaO_Constant;
  //Design Parameters
  //Fixed
  ...
  parameter Integer N_f_B = integer(frac_2*100);//90 "Number of fluid CVs in main tank";
  parameter Integer N_p_B = 10 "Number of filler CVs  in main tank";
  parameter Integer N_f_C = integer(frac_3*100);//5 "Number of fluid CVs in bottomer";
  parameter Integer N_p_C = 10 "Number of filler CVs  in bottomer";
  parameter Integer N_f_D = 100 - N_f_A - N_f_B - N_f_C ;//5 "Number of fluid CVs in bottomer";
  parameter Integer N_p_D = 10 "Number of filler CVs  in bottomer";
  ...
  parameter Real frac_1 = 0.25 "fraction of energy storage capacity in topper";
  //Derived
  parameter Real frac_2 = 0.25 "fraction of energy storage capacity in bottomer";
  parameter Real frac_3 = 0.25 "fraction of energy storage capacity in bottomer";
  ...
  SolarTherm.Models.Storage.Thermocline.Cascaded.Thermocline_Group_4 thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package_A, redeclare package Filler_Package_B = Filler_Package_B, redeclare package Filler_Package_C = Filler_Package_C, redeclare package Filler_Package_D = Filler_Package_D, N_f_A = N_f_A, N_p_A = N_p_A, N_f_B = N_f_B, N_p_B = N_p_B, N_f_C = N_f_C, N_p_C = N_p_C, N_f_D = N_f_D, N_p_D = N_p_D, frac_1=frac_1, frac_2=frac_2, frac_3=frac_3, T_max = T_max, T_min = T_min, E_max = E_max, ar = ar, eta = eta, d_p = d_p,U_loss_tank=U_loss_tank,Correlation=Correlation) annotation(
...
//efficiency
  if time > t_cycle*5.0 and time < t_cycle*6.0 then
    ...
    der(E_lost) = thermocline_Tank.Tank_A.Q_loss_total + thermocline_Tank.Tank_B.Q_loss_total + thermocline_Tank.Tank_C.Q_loss_total + thermocline_Tank.Tank_D.Q_loss_total;
    der(E_pump) = thermocline_Tank.Tank_A.W_loss_pump + thermocline_Tank.Tank_B.W_loss_pump + thermocline_Tank.Tank_C.W_loss_pump + thermocline_Tank.Tank_D.W_loss_pump;
  else
    der(numer) = 0.0;
    der(E_charged)=0.0;
    der(E_discharged) = 0.0;
...
end Part8_25CaO_25PCM520_25PCM710_25CaO_6h_10h_8h;