...
package CFD_Comparison_Study
  ...
  model Part1_MgO_5pctporosity_Spheres
    ...
    parameter SI.Length d_p = 0.10 "Filler diameter is equal to the side-length of a brick";
    //Optimise
...
//subtract pumping power
  ...
  connect(thermocline_Splitter1.fluid_c, thermocline_Tank.fluid_a) annotation(
    ...
    annotation(
      experiment(StopTime = 180000, StartTime = 0, Tolerance = 1e-3, Interval = 60));
  end Part1_MgO_5pctporosity_Spheres;
  
  model Part2_MgO_5pctporosity_HCylinders
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
    import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
