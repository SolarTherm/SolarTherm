...
package CFD_Comparison_Study
  model Al2O3_5pct_Spheres
  ...
  import SI = Modelica.SIunits;
    ...
    package Filler_Package = SolarTherm.Materials.Al2O3_Constant;//MgO_Constant;  //Can investigate different filler
    //Design Parameters
    //Fixed
  ...
  equation
    ...
    if thermocline_Tank.Tank_A.m_flow < -1.0e-3 then //charging
      T_outlet_degC = thermocline_Tank.Tank_A.T_bot - 273.15;
    elseif thermocline_Tank.Tank_A.m_flow > 1.0e-3 then
      T_outlet_degC = thermocline_Tank.Tank_A.T_top - 273.15;
    else
      T_outlet_degC = 0.0;
    end if;
  /*
  ...
  */
  //efficiency
    if time > t_cycle*1.0 and time < t_cycle*2.0 then
      ...
      if time < t_cycle*1.0+t_charge then //charging
    ...
    if time > t_cycle*1.0 + 100.0 then
      ...
      effectiveness_storage = (numer)/denom; //subtract pumping power
    ...
    annotation(
      experiment(StopTime = 180000, StartTime = 0, Tolerance = 1e-3, Interval = 300));
  
  end Al2O3_5pct_Spheres;
  
  model CaO_5pct_Spheres
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
  ...
  */
  ...
  end CaO_5pct_Spheres;
  
  model MgO_5pct_Spheres
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
    ...
    package Medium = SolarTherm.Media.Sodium.Sodium_pT; //Do not change
    package Fluid_Package = SolarTherm.Materials.Sodium; //Do not change
    package Filler_Package = SolarTherm.Materials.MgO_Constant;//MgO_Constant;  //Can investigate different filler
    //Design Parameters
    //Fixed
    parameter Integer Correlation = 3 "Conservative";
    parameter SI.Temperature T_min = 520 + 273.15 "Minimum temperature";
  ...
  */
  ...
  end MgO_5pct_Spheres;
  
  model PigIron_5pct_Spheres
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
  ...
  */
  ...
  end PigIron_5pct_Spheres;
  
  model PigIron_26pct_Spheres
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
  ...
  */
  ...
  end PigIron_26pct_Spheres;
  
  model Al2O3_5pct_Spheres_Alternate
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
    ...
    parameter SI.SpecificEnthalpy h_f_min = Fluid_Package.h_Tf(T_min, 0.0);
    parameter SI.SpecificEnthalpy h_f_max = Fluid_Package.h_Tf(T_max, 1.0);
    parameter SI.MassFlowRate m_charge = 1064.0;//E_max / (t_charge * (h_f_max - h_f_min));
    parameter SI.MassFlowRate m_discharge = 638.4;//E_max / (t_discharge * (h_f_max - h_f_min));
    ...
    SolarTherm.Models.Storage.Thermocline.Thermocline_Tank thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f= N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max,Tank_A.H_tank=39.07,Tank_A.D_tank=19.53, ar = ar, eta = eta, d_p = d_p,U_loss_tank=U_loss_tank, Correlation=Correlation) annotation(
      Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
  ...
  */
  ...
  end Al2O3_5pct_Spheres_Alternate;
  
  model Al2O3_5pct_Spheres_Alternate_Surface
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
    ...
    parameter Integer N_f = 40 "Number of fluid CVs in main tank";
    ...
    parameter SI.Time t_charge = 6.0 * 3600.0 "Charging period";
    parameter SI.Time t_standby = (24.0 * 3600.0) - t_charge - t_discharge "Standby period between discharge and charge";
    parameter SI.Length d_p = 0.01 "Filler diameter";
    parameter Real f_surface = 1.0;//2.0/((4*CN.pi/3)^(1/3));
    //Optimise
    parameter SI.CoefficientOfHeatTransfer U_loss_tank = 0.0 "W/m2K";
    parameter SI.Power P_name = 100.0e6 * (t_charge/t_discharge) "Nameplate power block";
    parameter SI.Time t_discharge = 10.0 * 3600.0 "Discharging period";
    ...
    SolarTherm.Models.Storage.Thermocline.Thermocline_Tank thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f= N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max,Tank_A.H_tank=38.9586,Tank_A.D_tank=19.52096, ar = ar, eta = eta, d_p = d_p,U_loss_tank=U_loss_tank,Tank_A.f_surface=f_surface, Correlation=Correlation) annotation(
      Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
  ...
  */
  ...
  end Al2O3_5pct_Spheres_Alternate_Surface;
  
  model Al2O3_5pct_Spheres_Alternate_Surface_33m
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
    ...
    parameter Integer N_f = 80 "Number of fluid CVs in main tank";
    parameter Integer N_p = 5 "Number of filler CVs  in main tank";
    ...
    SolarTherm.Models.Storage.Thermocline.Thermocline_Tank thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f= N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max,Tank_A.H_tank=32.65,Tank_A.D_tank=16.325, ar = ar, eta = eta, d_p = d_p,U_loss_tank=U_loss_tank,Tank_A.f_surface=f_surface, Correlation = Correlation) annotation(
      Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
  ...
  */
  ...
  end Al2O3_5pct_Spheres_Alternate_Surface_33m;
  
  model Al2O3_5pct_Spheres_Alternate_Surface_33m_2
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
  ...
  */
  ...
  end Al2O3_5pct_Spheres_Alternate_Surface_33m_2;
  
  model Al2O3_5pct_Cylinders_Alternate_Surface_33m
    //The comparison study for Medhi/Felipe aims to investigate how similar or different the results are when a sphere-based model with 5% porosity is used to model a brick filler system. Due to computational cost of CFD, only 2 days will be simulated, with the last day being used to calculate efficiency & effectiveness
  import SI = Modelica.SIunits;
    import CN = Modelica.Constants;
    import CV = Modelica.SIunits.Conversions;
    ...
    //Design Parameters
    //Fixed
    parameter Integer Correlation = 8 "Conservative";
    parameter SI.Temperature T_min = 520 + 273.15 "Minimum temperature";
    parameter SI.Temperature T_max = 740 + 273.15 "Maximum temperature";
    ...
    parameter SI.Time t_charge = 6.0 * 3600.0 "Charging period";
    parameter SI.Time t_standby = (24.0 * 3600.0) - t_charge - t_discharge "Standby period between discharge and charge";
    parameter SI.Length d_p = 0.15244 "Filler diameter";
    parameter Real f_surface = 10.0;//2.0/((4*CN.pi/3)^(1/3));
    //Optimise
    parameter SI.CoefficientOfHeatTransfer U_loss_tank = 0.0 "W/m2K";
    parameter SI.Power P_name = 100.0e6 * (t_charge/t_discharge) "Nameplate power block";
    parameter SI.Time t_discharge = 10.0 * 3600.0 "Discharging period";
    ...
    SolarTherm.Models.Storage.Thermocline.Thermocline_Tank_Cylinders thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f= N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max,Tank_A.H_tank=32.65,Tank_A.D_tank=16.325, ar = ar, eta = eta, d_p = d_p,U_loss_tank=U_loss_tank,Tank_A.f_surface=f_surface, Correlation = Correlation) annotation(
      Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
  ...
  */
  ...
  end Al2O3_5pct_Cylinders_Alternate_Surface_33m;
