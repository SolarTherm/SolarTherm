...
model Part3_Preserve_H_PartialFlow_Modulated
  ...
  //Modulated temperature threshold
  parameter SI.Temperature T_low_1 = 530 + 273.15 "Low temperature to trigger partial flow during discharging";
  parameter SI.Temperature T_high_1 = 700 + 273.15 "High temperature to trigger partial flow during charging";
  ...
  //PartialFlow
  parameter Real f_divert = 0.40 "Diverted fraction for partial flow";
  ...
  //COntrol
  SolarTherm.Models.Storage.Thermocline.Parallel.Thermocline_Spheres_Parallel_A3_v3_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package, redeclare package Filler_Package_B = Filler_Package, redeclare package Filler_Package_C = Filler_Package, frac_1 = 1.0/3.0, N_f_A = N_f, N_p_A = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar_A = ar, eta_A = eta, d_p_A = d_p, U_loss_tank_A = U_loss_tank, T_bot_high = T_Recv_max - 1, T_top_low = T_PB_min + 1, Correlation = Correlation, f_divert = f_divert, T_low_1 = T_low_1, T_high_1 = T_high_1) annotation(
...
equation
  ...
  if thermocline_Tank.fluid_a.m_flow > 1e-3 then //there is mass flow into the top (charge)
    T_outlet_degC = thermocline_Tank.fluid_bot.T - 273.15;
  elseif thermocline_Tank.fluid_a.m_flow < -1e-3 then //mass flow out of the top (discharge)
...
end Part3_Preserve_H_PartialFlow_Modulated;