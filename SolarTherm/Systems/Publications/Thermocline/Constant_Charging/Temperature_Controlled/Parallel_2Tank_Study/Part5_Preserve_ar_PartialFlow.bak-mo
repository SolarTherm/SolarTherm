within SolarTherm.Systems.Publications.Thermocline.Constant_Charging.Temperature_Controlled.Parallel_2Tank_Study;

model Part5_Preserve_ar_PartialFlow
  ...
  //COntrol
  SolarTherm.Models.Storage.Thermocline.Parallel.Thermocline_Spheres_Parallel_A2_v2_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package, redeclare package Filler_Package_B = Filler_Package, frac_1 = 1.0 / 2.0, N_f_A = N_f, N_p_A = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar_A = ar, eta_A = eta, d_p_A = d_p, U_loss_tank_A = U_loss_tank, T_bot_high = T_Recv_max - 1, T_top_low = T_PB_min + 1, Correlation = Correlation,f_divert = f_divert,T_high_1 = T_high_1,T_low_1=T_low_1) annotation(
  ...
  //v2 Control Partial Flow
  parameter Real f_divert = 0.25;
  parameter SI.Temperature T_high_1 = 710.0 + 273.15 "Thereshold when top temperature falls below it, causes partial flow";
  parameter SI.Temperature T_low_1 = 520.0 + 273.15 "Thereshold when bottom temperature falls below it, causes partial flow";
algorithm
...
equation
  ...
  T_bot_degC = thermocline_Tank.T_bot_measured - 273.15;
    ...
    if thermocline_Tank.fluid_b.m_flow > 1e-6 then //discharging
      T_outlet_degC = thermocline_Tank.fluid_top.T - 273.15;
    elseif thermocline_Tank.fluid_b.m_flow < -1e-6 then //charging
...
end Part5_Preserve_ar_PartialFlow;