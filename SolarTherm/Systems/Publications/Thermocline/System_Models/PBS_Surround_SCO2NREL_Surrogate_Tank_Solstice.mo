...
model PBS_Surround_SCO2NREL_Surrogate_Tank_Solstice
...
//Actually SM*10"
  ...
  import metadata = SolarTherm.Utilities.Metadata_Optics_ParticleBranch;
  ...
  parameter Real SM = 2.5 "Real solar multiple";
  parameter SI.Length H_recv = 20;
  parameter SI.Length D_recv = 20;
  parameter SI.Length H_tower = 200 "Height of the tower";
  ...
  parameter SI.Area A_heliostat = metadata_list[2] "Area of one heliostat";
  parameter SI.Length H_heliostat = A_heliostat ^ 0.5 "Height of helio";
  parameter SI.Length W_heliostat = A_heliostat ^ 0.5 "Width of helio";
  //parameter SI.Area A_land = land_mult * A_field "Land area occupied by the plant";
  parameter SI.Area A_land = metadata_list[9] "Land area occupied by the plant";
  ...
  parameter SI.HeatFlowRate Q_in_rcv = (P_gross_des / eff_blk_des - Q_flow_rec_loss_des) * SM "Receiver Thermal power output at design";
  ...
  parameter SI.MassFlowRate m_flow_recv_des = Q_in_rcv / (h_out_ref_recv - h_in_ref_recv) "Design mass flow rate into recv";
  // Power block
  ...
  parameter FI.Money C_tower = 3e6 * Modelica.Math.exp(0.0113 * (H_tower + 0.5 * A_heliostat ^ 0.5 - H_recv * 0.5)) "Tower cost based on SAM";
  //3117043.67 * exp(0.0113 * H_tower) "Tower cost";
  ...
  /*SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatsField heliostatsField(redeclare model Optical = Models.CSP.CRS.HeliostatsField.Optical.Table_Full(angles = angles, file = opt_file), A_h = A_heliostat, Q_design = Q_flow_defocus, Wspd_max = Wspd_max, ele_min(displayUnit = "deg") = ele_min, he_av = he_av_design, lat = data.lat, lon = data.lon, n_h = n_heliostat, nu_defocus = nu_defocus, nu_min = nu_min_sf, nu_start = nu_start, use_defocus = false, use_on = true, use_wind = true) annotation(
          ...
          */
  // Solar heliostat field
  SolarTherm.Models.CSP.CRS.HeliostatsField.HeliostatsFieldSolstice_1stApproach heliostatsField(lon = lon, lat = lat, ele_min(displayUnit = "deg") = ele_min, use_wind = false, t_start = 3600, Wspd_max = Wspd_max, he_av = he_av_design, use_on = true, use_defocus = true, A_h = A_heliostat, nu_defocus = 1, nu_min = nu_min_sf, Q_design = Q_flow_defocus, nu_start = nu_start, Q_in_rcv = Q_in_rcv, H_rcv = H_recv, W_rcv = D_recv, tilt_rcv = 0, field_type = field_type, W_helio = W_heliostat, H_helio = H_heliostat, H_tower = H_tower, R_tower = D_recv / 2, R1 = R1, fb = fb, helio_refl = helio_refl, slope_error = slope_error, slope_error_windy = slope_error_windy, n_row_oelt = n_row_oelt, n_col_oelt = n_col_oelt, n_procs = n_procs, psave = casefolder, wea_file = wea_file, get_optics_breakdown = false, set_swaying_optical_eff = false) annotation(
    Placement(transformation(extent = {{-88, 2}, {-56, 36}})));
  ...
  Modelica.Blocks.Sources.BooleanExpression always_off(y = false) annotation(
...
//Line(points = {{34, 44}, {46, 44}, {46, -16}, {52, -16}, {52, -16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  ...
  connect(always_on.y, heliostatsField.on_hopper) annotation(
    Line(points = {{-116, -4}, {-110, -4}, {-110, 20}, {-88, 20}, {-88, 20}}, color = {255, 0, 255}));
  connect(Control.defocus, heliostatsField.defocus) annotation(
    Line(points = {{68, -20}, {84, -20}, {84, -72}, {-102, -72}, {-102, 10}, {-88, 10}, {-88, 8}}, color = {255, 0, 255}));
  connect(always_off.y, receiver.defocus) annotation(
    Line(points = {{-64, -34}, {-46, -34}, {-46, 20}, {-34, 20}, {-34, 20}}, color = {255, 0, 255}));
protected
  annotation(
...
end PBS_Surround_SCO2NREL_Surrogate_Tank_Solstice;