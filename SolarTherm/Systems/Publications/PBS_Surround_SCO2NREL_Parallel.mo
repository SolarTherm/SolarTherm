...
//Actually SM*10"
  ...
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction_LoopBreaker Splitter_bot(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {36, 6}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction Splitter_top(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(extent = {{18, 56}, {50, 90}}, rotation = 0)));
  ...
  SolarTherm.Models.Fluid.Pumps.PumpSimple_EqualPressure pumpHot(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {63, 87}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
  SolarTherm.Models.Control.PBS_Controller Control(redeclare package HTF = Medium, T_recv_max = T_recv_max, T_PB_min = T_PB_min, T_target = T_max, m_flow_PB_des = m_flow_blk_des, Q_des_blk = Q_flow_ref_blk, T_recv_start = T_recv_start, T_PB_start = T_PB_start) annotation(
    Placement(visible = true, transformation(origin = {72, -7}, extent = {{12, -11}, {-12, 11}}, rotation = 0)));
...
//Connections from data
  ...
  connect(Wspd_input.y, heliostatsField.Wspd) annotation(
    Line(points = {{-111, 48}, {-24.5, 48}, {-24.5, 26}, {-76, 26}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
// Fluid connections
...
//Solar field connections i.e. solar.heat port and control
  connect(sun.solar, heliostatsField.solar) annotation(
    Line(points = {{-86, 60}, {-86, 51}, {-66, 51}, {-66, 42}}, color = {255, 128, 0}));
//PowerBlock connections
  P_elec = powerBlock.W_net;
  E_elec = powerBlock.E_net;
  R_spot = market.profit;
  connect(heliostatsField.heat, receiver.heat) annotation(
    Line(points = {{-56, 37}, {-56, 35}, {-42, 35}}, color = {191, 0, 0}));
  connect(parasities_input.y, powerBlock.parasities) annotation(
    Line(points = {{-2.5, 90}, {-2.5, 65}, {107, 65}, {107, 20}}, color = {0, 0, 127}));
  connect(powerBlock.T_amb, Tamb_input.y) annotation(
    Line(points = {{117, 29}, {111.5, 29}, {111.5, 80}, {120, 80}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(powerBlock.W_net, market.W_net) annotation(
    Line(points = {{107, 20}, {121.5, 20}, {121.5, 26}, {132, 26}}, color = {0, 0, 127}));
...
//  Line(points = {{-113, 48}, {-32, 48}, {-32, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tamb_input.y, Tank.T_amb) annotation(
    Line(points = {{120, 80}, {16, 80}, {16, 40}, {34, 40}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.p_amb, Pres_input.y) annotation(
    Line(points = {{34, 40}, {72, 40}, {72, 96}, {118, 96}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Splitter_bot.fluid_b, pumpCold.fluid_a) annotation(
    Line(points = {{44, -2}, {10, -2}, {10, 0}, {-14, 0}}, color = {0, 127, 255}));
  ...
  connect(receiver.fluid_b, Splitter_top.fluid_a) annotation(
    Line(points = {{-29, 37}, {-18, 37}, {-18, 73}, {34, 73}}, color = {0, 127, 255}));
  connect(pumpHot.fluid_b, powerBlock.fluid_a) annotation(
    Line(points = {{56, 87}, {80.5, 87}, {80.5, 15}, {114, 15}}, color = {0, 127, 255}));
  connect(powerBlock.fluid_b, Splitter_bot.fluid_a) annotation(
    Line(points = {{117, 27}, {57.5, 27}, {57.5, 6}, {36, 6}}, color = {0, 127, 255}));
  connect(Tank.fluid_b, Splitter_bot.fluid_c) annotation(
    Line(points = {{34, 24}, {25, 24}, {25, 6}, {36, 6}}, color = {0, 127, 255}));
  connect(Splitter_top.fluid_b, pumpHot.fluid_a) annotation(
    Line(points = {{21, 87}, {21, 63.5}, {63, 63.5}, {63, 87}}, color = {0, 127, 255}));
  connect(Splitter_top.fluid_c, Tank.fluid_a) annotation(
    Line(points = {{34, 73}, {34, 56}}, color = {0, 127, 255}));
  connect(receiver.Q_rcv_raw, Control.Q_rcv_raw) annotation(
    Line(points = {{-29.5, 29.5}, {10, 29.5}, {10, -11}, {59, -11}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_recv, pumpCold.m_flow) annotation(
    Line(points = {{59, -6}, {-6.5, -6}, {-6.5, 5}, {-14, 5}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_PB, pumpHot.m_flow) annotation(
    Line(points = {{85, -11}, {85, 39}, {63, 39}, {63, 93}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.defocus, receiver.defocus) annotation(
    Line(points = {{85, -17}, {-38, -17}, {-38, 30}, {-35, 30}}, color = {255, 0, 255}, pattern = LinePattern.Dash));
  connect(Tank.h_bot_outlet, Control.h_tank_outlet) annotation(
    Line(points = {{34, 40}, {34, 27}, {65, 27}, {65, 5}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(powerBlock.h_out_signal, Control.h_PB_outlet) annotation(
    Line(points = {{107, 20}, {107, 9}, {85, 9}, {85, 0}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.Q_defocus, receiver.Q_defocus) annotation(
    Line(points = {{59, -1}, {48.5, -1}, {48.5, 27}, {-30, 27}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_95_measured, Control.T_top_tank) annotation(
    Line(points = {{34, 40}, {34, 5.5}, {72, 5.5}, {72, -7}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_05_measured, Control.T_bot_tank) annotation(
    Line(points = {{34, 40}, {85, 40}, {85, -6}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  annotation(
