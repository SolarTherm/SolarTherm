...
model Annular_1T_AdaptiveCycle "Concrete storage perforated with steel pipes. Thermal oil (DowthermA) is used to charge and discharge the TES."
  import SI = Modelica.SIunits;
  import CN = Modelica.Constants;
  import CV = Modelica.SIunits.Conversions;
  extends Modelica.Icons.Example;
  package Medium = SolarTherm.Media.DowthermA.DowthermA_ph;
  package Medium_Utilities = SolarTherm.Media.DowthermA.DowthermA_ph_utilities;
  package Fluid_Package = SolarTherm.Materials.Dowtherm_Table;
  package Filler_Package = SolarTherm.Materials.Concrete_Laing_2006_Constant;
  ...
  parameter SI.Length d_pipe = 0.02 "Hole diameter in the filler (m)";
  parameter SI.Length d_solid = 0.06 "Equivalent solid diameter per repeating unit (m)";
  parameter SI.Length L_pipe = 25.0 "Length of TES in the charging/discharging axis (m)";
  parameter SI.Length E_roughness = 46.0e-6 "Surface roughness of fluid channel (m)";
  parameter SI.ThermalInsulance R_foul = 0.0009 "Fouling factor (m2K/W)";

  //Temperature Controls
  parameter SI.Temperature T_max = 400.0 + 273.15 "Maximum temperature (K)";
  parameter SI.Temperature T_process_des = 335.0 + 273.15 "Design process inlet temperature (K)";
  parameter SI.Temperature T_high_set = 335.0 + 273.15 "TES hot blend temperature temperature (K)";
  parameter SI.Temperature T_process_min = 335.0 + 273.15 "Minimum tolerated outlet temperature to process (K)";
  parameter SI.Temperature T_heater_max = 200.0 + 273.15 "Maximum tolerated outlet temperature to heater (K)";
  parameter SI.Temperature T_low_set = 200.0 + 273.15 "TES cold blend temperature (K)";
  parameter SI.Temperature T_heater_des = 100.0 + 273.15 "Design receiver inlet temperature (K)";
  parameter SI.Temperature T_min = 100.0 + 273.15 "Minimum temperature (K)";
  parameter SI.Temperature T_amb_des = 25.0 + 273.15 "Design ambient temperature (K)";
  parameter Integer Correlation = 2; //2: SwameeJain + KawaseDe

  //Tank Geometry
  parameter SI.MassFlowRate m_flow_process_des = 1.50;
  ...
  Real liq_frac_A = sum(TES.Tank_A.f_p)/N_f "Overall PCM liquid fraction of Tank B assuming equal masses for each vertical fluid element";
...
//Connectors
  ...
  Documentation(info = "<html><head></head><body><div><div>Hot Blast Stove TES:</div><div><br></div><div>Fluid = DowthermA (Liquid)</div><div>Filler = Laing2006_Concrete</div><div>TES Pipe Length = 25.0 m</div><div>Number of Pipe Units = 1041 (about a 33 x 33 grid)</div>
<div><br></div><div>Fluid pipe diameter = 2.0 cm</div><div>Solid annulus diameter = 6.0 cm</div><div><br></div><div>Heater Multiple = 2.0 (Charging and discharging occurs at the same rate)</div><div><br></div><div>Design discharging conditions (to the industrial process or heat exchanger):</div><div>1.5 kg/s of hot DowthermA at 335 degC, with a return cold stream at 100 degC. TES can discharge hot Dowtherm between 400 and 335 degC; when the top fluid outlet is &gt; 335 degC, the mass flow rate is &lt; 1.5 kg to maintain a constant heat-rate.</div><div><br></div><div><u><b>Operating Strategy:</b></u></div><div><br></div><div>TES is charged with hot DowthermA at 400degC from the top (z = H_tank), charging is halted when the bottom DowthermA outlet temperature rises to 200 degC.</div><div><br></div><div>TES then switches to discharging mode where cold Dowtherm at 100 degC is pumped into the bottom (z = 0) and hot DowthermA is extracted from the top. Discharging is halted when the top DowthermA outlet temperature drops to 335 degC.</div><div><br></div><div>The charging/discharging mass flow rates are adjusted to keep the charging/discharging heat-rates constant.</div><div><br></div><div>The simulation runs 10 charging-discharging cycles to establish cyclical operating behaviour and the tenth cycle is used to calculate performance metrics:</div><div><br></div><div>Energy efficiency \"eff_energy\" is the round-trip efficiency of the TES component. This is the energy discharged / energy charged into the TES over the 10th cycle.</div><div><br></div><div>Energy utilisation \"util_energy\" is the fraction of ideal storage capacity of the TES component that is actually used = energy discharged / ideal energy capacity. The ideal storage capacity is calculated as the combined enthalpy difference of filler and fluid phases between T_min and T_max. This is a measure of how well the storage material is utilised.</div><div><br></div><div><b><u>Outputs:</u></b></div><div><u>Settings:&nbsp;</u></div><div><br></div><div>Interval (step-size) = 60s</div><div>Method = dassl</div><div>Tolerance = 1.0e-4</div><div>Non-linear solver = homotopy</div><div><br></div><div><u>Calculated Variables (at end of simulation) at different mesh refinements:</u></div><div><u>Note: Total simulation time is based on an Intel Core i7-7700 CPU @ 3.60 GHz machine</u></div><div><br></div><div>N_f = 64, N_p = 10, util_energy = 0.169792, Total sim. time = 21.7451 s</div><div>N_f = 80, N_p = 10, util_energy = 0.174114, Total sim. time = 33.0588 s</div><div>N_f = 100, N_p = 10, util_energy = 0.177602, Total sim. time = 53.098 s</div></div><div><br></div><div><u>Output Variables to check:</u></div><div>TES.z_f = Z-axis positions of each fluid/filler element</div><div>TES.T_f = Temperature profile of each fluid element along the z-axis</div><div>TES.T_p = Temperature profile of each filler element along the z-axis</div><div>TES.Tank_A.Bi = Biot number of each fluid/filler pair</div><div>TES.Tank_A.Re = Reynolds number of the fluid in each vertical element</div></body></html>"));
end Annular_1T_AdaptiveCycle;