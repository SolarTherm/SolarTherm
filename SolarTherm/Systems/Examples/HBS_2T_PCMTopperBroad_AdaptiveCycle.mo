...
model HBS_2T_PCMTopperBroad_AdaptiveCycle "This component analysis allows a variable cycle length where charging and discharging follow immediately after each other assuming the system temperate constraints have not been violated. Tank heights are fixed to 30m for all cases."
  ...
  package Filler_Package_A = SolarTherm.Materials.Examples.Gan_Checker3_Constant;
  //package Filler_Package_B = SolarTherm.Materials.FeSi2;
  package Filler_Package_B = SolarTherm.Materials.Examples.PCM1050_Broad40K;
  ...
  parameter Integer N_f = 160 "Total number of vertical mesh elements";
  parameter Integer N_f_A = SolarTherm.Utilities.Round(frac_1*N_f);
  parameter Integer N_f_B = SolarTherm.Utilities.Round((1.0-frac_1)*N_f);
  parameter Integer N_tanks = 2 "Number of tanks in the storage array, needed to adjust aspect ratio such that all tanks are 35m high";
  parameter Real frac_1 = 0.90;
  ...
  parameter Real epsilon = 0.5151 "Packed-bed porosity";
  //parameter SI.Length H_tank = 35.0 "Fixed tank height (m)";
  parameter SI.EnergyDensity E_v_A = epsilon*rho_f_avg*(h_f_max-h_f_min)+(1.0-epsilon)*rho_p_A*(h_p_max_A-h_p_min_A) "Volumetric energy density (J/m3)";
  parameter SI.EnergyDensity E_v_B = epsilon*rho_f_avg*(h_f_max-h_f_min)+(1.0-epsilon)*rho_p_B*(h_p_max_B-h_p_min_B) "Volumetric energy density (J/m3)";
  parameter SI.Length D_tank = 7.826 "Fixed tank diameter (m)";
  parameter SI.Volume V_tank_A = E_max*frac_1/(E_v_A);
  parameter SI.Volume V_tank_B = E_max*(1-frac_1)/(E_v_B);
  
  parameter Real ar_A = 4.0*V_tank_A/(CN.pi*D_tank*D_tank*D_tank)"Aspect ratio H/D of tanks (-)";
  parameter Real ar_B = 4.0*V_tank_B/(CN.pi*D_tank*D_tank*D_tank)"Aspect ratio H/D of tanks (-)";
  ...
  parameter SI.Temperature T_heater_des = 300.0 + 273.15 "Design receiver inlet temperature (K)";
  parameter SI.Temperature T_min = 300.0 + 273.15 "Minimum temperature (K)";
  ...
  //Tank Geometry
  parameter SI.MassFlowRate m_flow_process_des = 30.0;
  ...
  parameter SI.Power Q_process_des = m_flow_process_des*(h_f_process_des - h_f_min) "Design required process heat-rate (W_th)";
  ...
  parameter SI.MassFlowRate m_flow_charge_des = (Q_heater_des - Q_process_des) / (h_f_max - h_f_min) "Design charging mass flow rate assuming design temperature outlet (kg/s)";
  parameter SI.MassFlowRate m_flow_discharge_des = Q_process_des / (h_f_max - h_f_min)  "Design discharging mass flow rate assuming design temperature outlet (kg/s)";
  //Derived Parameters
  parameter SI.Time t_cycle = t_charge + t_discharge "Total duration of one cycle (s)";
  //this is 24 hours
  
  //Storage sizing equations
  //Fluid
  parameter SI.SpecificEnthalpy h_f_min = Fluid_Package.h_Tf(T_min, 0) "Starting enthalpy of the HTF";
  parameter SI.SpecificEnthalpy h_f_max = Fluid_Package.h_Tf(T_max, 1.0) "Starting enthalpy of the HTF";
  ...
  Modelica.Blocks.Sources.RealExpression m_flow_Heater(y = m_flow_heater_signal) annotation(
  ...
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction thermocline_Splitter1(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {-2, 71.5547}, extent = {{-16, 0}, {16, 22.4453}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression m_flow_Process(y = m_flow_process_signal) annotation(
  ...
  SI.MassFlowRate m_flow_process_signal(start = 0.0);
  ...
  parameter SI.Energy X_max_stor = t_discharge * m_flow_discharge_des * ((h_f_max-h_f_min)-T_amb_des*(s_f_max-s_f_min)) "Ideal exergetic storage capacity of the TES (J)";
  parameter SI.Energy X_max_sys = t_discharge * m_flow_process_des * ((h_f_process_des-h_f_min)-T_amb_des*(s_f_process_des-s_f_min)) "Ideal Exergy delivered by the system (J)";
  ...
  //PCM Overall Liq Fraction
  Real liq_frac_B = sum(TES.Tank_B.f_p)/N_f_B "Overall PCM liquid fraction of Tank B assuming equal masses for each vertical fluid element";
...
equation
  der(E_stored) = TES.fluid_a.m_flow * (TES.fluid_a.h_outflow - TES.fluid_b.h_outflow) - TES.Q_dot_loss_total;
  
  if State == 1 then
    m_flow_heater_signal = m_flow_charge_des * (h_f_max - h_f_min) / (h_f_max - TES.h_bot_outlet);
    m_flow_process_signal = 1.0e-8;
  else 
    m_flow_process_signal = m_flow_discharge_des * (h_f_max - h_f_min) / (TES.h_top_outlet - h_f_min);
    m_flow_heater_signal = 1.0e-8;
  end if;
...
//there is mass flow
  ...
  elseif Cycle == 10 and State == 2 then //last discharging phase
    ...
    if TES.fluid_b.m_flow > 1.0e-6 then
      der(X_dis_sys) = m_flow_process_des*((h_f_process_des - h_f_min) - T_amb_des*(s_f_process_des - s_f_min));
    else
      der(X_dis_sys) = 0.0;
    end if;
    der(E_pump) = TES.W_dot_loss_pump;
...
//Connectors
  ...
  connect(thermocline_Splitter1.fluid_c, mass_loop_breaker.port_a) annotation(
    Line(points = {{-2, 83}, {-2, 64}}, color = {0, 127, 255}));
