...
model AnnularTES_ComponentLevel "Generic component-level analysis model for an annular TES system"
  ...
  package Filler_Package = SolarTherm.Materials.Concrete_Hoivik_2019;
  ...
  parameter SI.Temperature T_max = 500.0 + 273.15 "Maximum temperature";
  parameter SI.Temperature T_boiler_des = 500.0 + 273.15 "Design Power Block inlet temperature";
  parameter SI.Temperature T_boiler_min = 350.0 + 273.15 "Minimum tolerated outlet temperature to PB";
  parameter SI.Temperature T_heater_max = 300.0 + 273.15 "Maximum tolerated outlet temperature to recv";
  parameter SI.Temperature T_heater_des = 150.0 + 273.15 "Design receiver inlet temperature";
  parameter SI.Temperature T_min = 150.0 + 273.15 "Minimum temperature";
  ...
  parameter SI.Power Q_boiler_des = 100.0e6 "Design boiler discharge heat-rate";
...
//Mass flow controls
  when rem(time, t_cycle) > 1e-6 then
    Dis := false;
    if thermocline_Tank.T_bot_measured < T_heater_max then
      Chg := true;
    end if;
  end when;
  ...
  when rem(time, t_cycle) > 1e-6 + t_charge then
    Chg := false;
    if thermocline_Tank.T_top_measured > T_boiler_min then
      Dis := true;
    end if;
  end when;
  
  when thermocline_Tank.T_bot_measured > T_heater_max then
    Chg := false;
    Level_high := thermocline_Tank.Level;
  end when;
  
  when thermocline_Tank.T_top_measured < T_boiler_min then
    Dis := false;
