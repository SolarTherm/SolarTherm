...
model WindPVannularTESsystem_v2_Therminol
  ...
  replaceable package Medium = SolarTherm.Media.Therminol_66.Therminol_66_ph;
  replaceable package Fluid = SolarTherm.Materials.Therminol_66_Table;
  ...
  parameter SI.Length D_solid = 0.090;
  //0.09003;
  parameter Real U_loss_tank = 0.0;
  parameter Integer Correlation = 1; //1=Liq 2=Air
  //1:Liquid 2:Gas
  parameter Real eff_storage_des = 0.40; //Utilisation
  ...
  parameter Modelica.SIunits.Temperature T_hot_set = from_degC(345) "Ideal hot temperature";
  parameter Modelica.SIunits.Temperature T_cold_set = from_degC(100) "Ideal cold temperature";
  ...
  SolarTherm.Models.Control.WindPV_TESControl_v2 Control(redeclare package HTF = Medium, E_max = E_max, Q_des_blk = Q_flow_des, T_PB_min = T_hot_set - 100, T_PB_start = T_hot_set - 50, T_recv_max = T_cold_set + 100, T_recv_start = T_cold_set + 50, T_target = T_hot_set, eff_storage_des = eff_storage_des, h_target = h_air_hot_set, m_0 = 1e-8, m_flow_PB_des = m_boiler_des, m_min = 1e-8, m_tol = 0.001 * m_boiler_des, t_stor_startPB = 2.0 * 3600.0, t_wait = 2.0 * 3600.0)  annotation(
...
end WindPVannularTESsystem_v2_Therminol;