...
model WindPVRadChgTESsystem_Air_v2
  ...
  parameter Modelica.SIunits.MassFlowRate m_boiler_des = Q_flow_des / (h_air_PB_set - h_air_cold_set);
  ...
  SolarTherm.Models.Control.WindPV_RadTESControl_v2 windPV_RadTESControl(redeclare package HTF = Medium, Q_des_blk = Q_flow_des, T_PB_min = T_PB_min, T_PB_start = T_PB_start, T_target = T_target, T_top_max = T_top_max, T_top_start = T_top_start, h_target = h_air_PB_set, m_flow_PB_des = m_boiler_des)  annotation(
...
//grid_input.Q_defocus_y = min(gridInput.grid_input.y[1], scheduler.y[1] * (h_salt_hot_set - h_salt_cold_set));
  ...
  connect(RadChg_TES.T_top_measured, windPV_RadTESControl.T_top_tank) annotation(
    Line(points = {{-2, 36}, {36, 36}, {36, 10}, {48, 10}, {48, 10}}, color = {0, 0, 127}));
  annotation(
