...
model RadTES_Cyclical_VaryFlowRate
  ...
  parameter Real eta = 0.05;
  parameter SI.Length H_unit = 0.50;
  parameter SI.WaveNumber c_surf = 50.0;
  parameter SI.Length L_char = 0.005;
  parameter SI.Temperature T_rad = CV.from_degC(1100);
  //1=Liq 2=Air
  parameter SI.CoefficientOfHeatTransfer U_loss_tank = 0.0 "W/m2K";
  //Temperature Controls
  parameter SI.Temperature T_max = 1050 + 273.15 "Maximum temperature";
  ...
  //Storage Model
  SolarTherm.Models.Storage.Thermocline.RadCharge.RadCharge_Tank thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, T_max = T_max, T_min = T_min, Correlation = Correlation, E_max = E_max, U_loss_tank = U_loss_tank, eta = eta, H_unit = H_unit, c_surf = c_surf, L_char = L_char, T_rad = T_rad) annotation(
...
algorithm
  when rem(time, t_cycle) > 1e-6 then
    if thermocline_Tank.Tank_A.T_f[N_f-1] < T_chg_stop then
      Chg := true;
    end if;
  end when;
  
  when thermocline_Tank.Tank_A.T_f[N_f-1] > T_PB_min + 50.0 then 
  ...
  when thermocline_Tank.Tank_A.T_f[N_f-1] > T_chg_stop then
    Chg := false;
  end when;
...
end RadTES_Cyclical_VaryFlowRate;