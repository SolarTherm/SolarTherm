...
model SB_Reference
  ...
  Modelica.Blocks.Sources.RealExpression parasities_input(y = heliostatsField.W_loss + pumpHot.W_loss + pumpCold.W_loss) annotation(
    Placement(visible = true, transformation(origin = {113, 58}, extent = {{-13, -10}, {13, 10}}, rotation = -90)));
  ...
  // ReceiverControl
  // Power block
  SolarTherm.Models.PowerBlocks.PowerBlockModel powerBlock(redeclare package Medium = Medium, W_des = P_gross, enable_losses = blk_enable_losses, redeclare model Cycle = Cycle(p_bo = p_blk), nu_min = nu_min_blk, external_parasities = external_parasities, W_base = W_base_blk, p_bo = p_blk, T_des = blk_T_amb_des, nu_net = nu_net_blk, T_in_ref = T_in_ref_blk, T_out_ref = T_out_ref_blk, Q_flow_ref = Q_flow_des, redeclare model Cooling = Cooling(T_des = blk_T_amb_des)) annotation(
    Placement(visible = true, transformation(origin = {103.421, 21.7222}, extent = {{-28.7895, -30.3889}, {28.7895, 30.3889}}, rotation = 0)));
  // Price
  ...
  SolarTherm.Models.CSP.CRS.Receivers.SB_Receiver receiver(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {-26, 24}, extent = {{-16, -16}, {16, 16}}, rotation = 0)));
  SolarTherm.Models.Storage.PCM.DirectContact.SB_PCMStorage2 tankHot(N_tray = 400, T_max = 1118, T_min = 1028, T_start = 1028, U_loss_tank = 0.1, d_tank = 20, growth_ratio = 1.1, m_HTF_start = 150000, n = 10, z_PCM = 0.08, z_tank = 20, z_tray = 0.10) annotation(
  ...
  SolarTherm.Models.Control.SB_PowerBlockControl controlHot annotation(
...
//Connections from data
  ...
  connect(Tamb_input.y, powerBlock.T_amb) annotation(
    Line(points = {{119, 80}, {98, 80}, {98, 40}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
// Fluid connections
...
//PowerBlock connections
  connect(parasities_input.y, powerBlock.parasities) annotation(
    Line(points = {{113, 44}, {113, 42.5}, {109, 42.5}, {109, 40}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  connect(powerBlock.W_net, market.W_net) annotation(
    Line(points = {{118, 20}, {136, 20}}, color = {0, 0, 127}));
  P_elec = powerBlock.W_net;
  E_elec = powerBlock.E_net;
  R_spot = market.profit;
  connect(heliostatsField.heat, receiver.heat) annotation(
    Line(points = {{-56, 28}, {-42, 28}, {-42, 29}}, color = {191, 0, 0}));
  ...
  connect(pumpCold.fluid_b, receiver.fluid_a) annotation(
    Line(points = {{-12, 10}, {-23, 10}}, color = {0, 127, 255}));
  connect(Tamb_input.y, receiver.Tamb) annotation(
    Line(points = {{120, 80}, {-26, 80}, {-26, 36}, {-26, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tamb_input.y, tankHot.T_amb) annotation(
    Line(points = {{120, 80}, {32, 80}, {32, 36}, {30, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(receiver.fluid_b, loop_breaker1.port_a) annotation(
    Line(points = {{-20, 32}, {-14, 32}, {-14, 32}, {-14, 32}}, color = {0, 127, 255}));
  ...
  connect(receiver.Q_recv_in, controlCold.Q_recv_in) annotation(
    Line(points = {{-22, 24}, {-16, 24}, {-16, -14}, {-21, -14}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  ...
  connect(powerBlock.fluid_b, pumpHot.fluid_a) annotation(
    Line(points = {{87, 8}, {72, 8}}, color = {0, 127, 255}));
  ...
  connect(loop_breaker2.port_b, powerBlock.fluid_a) annotation(
    Line(points = {{78, 32}, {91, 32}}, color = {0, 127, 255}));
  ...
  connect(controlCold.m_flow_recv, pumpCold.m_flow) annotation(
    Line(points = {{-42, -14}, {-48, -14}, {-48, -6}, {4, -6}, {4, 24}, {-6, 24}, {-6, 16}, {-6, 16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  annotation(
