...
model RadTES_Cyclical_VaryFlowRate_v2
  ...
  package Filler_Package = SolarTherm.Materials.Al2O3_Constant;
  ...
  parameter SI.WaveNumber c_surf = 80.0;
  parameter SI.Length L_char = 0.013;
  ...
  parameter SI.Temperature T_PB_des = 650.0 + 273.15 "Design blended Power Block inlet temperature";
  parameter SI.Temperature T_PB_min = 650.0 + 273.15 "Minimum tolerated outlet temperature to PB";
  ...
  parameter SI.TemperatureDifference T_tol_PB = 600.0 "Power block Temperature Tolerance (K)";
  parameter SI.Temperature T_chg_stop = 1000.0 + 273.15 "Minimum temperature";
  //Tank Geometry
  parameter Real RM = 2.0 "Renewable Multiple";
  parameter SI.Power Q_dis_des = 600.0e6 "Design storage discharge heat-rate";
  parameter SI.Power Q_chg_des = RM * Q_dis_des "Design storage charge heat-rate";
  parameter Real stor_oversize_factor = 1.00;
  parameter SI.Energy E_max = t_discharge * Q_dis_des * stor_oversize_factor "Ideal storage capacity (J_thermal)";
  parameter SI.Time t_charge = (RM - 1.0) * t_discharge "Charging period";
  ...
  parameter SI.SpecificEnthalpy h_f_des = Fluid_Package.h_Tf(T_PB_des, 1.0);
  parameter SI.MassFlowRate m_charge_des = Q_chg_des / (h_f_max - h_f_min); //not used
  ...
  //Storage Model
  SolarTherm.Models.Storage.Thermocline.RadCharge.RadCharge_Tank_v2 thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, T_max = T_max, T_min = T_min, Correlation = Correlation, E_max = E_max, U_loss_tank = U_loss_tank, eta = eta, H_unit = H_unit, c_surf = c_surf, L_char = L_char, T_rad_max = T_rad_max, A_radperkg = A_radperkg, em_wire = em_wire) annotation(
  ...
  //Real util_energy_A(start = 0.0) "Storage utilisation of Tank A";T_p
...
algorithm
  when rem(time, t_cycle) > 1.0e-6 then
    //if thermocline_Tank.Tank_A.T_p[2] < T_chg_stop - 50.0 then
      Chg := true;
    //end if;
  end when;
  
  when thermocline_Tank.Tank_A.T_f[N_f] > T_PB_min + 50.0 then 
      //if rem(time, t_cycle) < t_charge +t_discharge then
        Dis := true;
      //end if;
  end when;
  
  //when rem(time, t_cycle) > t_charge then
    //if thermocline_Tank.Tank_A.T_f[N_f] > T_PB_min + 50.0 then
      //Dis := true;
    //end if;
  //end when;
  ...
  //when thermocline_Tank.Tank_A.T_p[2] > T_chg_stop then
    //Chg := false;
  //end when;
  
  when rem(time, t_cycle) > t_charge + 1e-6 then //Shut down
    Chg := false;
    //Dis := false;
  end when;
  
  when thermocline_Tank.Tank_A.T_f[N_f] < T_PB_min then 
      Dis := false;
  end when; 
...
end RadTES_Cyclical_VaryFlowRate_v2;