...
model Baseline_3Tank_Sequential
  //Baseline 3-tank sequential charging/discharging component analysis simulation from Chapter 8.3
  //Three tanks are in parallel. Charging in order C-B-A and discharging in A-B-C.
  ...
  parameter Integer N_f = 360 "Number of fluid CVs in each tank";
  ...
  //Storage Model
  SolarTherm.Models.Storage.Thermocline.Parallel.Thermocline_Spheres_3P_Sequential thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package, redeclare package Filler_Package_B = Filler_Package, redeclare package Filler_Package_C = Filler_Package, frac_1 = frac_1, frac_2 = frac_2, N_f_A = N_f, N_p_A = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar_A = ar, ar_B = ar_B, ar_C = ar_C, eta_A = eta, d_p_A = d_p, U_loss_tank_A = U_loss_tank, Correlation = Correlation, T_bot_high = T_Recv_max - 0.5, T_top_low = T_PB_min + 0.5) annotation(
