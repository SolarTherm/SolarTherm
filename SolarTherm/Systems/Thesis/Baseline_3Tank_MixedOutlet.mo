...
model Baseline_3Tank_MixedOutlet
  //Baseline 3-tank mixed-outlet component analysis simulation from Chapter 8.3
  ...
  parameter Integer N_f = 10 "Number of fluid CVs in each tank";//360
...
algorithm
  ...
  when rem(time,t_cycle) > 1.0e-6 then
    //Replace time + t_cycle - 1.0e-6 with 3600.0*24.0*10.0 to bypass the 1% difference check and always simulate 10 days
...
end Baseline_3Tank_MixedOutlet;