...
model AnnularTES_Cyclical
  ...
  //Tank Geometry
  parameter SI.Power Q_dis_des = 100.0e6 "Design storage discharge heat-rate";
  parameter SI.Power Q_chg_des = (t_discharge/t_charge)*Q_dis_des "Design storage charge heat-rate";
  parameter SI.Energy E_max = t_discharge * Q_dis_des "Ideal storage capacity (J_thermal)";
  ...
  parameter SI.MassFlowRate m_charge = Q_chg_des / (h_f_max - h_f_min);
  parameter SI.MassFlowRate m_discharge = Q_dis_des / (h_f_max - h_f_min);
  //Output signals
