...
model TestSCO2PB_Zeb
  ...
  Modelica.Blocks.Sources.RealExpression parasities_input(y = 0) annotation(
    Placement(visible = true, transformation(origin = {137, 92}, extent = {{-15, -12}, {15, 12}}, rotation = 180)));
  ...
  // ReceiverControl
  // Power block
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.recompPB powerBlock(redeclare package MedRec = Medium, E_net(displayUnit = ""), N_HTR = 40, N_LTR = 40, N_exch = 40, PR = 2.726281352, P_gro = 10e6, T_HTF_in_des = 700 + 273.15, T_amb_des = blk_T_amb_des, T_high = 700 + 273.15, T_low = from_degC(45), external_parasities = false, gamma = 0.221, m_HTF_des = 85.8, nu_min = nu_min_blk, p_high = 25e6) annotation(
    Placement(visible = true, transformation(extent = {{84, 4}, {120, 42}}, rotation = 0)));
  // Price
  ...
  SolarTherm.Models.Fluid.Sources.FluidSink fluidSink(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {10, 2}, extent = {{26, -26}, {-26, 26}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression Tamb(y = 298.15) annotation(
    Placement(visible = true, transformation(origin = {17, 105}, extent = {{-15, -13}, {15, 13}}, rotation = 0)));
...
// Fluid connections
  connect(pumpHot.fluid_b, powerBlock.fluid_a) annotation(
    Line(points = {{78, 50}, {86, 50}, {86, 29}, {94, 29}}, color = {0, 127, 255}));
...
//PowerBlock connections
  connect(parasities_input.y, powerBlock.parasities) annotation(
    Line(points = {{120.5, 92}, {120.5, 60}, {106, 60}, {106, 34}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(powerBlock.W_net, market.W_net) annotation(
    Line(points = {{111, 22}, {128, 22}}, color = {0, 0, 127}));
  ...
  connect(powerBlock.fluid_b, fluidSink.port_a) annotation(
    Line(points = {{92, 15}, {64, 15}, {64, 2}, {36, 2}}, color = {0, 127, 255}));
  ...
  connect(Tamb.y, powerBlock.T_amb) annotation(
    Line(points = {{33.5, 105}, {98, 105}, {98, 34}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
