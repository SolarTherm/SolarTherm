...
model WindPVannularTESsystem_v2
  ...
  //1:Liquid 2:Gas
  parameter Real eff_storage_des = 0.30; //Utilisation
  ...
  parameter Modelica.SIunits.Temperature T_hot_set = from_degC(350) "Ideal hot temperature";
  parameter Modelica.SIunits.Temperature T_cold_set = from_degC(50) "Ideal cold temperature";
  ...
  SolarTherm.Models.Control.WindPV_TESControl_v2 Control(redeclare package HTF = Medium, E_max = E_max, Q_des_blk = Q_flow_des, T_PB_min = T_hot_set - 40, T_PB_start = T_hot_set - 30, T_recv_max = T_cold_set + 100, T_recv_start = T_cold_set + 75, T_target = T_hot_set, eff_storage_des = eff_storage_des, h_target = h_air_hot_set, m_0 = 1e-7, m_flow_PB_des = m_boiler_des, m_min = 1e-7, m_tol = 0.01 * m_boiler_des, t_stor_startPB = 2.0 * 3600.0, t_wait = 2.0 * 3600.0)  annotation(
