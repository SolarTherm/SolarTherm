...
model HBSTES_CompLevel_Adaptive_2TSplit "This component analysis allows a variable cycle length where charging and discharging follow immediately after each other assuming the system temperate constraints have not been violated. Tank heights are fixed to 35m for all cases."
  ...
  SolarTherm.Models.Storage.Thermocline.Parallel.Thermocline_HBS_LC_2P_Split TES(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package, redeclare package Filler_Package_B = Filler_Package, N_f_A = N_f, T_max = T_max, T_min = T_min, Correlation = Correlation, E_max = E_max, ar_A = ar, d_p_A = d_p, eta_A = epsilon, U_loss_top_A = U_loss_top, U_loss_bot_A = U_loss_bot, T_PB_set = T_high_set, T_recv_set = T_low_set) annotation(
...
//Connectors
  ...
  annotation(
    experiment(StopTime = 864000, StartTime = 0, Tolerance = 1e-4, Interval = 300),
...
end HBSTES_CompLevel_Adaptive_2TSplit;