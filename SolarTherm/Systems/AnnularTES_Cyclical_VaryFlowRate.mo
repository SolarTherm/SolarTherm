...
model AnnularTES_Cyclical_VaryFlowRate
  ...
  parameter SI.Length D_solid = 0.08;
  ...
  parameter Real stor_oversize_factor = 3.0;
...
algorithm
  ...
  when rem(time, t_cycle) > 1e-6 then //Start charging
    if thermocline_Tank.T_bot_measured > T_Recv_max then
      Control_State := 2; //Cannot charge
    else
      Control_State := 1; //Can charge
    end if;
  end when;
  when rem(time, t_cycle) > t_charge + 1e-6 + t_extension then //Stop charging and start discharging time is up
    if thermocline_Tank.T_top_measured < T_PB_min then
      Control_State := 2; //Cannot discharge
    else
      Control_State := 3; //Can discharge
    end if;
  end when;
  ...
  when rem(time,t_cycle) > 1.0e-6 then
    ...
    if abs(((E_dis_total - E_dis_prev)/E_max) - util_prev)/(util_prev) < 0.01 and div(time,t_cycle) > 0.0 then //stable cycle
...
equation
  if Control_State == 1 then //Charging
    ...
    m_PB_signal = 1.0e-8;
  elseif Control_State == 2 then //Standby 
...
end AnnularTES_Cyclical_VaryFlowRate;