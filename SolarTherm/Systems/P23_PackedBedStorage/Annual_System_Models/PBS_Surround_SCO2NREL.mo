...
//This is a Gen3 inspired annual system model with two towers for and equivalent of 100 MWe
...
//Actually SM*10"
  ...
  parameter FI.PowerPrice pri_bop = if currency == Currency.USD then 0.29 else 0.29 "Balance of plant cost per gross rated power"; //not used
  ...
  parameter FI.Money C_tower = 3000000.0 * exp(0.0113 * H_tower) * 2.0 "Tower cost"; //two-tower system
  parameter FI.Money C_receiver = (4780420.0 + 21480230.0*(D_recv/14.0)*(H_recv/14.5)^0.6)*2.0 "Receiver cost";//two-tower system
  ...
  SolarTherm.Models.Control.PBS_Controller_PBLimit Control(redeclare package HTF = Medium, T_recv_max = T_recv_max, T_PB_min = T_PB_min, T_target = T_max, m_flow_PB_des = m_flow_blk_des, Q_des_blk = Q_flow_ref_blk, T_recv_start = T_recv_start, T_PB_start = T_PB_start, t_wait = t_PB_wait, eff_storage_des = eff_storage_des, t_stor_startPB = t_stor_startPB, E_max = Tank.E_max) annotation(
...
//  Line(points = {{-113, 48}, {-32, 48}, {-32, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tamb_input.y, Tank.T_amb) annotation(
    Line(points = {{99, 80}, {16, 80}, {16, 36}, {19, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.p_amb, Pres_input.y) annotation(
    Line(points = {{33, 36}, {72, 36}, {72, 96}, {99, 96}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  ...
  connect(Splitter_top.fluid_c, Tank.fluid_a) annotation(
    Line(points = {{26, 54}, {26, 49}}, color = {0, 127, 255}));
  connect(receiver.Q_rcv_raw, Control.Q_rcv_raw) annotation(
    Line(points = {{-24, 20}, {10, 20}, {10, -13}, {52, -13}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_recv, pumpCold.m_flow) annotation(
    Line(points = {{69, -13}, {70, -13}, {70, -2}, {14, -2}, {14, 14}, {-2, 14}, {-2, 10}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_PB, pumpHot.m_flow) annotation(
    Line(points = {{69, -17}, {76, -17}, {76, 76}, {66, 76}, {66, 72}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.defocus, receiver.defocus) annotation(
    Line(points = {{69, -21}, {70, -21}, {70, -26}, {-38, -26}, {-38, 20}, {-34, 20}}, color = {255, 0, 255}, pattern = LinePattern.Dash));
  connect(Tank.h_bot_outlet, Control.h_tank_outlet) annotation(
    Line(points = {{22, 26}, {22, 18}, {56, 18}, {56, -9}, {57, -9}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(powerBlock.h_out_signal, Control.h_PB_outlet) annotation(
    Line(points = {{84, 4}, {64, 4}, {64, -9}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.Q_defocus, receiver.Q_defocus) annotation(
    Line(points = {{51, -25}, {-18, -25}, {-18, 16}, {-24, 16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_top_measured, Control.T_top_tank) annotation(
    Line(points = {{34, 44}, {48, 44}, {48, -19}, {52, -19}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_bot_measured, Control.T_bot_tank) annotation(
    Line(points = {{34, 28}, {44, 28}, {44, -23}, {52, -23}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.Level, Control.Level) annotation(
    Line(points = {{34, 38}, {46, 38}, {46, -16}, {52, -16}, {52, -16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
protected
  annotation(
