within SolarTherm.Systems.P23_PackedBedStorage.Constant_Charging_Models;

model Part8_520_MgO_710
  //PCM top and bottom
  ...
  //Do not change
  package Filler_Package_B = SolarTherm.Materials.MgO_Constant;
  package Filler_Package_A = SolarTherm.Materials.PCM_520;
  package Filler_Package_C = SolarTherm.Materials.PCM_710;
  //Design Parameters
  //Fixed
  ...
  parameter Real frac_3 = 1.0 - frac_1 - frac_2 "Fraction of E_max in Tank C";
  
  parameter Integer N_f = 50 "Total Number of fluid CVs in total";
  parameter Integer N_p = 10 "Number of filler CVs  in main tank";
  
  parameter SI.Length t_e_A = 1.0e-3 "Encapsulation thickness of Filler A";
  parameter SI.Length t_e_C = 1.0e-3 "Encapsulation thickness of Filler C";
  ...
  parameter Integer N_f_C = SolarTherm.Utilities.Round(frac_3*N_f);
  //Study this
  ...
  parameter Real ar = 2.0 "Tank aspect ratio (Default)";
  parameter Real ar_B = ar*(frac_2);
  parameter Real ar_A = ar*(frac_1)*((E_dens_A/E_dens_B)^0.5);
  parameter Real ar_C = ar*(frac_3)*((E_dens_C/E_dens_B)^0.5);
  
  parameter Real E_dens_A = Filler_Package_A.rho_Tf(T_max, 1.0)*(Filler_Package_A.h_Tf(T_max, 1.0)-Filler_Package_A.h_Tf(T_min, 0.0))*(1.0-eta) + 0.5*(Fluid_Package.rho_Tf(T_min,0)+Fluid_Package.rho_Tf(T_max,0))*(Fluid_Package.h_Tf(T_max,0)-Fluid_Package.h_Tf(T_min,0))*(eta);
  parameter Real E_dens_B = Filler_Package_B.rho_Tf(T_max, 1.0)*(Filler_Package_B.h_Tf(T_max, 1.0)-Filler_Package_B.h_Tf(T_min, 0.0))*(1.0-eta) + 0.5*(Fluid_Package.rho_Tf(T_min,0)+Fluid_Package.rho_Tf(T_max,0))*(Fluid_Package.h_Tf(T_max,0)-Fluid_Package.h_Tf(T_min,0))*(eta);
  parameter Real E_dens_C = Filler_Package_C.rho_Tf(T_max, 1.0)*(Filler_Package_C.h_Tf(T_max, 1.0)-Filler_Package_C.h_Tf(T_min, 0.0))*(1.0-eta) + 0.5*(Fluid_Package.rho_Tf(T_min,0)+Fluid_Package.rho_Tf(T_max,0))*(Fluid_Package.h_Tf(T_max,0)-Fluid_Package.h_Tf(T_min,0))*(eta);
  ...
  //COntrol
  SolarTherm.Models.Storage.Thermocline.Series.Thermocline_Spheres_SGroup3_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package_A = Filler_Package_A, redeclare package Filler_Package_B = Filler_Package_B, redeclare package Filler_Package_C = Filler_Package_C, frac_1 = frac_1, frac_2 = frac_2, N_f_A = N_f_A, N_f_B = N_f_B, N_f_C = N_f_C, N_p_A = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar_A = ar_A, ar_B = ar_B, ar_C = ar_C, eta_A = eta, d_p_A = d_p, U_loss_tank_A = U_loss_tank, Correlation = Correlation, t_e_A = t_e_A, t_e_C = t_e_C) annotation(
  ...
  Real W "Non-dimensional thermocline degradation width, combined height";
...
//end if;
equation
  //Thermocline degradation width
  W = SolarTherm.Utilities.Thermocline.Degradation_Width_2(thermocline_Tank.ZDH,thermocline_Tank.T_f_degC,0.05,0.95,T_min-273.15,T_max-273.15);
