...
model Part1_Annular
  ...
  parameter SI.Temperature T_max = 390 + 273.15 "Maximum temperature";
  parameter SI.Temperature T_PB_min = 290 + 273.15 "Minimum tolerated outlet temperature to PB";
  parameter SI.Temperature T_Recv_max = 200 + 273.15 "Maximum tolerated outlet temperature to recv";
  ...
  parameter SI.Length D_pipe = 0.02 "Air pipe diameter (m)";
  parameter SI.Length D_solid = 0.09003 "Solid cylinder outer diameter (m)";
  parameter SI.Length L_pipe = 23.0 "Length of the pipe in x-axis (m)";
  //Optimise
  //parameter SI.CoefficientOfHeatTransfer U_loss_tank = 0.1 "W/m2K";
  parameter SI.Power P_name = 11.67e3 * (t_charge / t_discharge) "Nameplate power block";
  ...
  //Control
  SolarTherm.Models.Storage.Thermocline.Thermocline_Annular_SingleTank_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, Correlation = Correlation, E_max = E_max, L_pipe=L_pipe,D_pipe=D_pipe,D_solid=D_solid) annotation(
  ...
  parameter SI.Energy E_lost = 0;
  parameter SI.Energy E_pump = 0;
