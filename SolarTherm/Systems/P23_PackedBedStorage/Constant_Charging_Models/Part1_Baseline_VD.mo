...
model Part1_Baseline_VD
  ...
  //Control
  SolarTherm.Models.Storage.Thermocline.Variable_Density.Thermocline_Spheres_SingleTank_VD_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar = ar, eta = eta, d_p = d_p, U_loss_tank = U_loss_tank, Correlation = Correlation) annotation(
  ...
  Integer CS(start = 1) "Control state";
  Models.Fluid.HeatExchangers.mass_loop_breaker mass_loop_breaker annotation(
...
//W = SolarTherm.Utilities.Thermocline.Degradation_Width(thermocline_Tank.Tank_A.z_f,thermocline_Tank.Tank_A.T_f,0.05,0.95)/thermocline_Tank.Tank_A.H_tank;
  thermocline_Tank.Control_State = CS;
...
//subtract pumping power
  ...
  connect(mass_loop_breaker.port_b, thermocline_Tank.fluid_a) annotation(
    Line(points = {{0, 36}, {0, 36}, {0, 28}, {0, 28}}, color = {0, 127, 255}));
  annotation(
