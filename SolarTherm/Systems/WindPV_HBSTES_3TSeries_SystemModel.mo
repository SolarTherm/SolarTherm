...
model WindPV_HBSTES_3TSeries_SystemModel "Generic system model with WindPV input, Annular TES component and a boiler"
  ...
  parameter Real util_storage_des = 0.5265; //Utilisation determined via component-level analysis 0.5767
  parameter Real level_storage_mid = 0.4921; //Midpoint of minimum and maximum storage levels determine via component-level analysis 0.4637
  ...
  */
  ...
  parameter SI.Temperature T_boiler_start = 1050.0 + 273.15 "Temperature above-which TES can start discharge (K)"; //50K buffer
  ...
  parameter SI.Temperature T_heater_start = 690.0 + 273.15 "Temperature below-which TES can start charging (K)"; //50k buffer
...
end WindPV_HBSTES_3TSeries_SystemModel;