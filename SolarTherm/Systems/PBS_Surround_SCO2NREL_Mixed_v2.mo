...
model PBS_Surround_SCO2NREL_Mixed_v2
...
//Actually SM*10"
  ...
  parameter SI.Time t_stor_start_dis = 1.0 * 3600.0 "minimum hours of storage available to startup PB";
  ...
  parameter Real nu_start = 0.4 "Minimum energy start-up fraction to start the solar field";
  parameter Real nu_min_sf = 0.3 "Minimum turn-down energy fraction to stop the solar field";
  parameter Real nu_recv_min = 0.10 "Minimum receiver mass flow rate fraction";
  ...
  parameter SI.MassFlowRate m_recv_min = nu_recv_min * m_flow_recv_des;
  ...
  SolarTherm.Models.Control.Thermocline_Controller_PBLimit_v2 Control(redeclare package HTF = Medium, T_recv_max = T_recv_max, T_PB_min = T_PB_min, T_target = T_max, T_min = T_min, m_flow_PB_des = m_flow_blk_des, Q_des_blk = Q_flow_ref_blk, T_recv_start = T_recv_start, T_PB_start = T_PB_start, t_wait = t_PB_wait, eff_storage_des = eff_storage_des, t_stor_start_dis = t_stor_start_dis, E_max = Tank.E_max, m_recv_min = m_recv_min) annotation(
    Placement(visible = true, transformation(origin = {66, -24}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  //Analytics
...
//  Line(points = {{-113, 48}, {-32, 48}, {-32, 36}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tamb_input.y, Tank.T_amb) annotation(
    Line(points = {{99, 80}, {14, 80}, {14, 32}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  ...
  connect(Splitter_top.fluid_c, Tank.fluid_a) annotation(
    Line(points = {{26, 54}, {26, 52}, {22, 52}, {22, 46}}, color = {0, 127, 255}));
  connect(receiver.Q_rcv_raw, Control.Q_rcv_raw) annotation(
    Line(points = {{-24, 20}, {10, 20}, {10, -19}, {58, -19}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_recv, pumpCold.m_flow) annotation(
    Line(points = {{75, -19}, {70, -19}, {70, -2}, {14, -2}, {14, 14}, {-2, 14}, {-2, 10}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.m_flow_PB, pumpHot.m_flow) annotation(
    Line(points = {{75, -23}, {76, -23}, {76, 76}, {66, 76}, {66, 72}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.defocus, receiver.defocus) annotation(
    Line(points = {{75, -27}, {70, -27}, {70, -26}, {-38, -26}, {-38, 20}, {-34, 20}}, color = {255, 0, 255}, pattern = LinePattern.Dash));
  connect(powerBlock.h_out_signal, Control.h_PB_outlet) annotation(
    Line(points = {{84, 4}, {70, 4}, {70, -15}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.Q_defocus, receiver.Q_defocus) annotation(
    Line(points = {{57, -31}, {-18, -31}, {-18, 16}, {-24, 16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_top_measured, Control.T_top_tank) annotation(
    Line(points = {{30, 42}, {30, 13.5}, {58, 13.5}, {58, -22}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.T_bot_measured, Control.T_bot_tank) annotation(
    Line(points = {{30, 22}, {30, -8.5}, {58, -8.5}, {58, -29}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.Level, Control.Level) annotation(
    Line(points = {{30, 36}, {46, 36}, {46, -25}, {58, -25}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Control.h_tank_top, Tank.h_top_outlet) annotation(
    Line(points = {{60, -16}, {60, -16}, {60, 50}, {18, 50}, {18, 44}, {18, 44}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(Tank.h_bot_outlet, Control.h_tank_bot) annotation(
    Line(points = {{18, 20}, {20, 20}, {20, -6}, {64, -6}, {64, -16}, {64, -16}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
protected
  annotation(
...
end PBS_Surround_SCO2NREL_Mixed_v2;