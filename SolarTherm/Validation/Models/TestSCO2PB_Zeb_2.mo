...
model TestSCO2PB_Zeb_2
  ...
  replaceable package Medium = SolarTherm.Media.CO2.CO2_ph "Medium props for Chloride Salt";
  ...
  // Receiver lift
  // Pump Hot
  SolarTherm.Models.Fluid.Pumps.PumpSimple_EqualPressure pumpHot(redeclare package Medium = Medium, k_loss = 0) annotation(
    Placement(transformation(origin = {74, 44}, extent = {{0, 0}, {12, 12}})));
  // Temperature sensor
  // PowerBlockControl
  // ReceiverControl
  // Power block
  SolarTherm.Models.PowerBlocks.sCO2Cycle.DirectDesign.recompPB_2 powerBlock(redeclare package MedRec = Medium, E_net(displayUnit = ""), P_gro = 10e6, T_amb_des = 298.15, T_low = from_degC(36), eta_turb = 0.90, external_parasities = false, m_HTF_des = 87.13, nu_min = 0.1) annotation(
