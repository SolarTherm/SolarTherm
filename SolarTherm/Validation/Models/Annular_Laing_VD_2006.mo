...
model Annular_Laing_VD_2006
  ...
  Modelica.Blocks.Sources.RealExpression p_amb(y = 101325) annotation(
    Placement(visible = true, transformation(origin = {63, -16}, extent = {{13, -16}, {-13, 16}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression m_flow_Recv(y = m_Recv_signal) annotation(
    Placement(visible = true, transformation(origin = {-113, 7}, extent = {{-19, -17}, {19, 17}}, rotation = 0)));
...
//fluid_bot.T = T_bot;
  connect(p_amb.y, thermocline_Tank.p_amb) annotation(
    Line(points = {{49, -16}, {30.5, -16}, {30.5, -2}, {18, -2}}, color = {0, 0, 127}));
  ...
  connect(m_flow_Recv.y, pumpSimple_EqualPressure.m_flow) annotation(
    Line(points = {{-92, 7}, {-72, 7}, {-72, 70}, {-54, 70}, {-54, 56}}, color = {0, 0, 127}));
  ...
  connect(State.y, thermocline_Tank.Control_State) annotation(
    Line(points = {{55, 12}, {40, 12}, {40, 4}, {18, 4}}, color = {255, 127, 0}));
  connect(pumpSimple_EqualPressure.fluid_b, thermocline_Tank.fluid_a) annotation(
    Line(points = {{-44, 48}, {0, 48}, {0, 28}, {0, 28}}, color = {0, 127, 255}));
  connect(thermocline_Tank.fluid_b, Recv_Sink.port_a) annotation(
    Line(points = {{0, -32}, {0, -32}, {0, -36}, {-94, -36}, {-94, -36}}, color = {0, 127, 255}));
  annotation(
    experiment(StopTime = 22680, StartTime = 0, Tolerance = 1e-3, Interval = 180.0));
end Annular_Laing_VD_2006;