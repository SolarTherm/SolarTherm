...
model Thermocline_Meier_1991
  ...
  parameter SI.Temperature T_p_start[N_f,N_p] = fill(fill(150.0+273.15,N_p),N_f);
  parameter SI.Temperature h_p_start[N_f,N_p] = fill(fill(Filler.h_Tf(150.0+273.15,0.0),N_p),N_f);
 ...
 SolarTherm.Models.Storage.Thermocline.Thermocline_Spheres_SingleTank_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid, redeclare package Filler_Package = Filler, Correlation = Correlation, Tank_A.H_tank = H_tank, Tank_A.D_tank = D_tank, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, U_loss_tank = U_loss_tank, d_p = d_p, eta = eta) annotation(
