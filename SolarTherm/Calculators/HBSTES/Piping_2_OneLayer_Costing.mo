within SolarTherm.Calculators.HBSTES;

model Piping_2_OneLayer_Costing
  ...
  //Default Pressure
  parameter SI.Pressure p_des = 100.0e3; //Default pressure of 1 bar
  
  //Input Parameters
  parameter Real HM = 2.0; //Heater Multiple
  parameter SI.Temperature T_max = 1100.00 + 273.15;
  
  //The rest of these don't need to be changed
  parameter SI.Temperature T_process_des = 1000.00 + 273.15;
  parameter SI.Temperature T_heater_des = 450.00 + 273.15;
  parameter SI.Temperature T_min = 317.3 + 273.15;
  parameter SI.MassFlowRate m_flow_process_des = 26.6;
  
  //Get Specific Enthalpy
  parameter SI.SpecificEnthalpy h_air_max_des = Fluid_Package.h_Tf(T_max,1.0);
  parameter SI.SpecificEnthalpy h_air_process_des = Fluid_Package.h_Tf(T_process_des,1.0);
  parameter SI.SpecificEnthalpy h_air_heater_des = Fluid_Package.h_Tf(T_heater_des,1.0);
  parameter SI.SpecificEnthalpy h_air_min_des = Fluid_Package.h_Tf(T_min,1.0);
 
  //Get Density
  parameter SI.Density rho_air_max_des = Fluid_Package.rho_Tf(T_max,1.0);
  parameter SI.Density rho_air_process_des = Fluid_Package.rho_Tf(T_process_des,1.0);
  parameter SI.Density rho_air_heater_des = Fluid_Package.rho_Tf(T_heater_des,1.0);
  parameter SI.Density rho_air_min_des = Fluid_Package.rho_Tf(T_min,1.0);
  
  //Calculate heat duty
  parameter SI.HeatFlowRate Q_flow_process_des = m_flow_process_des*(h_air_process_des-h_air_min_des);
  parameter SI.HeatFlowRate Q_flow_heater_des = HM*Q_flow_process_des;

  //Calculate Mass Flow Rates
  //parameter SI.MassFlowRate m_flow_piping1_max = m_flow_process_des;
  //parameter SI.MassFlowRate m_flow_piping2_max = (HM - 1)
  
  //Worst Case Scenarios
  parameter SI.Temperature T_piping1_max = T_min;
  parameter SI.Temperature T_piping2_max = T_heater_des;
  parameter SI.Temperature T_piping3_max = T_max;
  parameter SI.Temperature T_piping4_max = T_max;
  parameter SI.Temperature T_piping5_max = T_min;
  
  parameter SI.MassFlowRate m_flow_piping1_max = m_flow_process_des;
  parameter SI.MassFlowRate m_flow_piping2_max = (m_flow_process_des*(h_air_heater_des-h_air_min_des) - Q_flow_heater_des)/(h_air_heater_des - h_air_max_des);
  parameter SI.MassFlowRate m_flow_piping3_max = m_flow_piping2_max;
  parameter SI.MassFlowRate m_flow_piping4_max = m_flow_process_des;
  parameter SI.MassFlowRate m_flow_piping5_max = m_flow_process_des*(h_air_max_des-h_air_process_des)/(h_air_max_des-h_air_min_des);
  
  //Now Feed in the Worst case Temps amd mass flow rates into Piping 1
  parameter SI.Temperature T1 = T_piping2_max "Inner Pipe temperature (K)";
  parameter SI.MassFlowRate m_flow_gas = m_flow_piping2_max "Maximum mass flow rate of air (kg/s)";
  parameter SI.Temperature T2_Max = 50.0 + 273.15 "Maximum tolerable outer insulation temperature (K)";
  parameter Real F_m_duct = 4.3 "Duct material factor (galvanised steel = 1.0, stainless steel = 4.3)";
  ...
  */
  ...
  parameter Real c2 = 110.45 "Cost per unit volume of Thermal ceramic fibre Insulation (USD_2022)";
  ...
  parameter SI.Temperature T3 = 25.0 + 273.15 "Ambient temperature (K)";
  ...
  //Calculation
  SI.Temperature T2(min = 273.15, start = 300.0 + 273.5);
  //SI.Temperature T3 (start = 200.0 + 273.15);
  parameter Real r1 = d1 / 2.0;
  SI.Length r2(min = 1e-6, start = r1 + 1e-3);
...
//der(t_sum) = 0.01;
  ...
  //d3 = 2.0 * r3;
  d2 = 2.0 * r2;
QpL=(2.0*CN.pi/log(r1/r2))*(((E/1)*(T2^1-T1^1))+((F/2)*(T2^2-T1^2))+((G/3)*(T2^3-T1^3))+((H/4)*(T2^4-T1^4)));
