...
model Heat_Sink_v2
  ...
  parameter Real flowrate_factor = 7.69461 "Multiplier to the 400L/min (divided by 6) lab air-flow input";
  ...
  //Additions
  Boolean RampDown(start=false) "Is the solar ramping down?";
  Real Solar_Factor(start=1.0) "fraction of solar irradiation";
  SI.Time time_start(start=-10.0) "Time at which cooldown starts";
  SI.Time time_stop(start=0) "Time at which cooldown stops";
  parameter SI.Time time_ramp=0.001 "Solar ramp down time";
algorithm
  when T_surf > 799.5 + 273.15 then
    RampDown := true;
    time_start := time;
  end when;
  when T_surf < 600.0 + 273.15 then
    time_stop := time;
  end when;
  
  if RampDown == true then
    Solar_Factor := 1.0 - max(min((time-time_start)/(time_ramp),1.0),0.0);
  else 
    Solar_Factor := 1.0;
  end if;
