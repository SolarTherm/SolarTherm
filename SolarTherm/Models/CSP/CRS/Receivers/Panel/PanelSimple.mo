...
model PanelSimple
	...
	parameter Real R_foul = 8.808e-5 "Fouling resistance";
...
equation	
	...
	//First element
	h_int[1] = Nu[1]*lambda[1]/(2*a)/(1 + R_foul*Nu[1]*lambda[1]/(2*a));
	...
	Nu[1] = (f[1]/8)*(Re[1]-1000)*Pr[1]/(1+12.7*(f[1]/8)^0.5*((Pr[1]^0.66)-1));
	...
	for i in 2:(Nel*Npa_fl) loop
		h_int[i] = Nu[i]*lambda[i]/(2*a)/(1 + R_foul*Nu[i]*lambda[i]/(2*a));
		...
		Nu[i] = (f[i]/8)*(Re[i]-1000)*Pr[i]/(1+12.7*(f[i]/8)^0.5*((Pr[i]^0.66)-1));
