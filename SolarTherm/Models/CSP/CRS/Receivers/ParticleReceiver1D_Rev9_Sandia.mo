...
Condition 1:	
Wall conduction is not activated, curtain radiation properties varied while particles are fallin --> maximum discretisation is 30 --> the result is OK

Condition 2
Wall conduction is activated, curtain radiation properties are fixed -->  discretisation can reach up to 50, with bizarre results (particle temperature overlaps the wall temperature in the beginning) when discretisation is less than 20.

Condition 3
If wall conduction is not activated and curtain radiation properties are fixed --> the discretisation can reach up to 50 with bizarre results (particle temperature overlaps the wall temperature in the beginning) when discretisation is less than 20.

Condition 4
If wall conduction is activated and curtain radiation properties are varied --> the discretisation can reach up to 30-->the result is OK

=============================================== END OF MODEL LIMITATIONS ==============================================
*/

model ParticleReceiver1D_Rev9_Sandia
...
// Ports Declaration
	...
	parameter Boolean fixed_geometry = false
"If true, specified H_drop, t_c_in and calculate T_out. 
...
Always use true for annual simulation, use false only when you want to run the particle 1d receiver in test rig";
    ...
    parameter Boolean with_wall_conduction = true "Whether to model vertical conduction in backwall"; // FIXME may need to revisit this
   ...
   for i in 1 : N+1 loop
	...
	// FIXME note that tau_c should have an adjustment for cos(theta) of rays.         
		...
		if with_isothermal_backwall then
			...
			q_conv_wall[i] = (T_w[i+1] - Tamb) / (1/h_conv + th_w/k_w); //q loss conv wall
			0 = (
					if with_wall_conduction
					then -k_w*(((T_w[i+2]-T_w[i+1])/(x[i+2]-x[i+1])) - ((T_w[i+1]-T_w[i])/(x[i+1]-x[i])))*th_w
					else 0
				) - (g_w[i]-(eps_w * CONST.sigma * T_w[i+1]^4 + (1-eps_w)*g_w[i]))*dx
				+ (q_conv_wall[i])*dx;
		end if;
	end for;
...
end ParticleReceiver1D_Rev9_Sandia;