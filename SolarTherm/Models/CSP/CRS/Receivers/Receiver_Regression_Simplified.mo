...
model Receiver_Regression_Simplified
  ...
  parameter Real mu_min = 0.20;
  parameter Real mu_max = 1.10;
  
  parameter SI.Temperature T_in_des = 416.2 + 273.15;
  parameter SI.Temperature T_out_des = 872.6 + 273.15;
  
  parameter SI.MassFlowRate m_flow_min = mu_min*m_flow_des;
  parameter SI.MassFlowRate m_flow_max = mu_max*m_flow_des;
  ...
  Real mu = fluid_a.m_flow / m_flow_des "Non-dimensional mass flow rate used in regression calculations (-)";
...
equation
  ...
  */
  ...
  if fluid_a.m_flow > 1e-8 then
    ...
    h_out = h_in + Q_flow_recv/fluid_a.m_flow;
  else
    Q_flow_recv = 0.0;
    //Q_flow_loss = 0.0; 
    h_out = h_out_des;
...
end Receiver_Regression_Simplified;