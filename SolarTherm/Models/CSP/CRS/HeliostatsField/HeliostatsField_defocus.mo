...
equation
  ...
  if on_internal then
    if defocus_internal then
      ...
      der(E_curtail) = Q_raw-Q_net;
      der(E_low_dumping) = 0;
      der(E_high_dumping) = 0;
    else
      Q_net=min(Q_dump_field,Q_raw);
      der(E_high_dumping)=Q_raw-Q_net;
      der(E_curtail) = 0;
      der(E_low_dumping) = 0;
    end if;
  else 
    Q_net=0;
    der(E_low_dumping) = Q_raw-Q_net;
    der(E_high_dumping)=0;
      der(E_curtail) = 0;
  end if;
