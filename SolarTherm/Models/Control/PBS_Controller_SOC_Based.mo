...
model PBS_Controller_SOC_Based
  ...
  parameter Real SOC_start_charging = 0.25 "SOC when charging has to start. Equilibirum cycle";
  parameter Real SOC_stop_charging = 0.85 "SOC when charging has to stop.Equilibirum cycle";
  
  parameter Real SOC_start_discharging = 0.5 "SOC when discharging may start. Equilibirum cycle";
  parameter Real SOC_stop_discharging = 0.21 "SOC when discharging has to stop.Equilibrium cycle";
  ...
  Modelica.Blocks.Interfaces.RealInput SOC "Temperature of the bottom of HTF in storage"
    annotation (Placement(visible = true, transformation(extent = {{-126, -28}, {-86, 12}}, rotation = 0), iconTransformation(extent = {{-126, -28}, {-86, 12}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealInput Q_rcv_raw "The net receiver heat rate before curtailment"
    annotation (Placement(visible = true, transformation(extent = {{-124, 36}, {-84, 76}}, rotation = 0), iconTransformation(extent = {{-126, 48}, {-86, 88}}, rotation = 0)));
  ...
  Real D;
algorithm
  //********************* Changing storage states
  when SOC < SOC_start_charging then
    Chg := true;
  elsewhen SOC > SOC_stop_charging then
    Chg := false;
  end when;
  
  when SOC > SOC_start_discharging then
    Disch := true;
  elsewhen SOC < SOC_stop_discharging then
    Disch := false;
  end when;
...
equation
  ...
  D = h_target-h_tank_outlet;
  
  //New logic from ZK branch
