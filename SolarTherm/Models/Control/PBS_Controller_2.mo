...
algorithm
  //timer
  when m_flow_PB < 0.1*m_0 then //PB is shut down so start the timer
    t_shutdown := time;
  end when;
  //change timer state
  when time > t_shutdown + t_wait then //we may be able to turn on the PB
    if Q_rcv_raw <= Q_rcv_min then //2 or 6
      if Disch then
        Control_State := 2;
      else
        Control_State := 6;
      end if;
    elseif Q_rcv_raw > Q_rcv_min and Q_rcv_raw < Q_des_blk then //1 4 or 6
      if Disch then
        Control_State := 4;
      else
        if Chg then
    ...
    else //Q >= Q_des_blk so either 5 or 3
      if Chg then
        Control_State := 5;
      else
        Control_State := 3;
  ...
  when T_top_tank < T_PB_min then //Tank cannot be discharged, Disch -> false
    Disch := false;
    if Q_rcv_raw <= 0.5*Q_rcv_min then //2 -> 6
      Control_State := 6;
  ...
  when T_top_tank > T_PB_start then //Can start discharging storage, Disch -> true
    Disch := true;
    if Q_rcv_raw <= Q_rcv_min then //6 -> 2, but only if timer is expired
      if time > t_shutdown + t_wait then //new
        Control_State := 2;
      end if; //new
  ...
  when T_bot_tank < T_recv_start then //Can start charging, Chg -> true
    Chg := true;
  ...
  when Q_rcv_raw > Q_rcv_min then //Originally 2 or 6
    if Q_rcv_raw >= Q_des_blk then //either 5 or 3
      if Control_State == 2 then //PB is already on so no need to check timer //new
        if Chg then //2 or 6 -> 5
          Control_State := 5;
        else //2 or 6 -> 3
          Control_State := 3;
        end if;
    ...
    else //becomes either 1, 4 or 6
      if Disch then //either 2 or 6 -> 4
        if Control_State == 2 or time > t_shutdown + t_wait then //new //we can go to 4
          Control_State := 4;
        end if;
  ...
  when Q_rcv_raw >= Q_des_blk then // 1 or 4 or 6 -> 3 or 5
    if Control_State == 4 or time > t_shutdown + t_wait then //new
      if Chg then
        Control_State := 5;
      else
        Control_State := 3;
  ...
  when Q_rcv_raw < Q_des_blk then // 5 or 3 -> 1 or 2 or 4 or 6
    if Q_rcv_raw <= Q_rcv_min then // 5 or 3 -> either 2 or 6
      if Disch then
        Control_State := 2;
      else
        Control_State := 6;
      end if;
    else //5 or 3 -> either 1 or 4 or 6, PB already on
      if Disch then
        Control_State := 4;
      else
        if Chg then
  ...
  when Q_rcv_raw <= Q_rcv_min then //1 or 4 or 6 -> 2 or 6
    if Disch then
      if Control_State == 4 or time > t_shutdown + t_wait then //new
        Control_State := 2;
      end if; //new
