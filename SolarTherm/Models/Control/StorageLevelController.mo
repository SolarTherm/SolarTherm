...
model StorageLevelController
  ...
  parameter Real L_1 = 0.5"Level to stop discharge, minimum PB temperature";
  parameter Real L_2 = 0.15 "Level to start discharge, slightly above L1";
  parameter Real L_3 = 0.85 "Level to start charge, slightly below L4";
  parameter Real L_4 = 0.95 "Level to stop charge, maximum recv temperature";
...
/* Modelica.Blocks.Interfaces.RealInput flow_dir "flow direction, 1 is charging, -1 is discharging" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput level "level of storage, from 0 to 1" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput Q_rcv_raw "The net receiver heat rate before curtailment" annotation (Placement(visible = true, transformation(extent = {{-124, 22}, {-84, 62}}, rotation = 0), iconTransformation(extent = {{-126, 48}, {-86, 88}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput h_PB_outlet "Enthalpy of the HTF coming out of PB"  annotation (Placement(visible = true, transformation(extent = {{-128, -80}, {-88, -40}}, rotation = 0), iconTransformation(origin = {56, 112},extent = {{-20, -20}, {20, 20}}, rotation = -90)));
  Modelica.Blocks.Interfaces.RealInput h_tank_outlet "Enthalpy of the HTF coming out of bottom of tank"  annotation (Placement(visible = true, transformation(extent = {{-128, -100}, {-88, -60}}, rotation = 0), iconTransformation(origin = {-40, 112},extent = {{-20, -20}, {20, 20}}, rotation = -90)));
...
*/
  ...
  when m_flow_PB <= 2.0 * m_0 then //take this as shutdown
    PB := false;
    t_threshold := time + t_wait;//start the cooldown
  end when;
...
//6 = Everything is off.
  annotation(
    Documentation(revisions = "<html>
		<p> It is originally from the PBS_Controller by Zebedee Kee on 03/12/2020. Ye changed the control strategy by checking the storage level </p>
		</html>", info = "<html>
