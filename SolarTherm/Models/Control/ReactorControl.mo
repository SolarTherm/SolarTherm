...
model ReactorControl
  ...
  Modelica.Blocks.Interfaces.RealInput m_flow_in annotation (Placement(
        ...
        rotation=0,
        origin={-108,50})));
  SolarTherm.Models.Control.StartUpLogic6 logic(
    m_flow_max=m_flow_on,
    m_flow_startup=m_flow_on/2,
    level_on=L_on,
    level_off=L_off,
    m_flow_standby=0,
    T_crit_reactor = T_crit_reactor)
  ...
  Modelica.Blocks.Interfaces.RealInput T_hot_tank annotation(
    Placement(visible = true, transformation(origin = {-110, 6}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(extent = {{-132, -18}, {-92, 22}}, rotation = 0)));
equation
  ...
  connect(T_hot_tank, logic.T_hot_tank) annotation(
    Line(points = {{-110, 6}, {-12, 6}, {-12, 4}, {-10, 4}}, color = {0, 0, 127}));
