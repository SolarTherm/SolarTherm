...
model WindPV_RadTES_Control_Final
  //The controller for Rio-Tinto ft. Rondo system model
  extends Icons.Control;
  replaceable package HTF = SolarTherm.Media.Air.Air_amb_p; //Fluid property model
  
  //Input Parameters
  parameter SI.Temperature T_boiler_min = 650.0 + 273.15 "Minimum acceptable boiler HTF inlet temperature (K), also the design blend-down temperature";
  parameter SI.Temperature T_boiler_start = 700.0 + 273.15 "Temperature of the top of the TES at which discharge can be started (K)";
  parameter SI.Temperature T_rad_max = 1200.0 + 273.15 "Maximum allowable heater element temperature at which charging is stopped (K)";
  parameter SI.Temperature T_rad_start = 1150.0 + 273.15 "Temperature of the hottest heater element below which charging can start (K)";
...
equation
  ...
  if Dis == true then
    m_flow_boiler = m_boiler_dem * (h_boiler_des - h_boiler_out) / (h_tank_top - h_boiler_out); //Mass flow after considering blend-down
  else
    m_flow_boiler = m_min;
  end if;
  ...
  annotation(
    Documentation(revisions = "<html>
		...
		<p>This component controls the steam-supply system involving a Wind-PV electrical input, boiler output and a TES that is charged internally via radiative heating.</p>
		</html>"));
end WindPV_RadTES_Control_Final;