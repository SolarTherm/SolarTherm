...
model Thermocline_Controller_PBLimit_v2
  extends Icons.Control;
  replaceable package HTF = SolarTherm.Media.Sodium.Sodium_pT;
  parameter SI.Temperature T_target = 720.0 + 273.15 "Target receiver outlet temperature";
  parameter SI.Temperature T_PB_start = 710.0 + 273.15 "PB input temperature at which it can start";
  parameter SI.Temperature T_PB_min = 700.0 + 273.15 "Minimum tolerated PB input temperature";
  parameter SI.Temperature T_recv_max = 540.0 + 273.15 "Maximum tolerated receiver input temperature";
  parameter SI.Temperature T_recv_start = 530.0 + 273.15 "Receiver input temperature at which receiver can start";
  parameter SI.Temperature T_min = 500.0 + 273.15 "Minimum system temperature";
  
  parameter SI.MassFlowRate m_flow_PB_des = 100.0 "Reference power block mass flow rate";
  parameter SI.MassFlowRate m_recv_min = 10.0 "Minimum receiver mass flow rate for safety";
  ...
  Boolean Dis(start=false) "Can the storage be discharged?";
  ...
  Modelica.Blocks.Interfaces.RealInput h_tank_bot "Enthalpy of the HTF coming out of bottom of tank"
    annotation (Placement(visible = true, transformation(extent = {{-128, -100}, {-88, -60}}, rotation = 0), iconTransformation(origin = {-30, 112},extent = {{-20, -20}, {20, 20}}, rotation = -90)));
    
  Modelica.Blocks.Interfaces.RealInput h_tank_top "Enthalpy of the HTF coming out of top of tank (J/kg)"
    annotation (Placement(visible = true, transformation(extent = {{-124, 80}, {-84, 120}}, rotation = 0), iconTransformation(origin = {-84, 112},extent = {{-20, -20}, {20, 20}}, rotation = -90)));
  ...
  parameter SI.Time t_stor_start_dis = 2.0*3600.0 "minimum hours of storage available to startup PB"; 
  ...
  parameter Real L_start_dis = t_stor_start_dis/t_stor_cap + 0.5*(1.0-eff_storage_des) "Minimum tank level to start PB to ensure the required time of stored energy available";
...
algorithm
  ...
  //Flow state calcs
  when m_guess > m_recv_min + m_tol then
...
//New controls: PB triggered by shutdown and re-enabled after tank level increases past a certain value
  when m_flow_PB < 0.2 * m_flow_PB_des then
    if Level <= L_start_dis then //try this condition
      PB := false;
    end if;
  end when;
  when Level > L_start_dis then
...
equation
  ...
  if Q_rcv_raw >= Q_des_blk then
    m_guess = (Q_rcv_raw + m_flow_PB_des*(h_PB_outlet-h_tank_bot))/(h_target-h_tank_bot);
  else
  ...
  if Control_State == 1 then
    m_flow_recv = Q_rcv_raw/(h_target-h_tank_bot);
    m_flow_PB = m_0;
    defocus = false;
  ...
  elseif Control_State == 2 then
    ...
    m_flow_PB = m_flow_PB_des*(h_target-h_PB_outlet)/(h_tank_top-h_PB_outlet);
    defocus = false;
    Q_defocus = Q_des_blk; //Not used anyway
  ...
  elseif Control_State == 4 then
    ...
    m_flow_PB = (m_flow_recv*(h_tank_top-h_target) + m_flow_PB_des*(h_target-h_PB_outlet))/(h_tank_top-h_PB_outlet);
    defocus = false;
    Q_defocus = Q_des_blk; //Not used anyway

  elseif Control_State == 5 then
    m_flow_recv = (Q_rcv_raw + m_flow_PB*(h_PB_outlet-h_tank_bot))/(h_target-h_tank_bot);
    m_flow_PB = m_flow_PB_des;
    defocus = false;
    Q_defocus = Q_des_blk; //Not used anyway
...
end Thermocline_Controller_PBLimit_v2;