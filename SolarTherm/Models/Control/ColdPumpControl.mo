...
model ColdPumpControl
  ...
  Modelica.Blocks.Math.Feedback feedback annotation(Placement(transformation(extent={{-28,38},
            {-8,18}})));
  PID_AW_reset                PI(initType = Modelica.Blocks.Types.InitPID.DoNotUse_InitialIntegratorState,                  Ti = 1, Kp = -10,           Tt = 1,
    uMax=m_flow_max,
    uMin=m_flow_min,
    xi_start=600)                                                                                                     annotation(Placement(transformation(extent={{14,18},
            {34,38}})));
...
equation
  ...
  connect(switch.y, m_flow)
    annotation (Line(points={{66.6,0},{66.6,0},{112,0}}, color={0,0,127}));
