...
model ReactorRecuperator
  ...
  replaceable package MedGas2 = Modelica.Media.IdealGases.Common.SingleGasNasa(data = Modelica.Media.IdealGases.Common.SingleGasesData.H2, mediumName = "Hydrogen", fluidConstants = {Modelica.Media.IdealGases.Common.FluidData.H2});
  replaceable package MedWater = Modelica.Media.Water.WaterIF97_ph;
  replaceable package MedAir = Modelica.Media.Air.DryAirNasa;
  ...
  /*Temperatures*/
  parameter SI.Temperature T_air_in = 20 + 273.15 "Inlet air temperature to the condenser [K]";
  parameter SI.Temperature T_air_out = T3 - 5 "Outlet air temperature from the condenser [K]";
  ...
  parameter MedWater.SaturationProperties sat = MedWater.setSat_T(T4);
  parameter MedWater.ThermodynamicState bubble_H2O_3 = MedWater.setBubbleState(sat);
  parameter MedWater.ThermodynamicState dew_H2O_3 = MedWater.setDewState(sat);
  
  parameter MedAir.ThermodynamicState state_air_in = MedAir.setState_pTX(p=1e5, T= T_air_in);
  parameter MedAir.ThermodynamicState state_air_out = MedAir.setState_pTX(p=1e5, T= T_air_out);
  ...
  // We assume that the temperature of both fluids are similar
  parameter SI.HeatFlowRate Q_condenser = m_dot_excess_H2O*(
      MedWater.specificEnthalpy(state_H2O_3) - MedWater.specificEnthalpy(bubble_H2O_3) - MedWater.specificEnthalpy(dew_H2O_3)
  ) + m_dot_excess_H2 * (
      MedGas2.specificEnthalpy(state_H2_3) - MedGas2.specificEnthalpy(state_H2_4)
  );
  parameter SI.MassFlowRate m_dot_air_condenser =  Q_condenser / (
      MedAir.specificEnthalpy(state_air_out) - MedAir.specificEnthalpy(state_air_in)
  );
  parameter SI.TemperatureDifference T_LMTD_condenser = (T3 - T_air_out - (T4 - T_air_in)) / log((T3 - T_air_out) / (T4 - T_air_in));
  
  parameter SI.Area A_condenser(fixed = false);
...
initial equation
  ...
  A_recup2 = Q_recup2 / (T_LMTD_recup2 * U_recup1);
  A_condenser = Q_condenser / (T_LMTD_condenser * U_recup2);
