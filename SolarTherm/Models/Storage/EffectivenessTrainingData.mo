...
model EffectivenessTrainingData
 ...
 //Optimise
  ...
  parameter SI.Temperature T_PB_start = 710 + 273.15 "Minimum tolerated outlet temperature to PB"; //L5
  parameter SI.Temperature T_PB_min = 680 + 273.15 "Minimum tolerated outlet temperature to PB"; //L4
  parameter SI.Temperature T_Recv_max = 570 + 273.15 "Maximum tolerated outlet temperature to recv"; //L3
  parameter SI.Temperature T_Recv_start = 540 + 273.15 "Maximum tolerated outlet temperature to recv"; //L2
  parameter SI.Temperature T_min = 510 + 273.15 "Minimum temperature"; //L1
  ...
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction Splitter1(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {0, 67.5547}, extent = {{-16, 0}, {16, 22.4453}}, rotation = 0)));
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction Splitter2(redeclare package Medium = Medium) annotation(
...
algorithm
  ...
  when storage_model.T_bot_measured > T_Recv_max then//stop charge
    m_Recv_signal := 0.0;
  ...
  when storage_model.T_top_measured < T_PB_min then//stop discharge
    m_PB_signal := 0.0;
    if time > t_cycle*5.0 then //Log this level
      L_PB_min := L;
  ...
  when storage_model.T_bot_measured > T_Recv_start then
    if time > t_cycle*5.0 and time < t_cycle*5.0 + t_charge then
      L_recv_start := L;
    end if;
  end when;
  
  when storage_model.T_top_measured < T_PB_start then
...
equation
  ...
  L = storage_model.Level;
  //der_L = der(L);
  //M = 3600.0*storage_model.Tank_A.m_flow*(h_f_max-h_f_min)/E_max;
  e_top = (storage_model.T_top_measured - T_min)/(T_max-T_min);
  e_bot = (T_max - storage_model.T_bot_measured)/(T_max-T_min);
  
  if storage_model.Tank_A.m_flow > 1e-6 then //discharge
    e_out = (storage_model.T_top_measured - T_min)/(T_max-T_min);
  elseif storage_model.Tank_A.m_flow < -1e-6 then //charge
    e_out = (T_max - storage_model.T_bot_measured)/(T_max-T_min);
  else
    e_out = 0.0;
  end if;

  if time > t_cycle * 5.0 and time < t_cycle * 6.0 then
    der(numerator) = PB_Sink.port_a.m_flow * (storage_model.fluid_top.h - h_f_min);
  else
    der(numerator) = 0.0;
  end if;
  
  T_top_degC = storage_model.T_top_measured - 273.15;
  T_bot_degC = storage_model.T_bot_measured - 273.15;
  if storage_model.Tank_A.m_flow > 1e-3 then
//dicharging
    T_outlet_degC = T_top_degC;
  elseif storage_model.Tank_A.m_flow < (-1e-3) then
//charging
...
//charging
      der(E_charged) = storage_model.fluid_a.m_flow * (inStream(storage_model.fluid_a.h_outflow) - storage_model.fluid_b.h_outflow);
      der(E_discharged) = 0.0;
    elseif time > t_cycle * 5.0 + t_charge and time < t_cycle * 5.0 + t_charge + t_discharge then
      der(E_charged) = 0.0;
      der(E_discharged) = storage_model.fluid_b.m_flow * (storage_model.fluid_a.h_outflow - inStream(storage_model.fluid_b.h_outflow));
    else
      der(E_charged) = 0.0;
      der(E_discharged) = 0.0;
    end if;
    der(E_lost) = storage_model.Tank_A.Q_loss_total;
    der(E_pump) = storage_model.Tank_A.W_loss_pump;
  else
    der(E_charged) = 0.0;
    der(E_discharged) = 0.0;
    der(E_lost) = 0.0;
...
//subtract pumping power
  ...
  connect(storage_model.fluid_b, Splitter2.fluid_c) annotation(
    Line(points = {{0, -32}, {0, -46}}, color = {0, 127, 255}));
  connect(Splitter2.fluid_b, pumpSimple_EqualPressure2.fluid_a) annotation(
  ...
  connect(pumpSimple_EqualPressure3.fluid_b, Splitter2.fluid_a) annotation(
    Line(points = {{30, -60}, {12, -60}}, color = {0, 127, 255}));
  connect(storage_model.T_amb, Tamb.y) annotation(
    Line(points = {{-17, -2}, {-25, -2}}, color = {0, 0, 127}));
  connect(pumpSimple_EqualPressure.fluid_b, Splitter1.fluid_a) annotation(
    Line(points = {{-44, 48}, {-30, 48}, {-30, 92}, {-13, 92}}, color = {0, 127, 255}));
  connect(Splitter1.fluid_b, pumpSimple_EqualPressure1.fluid_a) annotation(
    Line(points = {{13, 92}, {22, 92}, {22, 44}, {34, 44}}, color = {0, 127, 255}));
  connect(storage_model.p_amb, p_amb.y) annotation(
  ...
  connect(mass_loop_breaker.port_b, storage_model.fluid_a) annotation(
    Line(points = {{0, 36}, {0, 28}}, color = {0, 127, 255}));
  connect(Splitter1.fluid_c, mass_loop_breaker.port_a) annotation(
    Line(points = {{0, 78}, {0, 64}}, color = {0, 127, 255}));
  annotation(
    experiment(StopTime = 518400, StartTime = 0, Tolerance = 1e-3, Interval = 60),
    Documentation(info = "<html><head></head><body>This is the model to run a storage component model, and generate data for obtaining the relationship between effectiveness and level of charge, etc. The model runs 6 cycles of chg&amp;dischg until it reaches steady.&nbsp;<div><br></div><div>The model is adapted from MgO_TrainingData.mo developed by Zebedee Kee for MgO thermalcline packed-bed system. &nbsp;</div></body></html>"));

end EffectivenessTrainingData;
