...
model TubeInTank_Section3
  ...
  //Materials used
  replaceable package Fluid = SolarTherm.Materials.Sodium_Table constrainedby SolarTherm.Media.Materials.PartialMaterial;
  replaceable package Storage = SolarTherm.Media.Materials.PartialMaterial;
  replaceable package Wall = SolarTherm.Media.Materials.SS316L "Wall Package";
  Fluid.State fluid[N_sec](each h_start = h_f_min) "Fluid object array";
  Fluid.State fluid_in "Model which calculates properties at top of the section";
  Fluid.State fluid_out "Model which calculates properties at bottom of the section";
  ...
  //Inlet and Outlet
  SI.SpecificEnthalpy h_in(start=h_f_start[1]) "J/kg";
  SI.SpecificEnthalpy h_out(start=h_f_start[N_sec]) "J/kg";
  SI.MassFlowRate m_flow(start=0.0) "kg/s";
  //Boundary Conditions
  SI.Temperature T_in(start = T_min) "Temperature at the top";
  SI.Temperature T_out(start = T_min) "Temperature at the bottom";
...
//HTF Equation
  if State == 1 then
//Charging: HTF is flowing downwards from Node N_sec to Node 1
//Bottom HTF node (node 1)
...
//Top HTF node (node N_sec)   
    m_f[N_sec] * der(h_f[N_sec]) = abs(m_flow) * (h_in - h_f[N_sec]) + 2.0 * CN.pi * dL * (T_w[N_sec] - T_f[N_sec]) / (log(r_Wall / r_tube_in) / k_w[N_sec] + 1.0 / (h_v[N_sec] * r_tube_in)) + k_f[N_sec] * A_HTF * (T_f[N_sec-1] - T_f[N_sec]) / dL;
//End bottom charging HTF node (node N_sec)
  elseif State == 2 then
//Standby
//Top standby HTF node (node 1)
    m_f[1] * der(h_f[1]) = 2.0 * CN.pi * dL * (T_w[1] - T_f[1]) / (log(r_Wall / r_tube_in) / k_w[1]) + k_f[1] * A_HTF * (T_f[2] - T_f[1]) / dL;
    h_out = h_f[1];
//End top standby HTF node (node 1)
//Middle standby HTF nodes (nodes 2 - N_sec-1)
    for i in 2:N_sec-1 loop
      m_f[i] * der(h_f[i]) = 2.0 * CN.pi * dL * (T_w[i] - T_f[i]) / (log(r_Wall / r_tube_in) / k_w[i]) + k_f[i] * A_HTF * (T_f[i+1] - T_f[i]) / dL + k_f[i] * A_HTF * (T_f[i-1] - T_f[i]) / dL;
    end for;
//End middle standby HTF nodes (nodes 2 - N_sec-1)
//Bottom standby HTF node (node N_sec)
    m_f[N_sec] * der(h_f[N_sec]) = 2.0 * CN.pi * dL * (T_w[N_sec] - T_f[N_sec]) / (log(r_Wall / r_tube_in) / k_w[N_sec]) + k_f[N_sec] * A_HTF * (T_f[N_sec-1] - T_f[N_sec]) / dL;

//End bottom standby HTF node (node N_sec)
  else
//Discharging: HTF is flowing upwards from Node 1 to Node N_Sec
//Top discharging HTF node (node 1)
    m_f[1] * der(h_f[1]) = abs(m_flow) * (h_in - h_f[1]) + 2.0 * CN.pi * dL * (T_w[1] - T_f[1]) / (log(r_Wall / r_tube_in) / k_w[1] + 1.0 / (h_v[1] * r_tube_in)) + k_f[1] * A_HTF * (T_f[2] - T_f[1]) / dL;
...
//Bottom discharging HTF node (node N_sec)
    m_f[N_sec] * der(h_f[N_sec]) = abs(m_flow) * (h_f[N_sec-1] - h_f[N_sec]) + 2.0 * CN.pi * dL * (T_w[N_sec] - T_f[N_sec]) / (log(r_Wall / r_tube_in) / k_w[N_sec] + 1.0 / (h_v[N_sec] * r_tube_in)) + k_f[N_sec] * A_HTF * (T_f[N_sec-1] - T_f[1]) / dL;
    h_out = h_f[N_sec];
  ...
  for i in 1:N_sec loop
    ...
    if abs(m_flow) > 1e-6 then
      ...
      if State == 3 then
        ...
        Xplus[i] = (L - z_f[i]) / r_tube_in / Re[i] / Pr[i];
      end if;
      if Re[i] < 2300 then
...
//Pumping losses
  ...
  annotation (Documentation(info ="<html>
        <p>This component contains the same 1D numerical model as documented in the component of TubeInTank_Section. In addition, it enables the HTF entering into the storage tank from opposite directions in charging (when m_flow < 0) and discharging process (when m_flow > 0), respectively. This component is for the training data generation and CSP/CST system simulation. </p>
  		<p>By Ming Liu on 28/06/2021</p>
  		</html>"));  
end TubeInTank_Section3;
