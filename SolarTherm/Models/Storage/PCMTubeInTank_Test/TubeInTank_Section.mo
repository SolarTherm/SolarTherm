...
model TubeInTank_Section
  ...
  replaceable package Storage = SolarTherm.Media.Materials.PCM635_UniSA constrainedby SolarTherm.Media.Materials.PartialMaterial "PCM Package";
...
//  parameter SI.Length r_Wall = ((r_tube_out ^ 2 + r_tube_in ^ 2) / 2) ^ 0.5 "Radius from Wall volume center";
  parameter SI.Length r_PCM = (r_shell + r_tube_out) / 2.0;
  parameter SI.Length r_Wall = (r_tube_out + r_tube_in) / 2.0;
  ...
  parameter SI.SpecificEnthalpy h_w_start[N_sec] = fill(h_w_min, N_sec) "setting up initial specific enthalpy of PCM";
  ...
  SI.Temperature T_f[N_sec](start = T_f_start);
