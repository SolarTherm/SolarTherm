...
model eNTU_Thermocline
  ...
  Modelica.Blocks.Interfaces.RealOutput h_bot_outlet "Enthaply at the bottom of the tank as an output signal (K)"
          ...
          origin = {-40, -70},extent = {{-10, -10}, {10, 10}}, rotation = -90), iconTransformation(origin = {-33, -41}, extent = {{-5, -5}, {5, 5}}, rotation = -90)));
...
equation
  ...
  if fluid_a.m_flow>0 then
    T_out = T_max - eff*(T_max-T_0);
  else
    T_out = T_0 + eff*(T_max-T_0);
  end if;
  //eff=(T_in-T_out)/(T_in-T_st);
  h_bot_outlet = Medium.specificEnthalpy(Medium.setState_pTX(p_amb,T_max - eff*(T_max-T_0)));
