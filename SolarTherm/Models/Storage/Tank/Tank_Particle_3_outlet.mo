...
model Tank_Particle_3_outlet
  ...
  Modelica.Fluid.Interfaces.FluidPort_b fluid_b2(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(extent = {{90, -106}, {110, -86}}, rotation = 0), iconTransformation(extent = {{90, -106}, {110, -86}}, rotation = 0)));
  Modelica.Fluid.Interfaces.FluidPort_b fluid_b3(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(extent = {{90, -56}, {110, -36}}, rotation = 0), iconTransformation(extent = {{90, -56}, {110, -36}}, rotation = 0)));
initial equation
...
equation
  ...
  fluid_b2.h_outflow=medium.h;
  fluid_b3.h_outflow=medium.h;
  
  h_in = inStream(fluid_a.h_outflow);
  
  der(m)=fluid_a.m_flow+fluid_b.m_flow+fluid_b2.m_flow + fluid_b3.m_flow;
  ...
  m*der(medium.h)+der(m)*medium.h= //net gain
                  fluid_a.m_flow * inStream(fluid_a.h_outflow) +
                      fluid_b.m_flow * medium.h + fluid_b2.m_flow * medium.h + fluid_b3.m_flow * medium.h -
                            Q_loss_radiation - 
