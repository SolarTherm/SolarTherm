...
model Annular_Storage_Section_VD_Final
  ...
  SI.MassFlowRate m_flow[N_f] "kg/s";
  ...
  parameter SI.Area A_px[N_p] = Annular.Annular_CSA(r_pipe, r_solid, N_p);
  parameter SI.Length r_pj[N_p] = Annular.Annular_Radii(r_pipe, r_solid, N_p);
...
algorithm
//Fluid equations
  if State == 1 then //m_flow is negative. Fluid flows right to left
    der_rho_f_h_f[1] := 
    (-2.0 * k_f[1] * k_f[2]) * (T_f[1] - T_f[2]) / ((k_f[1] + k_f[2]) * dx * dx) 
    + (rho_f[1]*u_flow[1]*h_f[1]-rho_f[2]*u_flow[2]*h_f[2])/dx 
    - h_c[1] * (T_f[1] - T_s[1]) * (A_fr / dV_f);
    
    h_out := h_f[1];
    der_rho_f[1] := (rho_f[1]*u_flow[1] - rho_f[2]*u_flow[2])/dx;
    m_flow_out := rho_f[1]*u_flow[1]*A_fx;
    der_h_f[1] := (der_rho_f_h_f[1] - h_f[1]*der_rho_f[1])/rho_f[1];
    
    for i in 2:N_f - 1 loop
      der_rho_f_h_f[i] := 
      (2.0 * k_f[i - 1] * k_f[i]) * (T_f[i - 1] - T_f[i]) / ((k_f[i - 1] + k_f[i]) * dx * dx) 
      - 2.0 * k_f[i] * k_f[i + 1] * (T_f[i] - T_f[i + 1]) / ((k_f[i] + k_f[i + 1]) * dx * dx)
      + (rho_f[i]*u_flow[i]*h_f[i]-rho_f[i+1]*u_flow[i+1]*h_f[i+1])/dx
      - h_c[i] * (T_f[i] - T_s[i]) * (A_fr / dV_f);
      
      der_rho_f[i] := (rho_f[i]*u_flow[i] - rho_f[i+1]*u_flow[i+1])/dx;
      der_h_f[i] := (der_rho_f_h_f[i] - h_f[i]*der_rho_f[i])/rho_f[i];
      
    end for;
    der_rho_f_h_f[N_f] := 
    (2.0 * k_f[N_f - 1] * k_f[N_f]) * (T_f[N_f - 1] - T_f[N_f]) / ((k_f[N_f - 1] + k_f[N_f]) * dx * dx) 
    + (rho_f[N_f]*u_flow[N_f]*h_f[N_f]-rho_in*u_in*h_in)/dx
    - h_c[N_f] * (T_f[N_f] - T_s[N_f]) * (A_fr / dV_f);
    
    der_rho_f[N_f] := (rho_f[N_f]*u_flow[N_f] - rho_in*u_in)/dx;
    der_h_f[N_f] := (der_rho_f_h_f[N_f] - h_f[N_f]*der_rho_f[N_f])/rho_f[N_f];
    
    
  else //m_flow is positive. Fluid flows from right to left
    der_rho_f_h_f[1] := 
    (-2.0 * k_f[1] * k_f[2] * (T_f[1] - T_f[2]) / ((k_f[1] + k_f[2]) * dx * dx)) 
    + (rho_in*u_in*h_in-rho_f[1]*u_flow[1]*h_f[1])/dx 
    - h_c[1] * (T_f[1] - T_s[1]) * (A_fr / dV_f);
    
    der_rho_f[1] := (rho_in*u_in - rho_f[1]*u_flow[1])/dx;
    der_h_f[1] := (der_rho_f_h_f[1] - h_f[1]*der_rho_f[1])/rho_f[1];
    ...
    for i in 2:N_f - 1 loop
      ...
      (2.0 * k_f[i - 1] * k_f[i]) * (T_f[i - 1] - T_f[i]) / ((k_f[i - 1] + k_f[i]) * dx * dx) 
      - 2.0 * k_f[i] * k_f[i + 1] * (T_f[i] - T_f[i + 1]) / ((k_f[i] + k_f[i + 1]) * dx * dx) 
      + (rho_f[i-1]*u_flow[i-1]*h_f[i-1]-rho_f[i]*u_flow[i]*h_f[i])/dx
      - h_c[i] * (T_f[i] - T_s[i]) * (A_fr / dV_f);
      
      der_rho_f[i] := (rho_f[i-1]*u_flow[i-1] - rho_f[i]*u_flow[i])/dx;
      der_h_f[i] := (der_rho_f_h_f[i] - h_f[i]*der_rho_f[i])/rho_f[i];
      
    end for;
    der_rho_f_h_f[N_f] := 
    (2.0 * k_f[N_f - 1] * k_f[N_f]) * (T_f[N_f - 1] - T_f[N_f]) / ((k_f[N_f - 1] + k_f[N_f]) * dx * dx) 
    + (rho_f[N_f-1]*u_flow[N_f-1]*h_f[N_f-1]-rho_f[N_f]*u_flow[N_f]*h_f[N_f])/dx 
    - h_c[N_f] * (T_f[N_f] - T_s[N_f]) * (A_fr / dV_f);
    
    der_rho_f[N_f] := (rho_f[N_f - 1] * u_flow[N_f - 1] - rho_f[N_f] * u_flow[N_f]) / dx;
    m_flow_out := rho_f[N_f]*u_flow[N_f]*A_fx;
    h_out := h_f[N_f];
...
//k_eff[i] = eta*fluid[i].k; //Effective thermal conductivity of fluid (weighted by porosity)
    ...
    m_flow[i] = rho_f[i]*u_flow[i]*A_fx;
...
//Use outermost shell conductivity
    Pe[i] = Re[i] * Pr[i];
    Pr[i] = max(1.0e-6,c_pf[i] * mu_f[i] / k_f[i]); //Safeguard Prandtl to be positive
    Re[i] = max(10.0, rho_f[i] * D_pipe * abs(u_flow[i]) / mu_f[i]); //Safeguard Reynolds to a minimum of 10.
    ...
    elseif Re[i] >= 4000 then //Turbulent
      ...
      if Correlation == 1 then //Liquids
        ...
        Nu[i] = 0.023*(Re[i]^0.8)*(Pr[i]^0.4)*((T_s[i]/T_f[i])^(-0.4));
      end if;
    else
    ...
    if Re[i] <= 2000.0 then
//Laminar
      f_fric[i] = 64.0 / Re[i] + SolarTherm.Utilities.Phis((Re[i] - 2.0e3) / 2.0e3) * ((0.790 * log(Re[i]) - 1.64) ^ (-2.0) - 64.0 / Re[i]);
//just avoid divison by zero. f_fric does not affect Nu
      Nu[i] = 3.66;
    elseif Re[i] >= 4000.0 then
//Turbulent
      f_fric[i] = (0.790 * log(Re[i]) - 1.64) ^ (-2.0);
      Nu[i] = f_fric[i] / 8.0 * (Re[i] - 1000.0) * Pr[i] / (1.0 + 12.7 * (f_fric[i] / 8.0) ^ 0.5 * (Pr[i] ^ 0.66 - 1.0));
    else
//transition
      f_fric[i] = 64.0 / Re[i] + SolarTherm.Utilities.Phis((Re[i] - 2.0e3) / 2.0e3) * ((0.790 * log(Re[i]) - 1.64) ^ (-2.0) - 64.0 / Re[i]);
      Nu[i] = 3.66 + SolarTherm.Utilities.Phis((Re[i] - 2.0e3) / 2.0e3) * (f_fric[i] / 8.0 * (Re[i] - 1000.0) * Pr[i] / (1.0 + 12.7 * (f_fric[i] / 8.0) ^ 0.5 * (Pr[i] ^ 0.66 - 1.0)) - 3.66);
    end if;
    */
/*
...
//Outermost solid annulus
    der(h_p[i, N_p]) = 
    (U_in[i, N_p] * (T_p[i, N_p - 1] - T_p[i, N_p]) 
    + U_right[i - 1, N_p] * (T_p[i - 1, N_p] - T_p[i, N_p]) 
    - U_right[i, N_p] * (T_p[i, N_p] - T_p[i + 1, N_p])
    - U_wall*(T_p[i,N_p]-T_amb)*CN.pi*D_solid*dx ) / m_pj[N_p];
  end for;
//Left end of the pipe i = 1
//Innermost solid annulus
  der(h_p[1, 1]) =
  (U_in[1, 1] * (T_s[1] - T_p[1, 1]) 
  - U_in[1, 2] * (T_p[1, 1] - T_p[1, 2]) 
  - U_right[1, 1] * (T_p[1, 1] - T_p[2, 1])
...
//Outermost solid annulus
  der(h_p[1, N_p]) = 
  (U_in[1, N_p] * (T_p[1, N_p - 1] - T_p[1, N_p]) 
  - U_right[1, N_p] * (T_p[1, N_p] - T_p[2, N_p])
  - U_wall*(T_p[1,N_p]-T_amb)*CN.pi*D_solid*dx
  - U_side*(T_p[1,N_p]-T_amb)*A_px[N_p] ) / m_pj[N_p];
//Right end of the pipe i = N_f
//Innermost solid annulus
  der(h_p[N_f, 1]) = 
  (U_in[N_f, 1] * (T_s[N_f] - T_p[N_f, 1]) 
  - U_in[N_f, 2] * (T_p[N_f, 1] - T_p[N_f, 2]) 
  + U_right[N_f - 1, 1] * (T_p[N_f - 1, 1] - T_p[N_f, 1])
  - U_side*(T_p[N_f,1]-T_amb)*A_px[1] ) / m_pj[1];
//Middle solid annuli
  for j in 2:N_p - 1 loop
    der(h_p[N_f, j]) = 
    (U_in[N_f, j] * (T_p[N_f, j - 1] - T_p[N_f, j]) 
    - U_in[N_f, j + 1] * (T_p[N_f, j] - T_p[N_f, j + 1]) 
    + U_right[N_f - 1, j] * (T_p[N_f - 1, j] - T_p[N_f, j])
    - U_side*(T_p[N_f,j]-T_amb)*A_px[j] ) / m_pj[j];
  end for;
//Outermost solid annulus
  der(h_p[N_f, N_p]) = 
  (U_in[N_f, N_p] * (T_p[N_f, N_p - 1] - T_p[N_f, N_p]) 
  + U_right[N_f - 1, N_p] * (T_p[N_f - 1, N_p] - T_p[N_f, N_p])
  - U_wall*(T_p[N_f,N_p]-T_amb)*CN.pi*D_solid*dx
  - U_side*(T_p[N_f,N_p]-T_amb)*A_px[N_p] ) / m_pj[N_p];
...
//Analyics
  der(E_stored) = (abs(m_flow_in)*h_in) - (abs(m_flow_out)*h_out);
  Level = E_stored / E_unit;
  if m_flow_in > 1.0e-3 then
//Discharging, outlet is the top
    T_outlet_degC = T_f[N_f] - 273.15;
  elseif m_flow_in < (-1.0e-3) then
...
end Annular_Storage_Section_VD_Final;