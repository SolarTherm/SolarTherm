...
model Thermocline_Spheres_Section_Final_keff
...
equation
  ...
  for i in 1:N_f loop
    ...
    if abs(m_flow) > 1.0e-3 then//regular operation
      k_f[i] = fluid[i].k;
    else//Standby
      k_f[i] = fluid[i].k + ((1.0-eta)/eta)*k_p[i,N_p]; //Effective thermal conductivity of fluid (weighted by porosity)
    end if;
    mu_f[i] = fluid[i].mu;
...
end Thermocline_Spheres_Section_Final_keff;