...
model Thermocline_Spheres_Parallel_A2_v2_Final
  //A2 Refers to A: Charge 1-2, Discharge 2-1; 2: 2-Tanks
  //v2 Implements partial flow where a percentage of mass flow to the Active tank can be diverted to the next one.
  ...
  //v2 additions
  Boolean Partial_Flow(start = false) "Is the tank flow rate diverted?";
...
equation
  ...
  if Partial_Flow == false then
    ...
    if fluid_a.m_flow > 0.0 then //Charging
      ...
      else //Then Active_Tank ==  2 //Cannot charge B partially coz C does not exist
        Tank_B.m_flow = -1.0 * fluid_a.m_flow;
        Tank_B.h_in = inStream(fluid_a.h_outflow);
        fluid_a.h_outflow = Tank_B.h_in;
        fluid_b.h_outflow = Tank_B.h_out;
...
end Thermocline_Spheres_Parallel_A2_v2_Final;