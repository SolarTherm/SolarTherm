...
model Thermocline_Spheres_2P_MixedFlow_Strat1
  //2P = 2 tanks in parallel, MixedFlow strategy, Strategy 1: Tank B always assists Tank A.
...
algorithm
  when Tank_A.T_f[1] > T_recv_set then
    Charge_State := 2;
  elsewhen Tank_A.T_f[1] < T_recv_set - 1 then
    Charge_State := 1;
  end when;
...
equation
  ...
  //Figure out assisted mass fraction
    ...
    if Tank_B.h_f[1] < Tank_A.h_f[1] - 10.0 then
      f_chg_guess = (h_recv_set - Tank_A.h_f[1]) / (Tank_B.h_f[1] - Tank_A.h_f[1] );
    else
      f_chg_guess = 0.0;
    end if;
...
end Thermocline_Spheres_2P_MixedFlow_Strat1;