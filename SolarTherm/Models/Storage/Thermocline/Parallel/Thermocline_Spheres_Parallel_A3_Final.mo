...
model Thermocline_Spheres_Parallel_A3_Final
  ...
  parameter Integer N_f_C = 20 "Number of fluid CVs in Tank_B";
  parameter Integer N_p_C = 5 "Number of filler CVs in Tank_B";
...
algorithm
  ...
  when T_05_B > T_bot_high then
    if Active_Tank == 2 then
      Active_Tank := 3;
    end if;
  end when;
  when T_95_C < T_top_low then
