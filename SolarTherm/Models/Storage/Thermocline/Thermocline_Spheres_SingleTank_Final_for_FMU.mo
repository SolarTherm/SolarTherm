...
model Thermocline_Spheres_SingleTank_Final_for_FMU
...
equation
  ...
  //Mass balance
  //fluid_a.m_flow = -1.0*fluid_b.m_flow; //Commented since this eq makes FMU generation impossible?
  ...
  if fluid_a.m_flow > 0.0 then //mass is flowing into the top, direction is downwards so Tank_A.m_flow is (negative), charging
    //Tank_A.m_flow = -1.0*fluid_a.m_flow --> MOVED OUTSIDE THE IF-ELSE;
    
    Tank_A.h_in = inStream(fluid_a.h_outflow);
    
    fluid_b.h_outflow = Tank_A.h_f[1];//Tank_A.h_out;//Tank_A.h_f[1];//fluid_bot.h; 
    fluid_a.h_outflow = Tank_A.h_f[N_f];//Tank_A.h_in;//Tank_A.h_f[N_f];//fluid_top.h;
  else //discharging
    //Tank_A.m_flow = -1.0*fluid_a.m_flow --> MOVED OUTSIDE THE IF-ELSE;
    
    Tank_A.h_in = inStream(fluid_b.h_outflow);
    
    fluid_a.h_outflow = Tank_A.h_f[N_f];//Tank_A.h_out;//Tank_A.h_f[N_f];//fluid_top.h;
    fluid_b.h_outflow = Tank_A.h_f[1];//Tank_A.h_in;//Tank_A.h_f[1];//fluid_bot.h;
  end if;
...
end Thermocline_Spheres_SingleTank_Final_for_FMU;