...
model Thermocline_Spheres_ModularSection_Bot_Final
  ...
  //Modular Section Inputs/Outputs
  //Inputs
  SI.ThermalConductivity k_a "Input k_f of fluid slice immediately ABOVE this tank";
  SI.Temperature T_a "Input T_f for fluid slice immediately ABOVE this tank";
  //Outputs
  SI.ThermalConductivity k_Nf = k_f[N_f] "Output k_f[1] of this tank, for the tank below this one";
  SI.Temperature T_Nf = T_f[N_f] "Output T_f[1] of this tank";
  
  SI.HeatFlowRate Q_diffuse_top "Heat rate flowing away from the top node";
protected  
...
algorithm
  Q_diffuse_top := 2.0*k_f[N_f]*k_a*(T_f[N_f]-T_a)/((k_f[N_f]+k_a)*dz*dz);
...
equation
  ...
  //Connect diffusion to the module above this one, remove top surface heat loss
...
end Thermocline_Spheres_ModularSection_Bot_Final;