...
model Thermocline_Spheres_ModularTank_Bot_Final
  ...
  //ModularTank Inputs and Outputs
    Modelica.Blocks.Interfaces.RealOutput k_Nf = Tank_A.k_f[N_f] "Bottom node fluid thermal conductivity"
                                          annotation (Placement(visible = true,transformation(
          extent = {{40, 50}, {60, 70}}, rotation = 0), iconTransformation(origin = {-30, 64}, extent = {{-4, -4}, {4, 4}}, rotation = 90)));
          
    Modelica.Blocks.Interfaces.RealOutput T_Nf = Tank_A.T_f[N_f] "Bottom node fluid temperature"
                                          annotation (Placement(visible = true,transformation(
          extent = {{40, 50}, {60, 70}}, rotation = 0), iconTransformation(origin = {-20, 64}, extent = {{-4, -4}, {4, 4}}, rotation = 90)));
    
    Modelica.Blocks.Interfaces.RealInput k_a "Fluid thermal conductivity of the immediately tank ABOVE this one" annotation (Placement(
        visible = true,transformation(
    ...
    Modelica.Blocks.Interfaces.RealInput T_a "Fluid temperature of the immediately tank ABOVE this one" annotation (Placement(
        visible = true,transformation(
...
annotation(
    ...
    Icon(graphics = {Text(origin = {0, 1}, extent = {{-14, -21}, {14, 21}}, textString = "Bot"), Line(origin = {-40, 75}, points = {{0, -15}, {0, 15}}, pattern = LinePattern.DashDot), Line(origin = {39.9254, 74.6866}, points = {{0, -15}, {0, 15}}, pattern = LinePattern.DashDot)}, coordinateSystem(initialScale = 0.1)));
end Thermocline_Spheres_ModularTank_Bot_Final;