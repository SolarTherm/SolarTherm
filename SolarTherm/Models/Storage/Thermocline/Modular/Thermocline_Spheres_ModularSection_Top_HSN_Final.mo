...
model Thermocline_Spheres_ModularSection_Top_HSN_Final
  ...
  parameter SI.Density rho_e = min(rho_e_min,rho_e_max) "kg/m3";
    ...
    //Discretization
  parameter SI.Length dz = H_tank / (N_f - 1) "discretization vertical length of fluid";
  ...
  parameter SI.Length z_f[N_f] = Z_position_HSN(H_tank, N_f) .+ z_offset;
  SI.Temperature T_f[N_f] "(K)";
  SI.SpecificEnthalpy h_f[N_f](start = h_f_start) "J/kg";

  //Plotting
  parameter Real ZDH[N_f] = Relative_Tank_Axes_HSN(H_tank, N_f) "Non-dimensional tank vertical axis";
...
equation
  ...
  //Bottom Charging Fluid Node, Ignore bottom heat loss
    0.5*rho_f_avg * der(h_f[1]) =
    (-2.0*k_f[1]*k_f[2])*(T_f[1]-T_f[2])/((k_f[1]+k_f[2])*dz*dz)
    + (rho_f_avg*u_flow)*(h_f[1]-h_f[2])/dz
    - 0.5*h_v[1]*(T_f[1] - T_s[1])/eta
    //- U_bot*(T_f[1]-T_amb)/(eta*dz) 
    - 0.5*U_wall*CN.pi*D_tank*(T_f[1]-T_amb)/(eta*A);
  ...
  p_drop[1] = dz*(((600*((1-eta)^2)*mu_f[1]*abs(m_flow))/((eta^3)*(d_p^2)*rho_f_avg*CN.pi*(D_tank^2)))+((28*(1-eta)*(m_flow^2))/((eta^3)*d_p*rho_f_avg*CN.pi*CN.pi*(D_tank^4))));
  
  p_drop[N_f] = dz*(((600*((1-eta)^2)*mu_f[N_f]*abs(m_flow))/((eta^3)*(d_p^2)*rho_f_avg*CN.pi*(D_tank^2)))+((28*(1-eta)*(m_flow^2))/((eta^3)*d_p*rho_f_avg*CN.pi*CN.pi*(D_tank^4))));
...
end Thermocline_Spheres_ModularSection_Top_HSN_Final;