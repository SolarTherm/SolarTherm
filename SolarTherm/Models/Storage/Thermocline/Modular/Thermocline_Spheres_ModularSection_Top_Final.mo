...
model Thermocline_Spheres_ModularSection_Top_Final
  ...
  //Inputs
  SI.ThermalConductivity k_b "Input k_f of fluid slice immediately BELOW this tank";
  SI.Temperature T_b "Input T_f for fluid slice immediately BELOW this tank";
  //Outputs
  SI.ThermalConductivity k_1 = k_f[1] "Output k_f[1] of this tank, for the tank below this one";
  SI.Temperature T_1 = T_f[1] "Output T_f[1] of this tank";
  
  SI.HeatFlowRate Q_diffuse_bot "Heat rate flowing away from the bottom node";
protected  
...
algorithm
  Q_diffuse_bot := -1.0*(2.0*k_b*k_f[1]*(T_b-T_f[1])/((k_b+k_f[1])*dz*dz));
initial equation
  for i in 1:N_f loop
...
equation
  ...
  //Connect diffusion to the module below this one, remove bottom surface heat loss
    rho_f_avg * der(h_f[1]) =
    - Q_diffuse_bot
...
end Thermocline_Spheres_ModularSection_Top_Final;