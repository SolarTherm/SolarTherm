...
model Annular_Storage_Section_Final
...
algorithm
  ...
  //Bottom Charging Fluid Node
    ...
    + (rho_f_avg*u_flow)*(h_f[1]-h_f[2])/dx
  ...
  //Middle Charging Fluid Nodes
    for i in 2:N_f - 1 loop
      ...
      + (rho_f_avg*u_flow)*(h_f[i]-h_f[i+1])/dx
  ...
  //Top Charging Fluid Node
    ...
    + (rho_f_avg*u_flow)*(h_f[N_f]-h_in)/dx
  ...
  //Bottom Discharge Node
    der_h_f[1] :=
    (-2.0*k_f[1]*k_f[2]*(T_f[1]-T_f[2])/((k_f[1]+k_f[2])*dx*dx)
    + (rho_f_avg*u_flow)*(h_in-h_f[1])/dx
    - h_c[1]*(T_f[1]-T_s[1])*(A_fr/dV_f)
    )/ (rho_f_avg);
  ...
  //Middle Discharge Nodes
    for i in 2:N_f - 1 loop
      ...
      ( 2.0*k_f[i-1]*k_f[i]*(T_f[i-1]-T_f[i])/((k_f[i-1]+k_f[i])*dx*dx)
      - 2.0*k_f[i]*k_f[i + 1]*(T_f[i]-T_f[i+1])/((k_f[i]+k_f[i+1])*dx*dx)
      + (rho_f_avg*u_flow)*(h_f[i-1]-h_f[i])/dx
      - h_c[i]*(T_f[i]-T_s[i])*(A_fr/dV_f)
      ) / (rho_f_avg);
    end for;
  ...
  //Top Discharge Node
    der_h_f[N_f] :=
    ( 2.0*k_f[N_f-1]*k_f[N_f]*(T_f[N_f-1]-T_f[N_f])/((k_f[N_f-1]+k_f[N_f])*dx*dx)
    + (rho_f_avg*u_flow)*(h_f[N_f-1]-h_f[N_f])/dx
    - h_c[N_f]*(T_f[N_f]-T_s[N_f])*(A_fr/dV_f)
    ) / (rho_f_avg);
...
equation
  ...
  u_flow = m_flow / (rho_f_avg * A_fx); //positive if flowing upwards (discharge)
  ...
  for i in 1:N_f loop
    ...
    //
    U_in[i,1] * (T_s[i] - T_p[i,1]) = h_c[i]*A_fr*(T_f[i] - T_s[i]); //reconciles fluid and filler heat transfer
  end for;
  //Axial Conductance
  for j in 1:N_p loop
  ...
  for i in 2:N_f-1 loop //not the ends of the pipe
    ...
    der(h_p[i,N_p]) = (U_in[i,N_p]*(T_p[i,N_p-1]-T_p[i,N_p]) + U_right[i-1,N_p]*(T_p[i-1,N_p]-T_p[i,N_p]) - U_right[i,N_p]*(T_p[i,N_p]-T_p[i+1,N_p]) ) / m_pj[N_p];
  end for;
  
  //Left end of the pipe i = 1
  //Innermost solid annulus
  der(h_p[1,1]) = (U_in[1,1]*(T_s[1]-T_p[1,1]) - U_in[1,2]*(T_p[1,1]-T_p[1,2]) - U_right[1,1]*(T_p[1,1]-T_p[2,1]) ) / m_pj[1];
    //Middle solid annuli
    for j in 2:N_p - 1 loop
      der(h_p[1,j]) = (U_in[1,j]*(T_p[1,j-1]-T_p[1,j]) - U_in[1,j+1]*(T_p[1,j]-T_p[1,j+1]) - U_right[1,j]*(T_p[1,j]-T_p[2,j]) ) / m_pj[j];
    end for;
    //Outermost solid annulus
    der(h_p[1,N_p]) = (U_in[1,N_p]*(T_p[1,N_p-1]-T_p[1,N_p]) - U_right[1,N_p]*(T_p[1,N_p]-T_p[2,N_p]) ) / m_pj[N_p];
    
  //Right end of the pipe i = N_f
  //Innermost solid annulus
    der(h_p[N_f,1]) = (U_in[N_f,1]*(T_s[N_f]-T_p[N_f,1]) - U_in[N_f,2]*(T_p[N_f,1]-T_p[N_f,2]) + U_right[N_f-1,1]*(T_p[N_f-1,1]-T_p[N_f,1]) ) / m_pj[1];
    //Middle solid annuli
    for j in 2:N_p - 1 loop
      der(h_p[N_f,j]) = (U_in[N_f,j]*(T_p[N_f,j-1]-T_p[N_f,j]) - U_in[N_f,j+1]*(T_p[N_f,j]-T_p[N_f,j+1]) + U_right[N_f-1,j]*(T_p[N_f-1,j]-T_p[N_f,j])  ) / m_pj[j];
    end for;
    //Outermost solid annulus
    der(h_p[N_f,N_p]) = (U_in[N_f,N_p]*(T_p[N_f,N_p-1]-T_p[N_f,N_p]) + U_right[N_f-1,N_p]*(T_p[N_f-1,N_p]-T_p[N_f,N_p]) ) / m_pj[N_p];
...
end Annular_Storage_Section_Final;