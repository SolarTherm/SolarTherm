...
model Thermocline_Spheres_Section_VDLC_Final
  ...
  parameter Integer N_p = 1 "Number of finite volume elements in filler, including encapsulation";
...
equation
  ...
  */
  ...
  for i in 1:N_f loop
    ...
    //encapsulation[i].h = h_p[i];
    //T_p[i] = encapsulation[i].T;
    //k_p[i] = encapsulation[i].k;
    //f_p[i] = encapsulation[i].f;
  end for;
...
end Thermocline_Spheres_Section_VDLC_Final;