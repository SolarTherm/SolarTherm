...
model SOCFinder
  ...
  //L1
  //Logged values
  Real SOC_stop_charging(start = 0.0);
  //L4
  Real SOC_start_charging(start = 0.0);
  //L3
  Real SOC_start_discharging(start = 0.0);
  //L2
  Real SOC_stop_discharging(start = 0.0);
  //L1
  Real t_stop_charge(start = 0.0);
  Real t_stop_discharge(start = 0.0);
  parameter SI.Power P_gross = 120.0e6 "Nameplate power block";
  parameter Real eff_PB = 0.5 "Power block heat to electricity conversion efficiency";
  //Design Parameters
  ...
  parameter SI.Energy E_max = t_storage *3600* (P_gross / eff_PB) "Storage capacity (J), t_discharge(s), 100MWe, 50% PB efficiency";
  ...
  parameter SI.Time t_standby = max(0, 24.0 * 3600.0 - t_charge - t_discharge) "Standby period between discharge and charge";
  //Optimise
  ...
  /*
    ...
    */
  SolarTherm.Models.Storage.Thermocline.Thermocline_Spheres_SingleTank_Final thermocline_Tank(redeclare package Medium = Medium, redeclare package Fluid_Package = Fluid_Package, redeclare package Filler_Package = Filler_Package, N_f = N_f, N_p = N_p, T_max = T_max, T_min = T_min, E_max = E_max, ar = ar, eta = eta, d_p = d_p, U_loss_tank = U_loss_tank, Correlation = Correlation,E_init=0) annotation(
    Placement(visible = true, transformation(origin = {0, -2}, extent = {{-38, -38}, {38, 38}}, rotation = 0)));
...
//stop charge
//Log this level
//Log stop charge time
  when thermocline_Tank.T_top_measured < T_PB_min then
    m_PB_signal := 0.0;
    if time > t_cycle * 5.0 then
      SOC_stop_discharging := L;
      t_stop_discharge := time;
    end if;
  end when;
//stop discharge
//Log this level
...
//subtract pumping power
  ...
  annotation(
    experiment(StopTime = 518400, StartTime = 0, Tolerance = 0.001, Interval = 300));
end SOCFinder;