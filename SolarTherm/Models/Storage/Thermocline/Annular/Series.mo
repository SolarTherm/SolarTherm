...
package Series
  model Annular_Storage_SGroup3_SM
    ...
    Modelica.Blocks.Interfaces.RealOutput T_p_top_measured = Tank_C.T_s[N_f] "Temperature of the innermost solid element at the the hot-end of the TES (K)"
                                            annotation (Placement(visible = true,transformation(
    ...
    Real T_f_degC[N_f+N_f+N_f](start=fill(T_min,N_f+N_f+N_f));
  ...
  equation
    ...
    if fluid_a.m_flow > 0.0 then //mass is flowing into the top, direction is downwards so Tank_A.m_flow is (negative), charging
      Tank_C.m_flow = -1.0*fluid_a.m_flow/Multiplier;
      Tank_C.h_in = inStream(fluid_a.h_outflow);
      fluid_a.h_outflow = Tank_C.h_in;//Tank_A.h_f[N_f];//fluid_top.h;
      ...
      fluid_a.h_outflow = Tank_C.h_out;//Tank_A.h_f[N_f];//fluid_top.h;
    ...
    T_amb = Tank_B.T_amb;
    T_amb = Tank_C.T_amb;
    T_top_measured = Tank_C.T_f[N_f];// 0.5*(Tank_A.T_f[N_f]+Tank_A.T_s[N_f]);//Tank_A.T_f[N_f];;Tank_A.T_p[N_f,1];
