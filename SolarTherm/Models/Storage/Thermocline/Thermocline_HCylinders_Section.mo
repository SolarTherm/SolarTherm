...
model Thermocline_HCylinders_Section
  ...
  //Correlation used
  parameter Integer Correlation = 8 "1=WakaoKaguei, 2=MelissariArgyropolus, 3=Conservative, 4=Bellan, 5=Laminar, 6=Laminar+Turbulent, 7 = Nie, 8 = Nu[i] = (Re[i] * Pr[i]) ^ 0.5";
  //Design parameters
...
protected
  ...
  //Mixed thermal conductivity used for heat conduction between fluid control volumes!
...
//Bottom Charging Fluid Node
    rho_f_avg * der(h_f[1]) = (-2.0 * k_eff[1] * k_eff[2]) * (T_f[1] - T_f[2]) / ((k_eff[1] + k_eff[2]) * dz * dz * eta) + rho_f_avg * u_flow * (h_f[1] - h_f[2]) / dz - h_v[1] * (T_f[1] - T_s[1]) / eta - U_bot * CN.pi * D_tank * D_tank * 0.25 * (T_f[1] - T_amb) / (eta * A * dz) - U_wall * CN.pi * D_tank * (T_f[1] - T_amb) / (eta * A);
    h_bot = h_f[1];
//End Bottom Charging Fluid Node
//Middle Charging Fluid Nodes
    for i in 2:N_f - 1 loop
      rho_f_avg * der(h_f[i]) = 2.0 * k_eff[i - 1] * k_eff[i] * (T_f[i - 1] - T_f[i]) / ((k_eff[i - 1] + k_eff[i]) * dz * dz * eta) - 2.0 * k_eff[i] * k_eff[i + 1] * (T_f[i] - T_f[i + 1]) / ((k_eff[i] + k_eff[i + 1]) * dz * dz * eta) + rho_f_avg * u_flow * (h_f[i] - h_f[i + 1]) / dz - h_v[i] * (T_f[i] - T_s[i]) / eta - U_wall * CN.pi * D_tank * (T_f[i] - T_amb) / (eta * A);
    end for;
//End Middle Charging Fluid Nodes
//Top Charging Fluid Node
    rho_f_avg * der(h_f[N_f]) = 2.0 * k_eff[N_f - 1] * k_eff[N_f] * (T_f[N_f - 1] - T_f[N_f]) / ((k_eff[N_f - 1] + k_eff[N_f]) * dz * dz * eta) + rho_f_avg * u_flow * (h_f[N_f] - h_top) / dz - h_v[N_f] * (T_f[N_f] - T_s[N_f]) / eta - U_wall * CN.pi * D_tank * (T_f[N_f] - T_amb) / (eta * A) - U_top * CN.pi * D_tank * D_tank * 0.25 * (T_f[N_f] - T_amb) / (eta * A * dz);
//End Top Charging Fluid Node
  elseif State == 2 then
//Standby
//Bottom Standby Fluid Node
    rho_f_avg * der(h_f[1]) = (-2.0 * k_eff[1] * k_eff[2] * (T_f[1] - T_f[2]) / ((k_eff[1] + k_eff[2]) * dz * dz * eta)) - h_v[1] * (T_f[1] - T_s[1]) / eta - U_bot * CN.pi * D_tank * D_tank * 0.25 * (T_f[1] - T_amb) / (eta * A * dz) - U_wall * CN.pi * D_tank * (T_f[1] - T_amb) / (eta * A);
//End Bottom Standby Fluid Node
//Middle Standby Fluid Nodes
    for i in 2:N_f - 1 loop
      rho_f_avg * der(h_f[i]) = 2.0 * k_eff[i - 1] * k_eff[i] * (T_f[i - 1] - T_f[i]) / ((k_eff[i - 1] + k_eff[i]) * dz * dz * eta) - 2.0 * k_eff[i] * k_eff[i + 1] * (T_f[i] - T_f[i + 1]) / ((k_eff[i] + k_eff[i + 1]) * dz * dz * eta) - h_v[i] * (T_f[i] - T_s[i]) / eta - U_wall * CN.pi * D_tank * (T_f[i] - T_amb) / (eta * A);
    end for;
//End Middle Standby Fluid Nodes
//Top Standby Fluid Node
    rho_f_avg * der(h_f[N_f]) = 2.0 * k_eff[N_f - 1] * k_eff[N_f] * (T_f[N_f - 1] - T_f[N_f]) / ((k_eff[N_f - 1] + k_eff[N_f]) * dz * dz * eta) - h_v[N_f] * (T_f[N_f] - T_s[N_f]) / eta - U_wall * CN.pi * D_tank * (T_f[N_f] - T_amb) / (eta * A) - U_top * CN.pi * D_tank * D_tank * 0.25 * (T_f[N_f] - T_amb) / (eta * A * dz);
    h_top = h_f[N_f];
//End Top Standby Fluid Node
  else
//Discharge
//Bottom Discharge Node
    rho_f_avg * der(h_f[1]) = (-2.0 * k_eff[1] * k_eff[2] * (T_f[1] - T_f[2]) / ((k_eff[1] + k_eff[2]) * dz * dz * eta)) + rho_f_avg * u_flow * (h_bot - h_f[1]) / dz - h_v[1] * (T_f[1] - T_s[1]) / eta - U_bot * CN.pi * D_tank * D_tank * 0.25 * (T_f[1] - T_amb) / (eta * A * dz) - U_wall * CN.pi * D_tank * (T_f[1] - T_amb) / (eta * A);
//End Bottom Discharge Node
//Middle Discharge Nodes
    for i in 2:N_f - 1 loop
      rho_f_avg * der(h_f[i]) = 2.0 * k_eff[i - 1] * k_eff[i] * (T_f[i - 1] - T_f[i]) / ((k_eff[i - 1] + k_eff[i]) * dz * dz * eta) - 2.0 * k_eff[i] * k_eff[i + 1] * (T_f[i] - T_f[i + 1]) / ((k_eff[i] + k_eff[i + 1]) * dz * dz * eta) + rho_f_avg * u_flow * (h_f[i - 1] - h_f[i]) / dz - h_v[i] * (T_f[i] - T_s[i]) / eta - U_wall * CN.pi * D_tank * (T_f[i] - T_amb) / (eta * A);
    end for;
//End Middle Discharge Nodes
//Top Discharge Node
    rho_f_avg * der(h_f[N_f]) = 2.0 * k_eff[N_f - 1] * k_eff[N_f] * (T_f[N_f - 1] - T_f[N_f]) / ((k_eff[N_f - 1] + k_eff[N_f]) * dz * dz * eta) + rho_f_avg * u_flow * (h_f[N_f - 1] - h_f[N_f]) / dz - h_v[N_f] * (T_f[N_f] - T_s[N_f]) / eta - U_wall * CN.pi * D_tank * (T_f[N_f] - T_amb) / (eta * A) - U_top * CN.pi * D_tank * D_tank * 0.25 * (T_f[N_f] - T_amb) / (eta * A * dz);
    h_top = h_f[N_f];
  end if;
...
end Thermocline_HCylinders_Section;