within SolarTherm.Models.Storage.Thermocline.HotBlastStove.Series;

model LC_TwoTank_Final
...
//Checkerbrick surface roughness
  ...
  SolarTherm.Models.Storage.Thermocline.HotBlastStove.LC_Section_Final Tank_B(redeclare replaceable package Fluid_Package = Fluid_Package, redeclare replaceable package Filler_Package = Filler_Package_B, Correlation = Correlation, E_max = E_max*(1.0-frac_1), ar = ar_B, epsilon = epsilon_B, d_p = d_p_B, T_min = T_min, T_max = T_max, N_f = N_f_B, U_loss_top = U_loss_top_B, U_loss_bot = U_loss_bot_B, z_offset = Tank_A.H_tank, E_roughness = E_roughness_B);
...
equation
  ...
  W_dot_loss_pump = Tank_A.W_dot_loss_pump + Tank_B.W_dot_loss_pump;
  Q_dot_loss_total = Tank_A.Q_dot_loss_total + Tank_B.Q_dot_loss_total;
