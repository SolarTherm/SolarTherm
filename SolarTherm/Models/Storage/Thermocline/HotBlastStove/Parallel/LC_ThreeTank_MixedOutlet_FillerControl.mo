within SolarTherm.Models.Storage.Thermocline.HotBlastStove.Parallel;

model LC_ThreeTank_MixedOutlet_FillerControl
  ...
  parameter SI.SpecificEnthalpy h_recv_set = Filler_Package_A.h_Tf(T_recv_set,0.0);
  parameter SI.SpecificEnthalpy h_PB_set = Filler_Package_C.h_Tf(T_PB_set,0.0);
  ...
  parameter SI.SpecificEnthalpy h_tol = 0.10*(Tank_A.h_p_max - Tank_A.h_p_min) "enthalpy tolerance, 0.5% of maximum difference";
...
//Tank A assists Tank B during charging, therefore when Tank B needs help, modify the chargestate boolean
  when Tank_C.T_p[1] > T_recv_set then
    if Charge_State == 1 then
      Charge_State := 2;
    end if;
  elsewhen Tank_B.T_p[1] > T_recv_set then
    if Charge_State == 2 or Charge_State == 3 then
      Charge_State := 4;
    end if;
  elsewhen Tank_B.T_p[1] < T_recv_set - T_tol then
    if Charge_State == 4 or Charge_State == 5 then
      Charge_State := 3;
    end if;
  elsewhen Tank_C.T_p[1] < T_recv_set - T_tol then
    if Charge_State == 2 or Charge_State == 3 then
      Charge_State := 1;
    end if;
  end when;
...
//Tank C assists Tank B during discharge
  when Tank_A.T_p[N_f_A] < T_PB_set then
    if Discharge_State == 1 then
      Discharge_State := 2;
    end if;
  elsewhen Tank_B.T_p[N_f_B] < T_PB_set then
    if Discharge_State == 2 or Discharge_State == 3 then
      Discharge_State := 4;
    end if;
  elsewhen Tank_B.T_p[N_f_B] > T_PB_set + T_tol then
    if Discharge_State == 4 or Discharge_State == 5 then
      Discharge_State := 3;
    end if;
  elsewhen Tank_A.T_p[N_f_A] > T_PB_set + T_tol then
    if Discharge_State == 2 or Discharge_State == 3 then
      Discharge_State := 1;
    end if;
  end when;
...
//Tank C no longer able to assist B, proceeds to discharge by itself
  when f_disch_guess_1 > 1.0 then
    if Discharge_State == 2 then
      if Tank_B.T_p[N_f_B] > T_PB_set + T_tol then
        Discharge_State := 3;
      else
        Discharge_State := 4;
      end if;
...
equation
  ...
  //Discharge
    ...
    if Tank_B.h_p[N_f_B] > Tank_A.h_p[N_f_A] + h_tol then //100.0 then
      //f_disch_guess_1 = (h_PB_set - Tank_A.h_f[N_f_A]) / (Tank_B.h_f[N_f_B] - Tank_A.h_f[N_f_A]);
      f_disch_guess_1 = (h_PB_set - Tank_A.h_p[N_f_A]) / min(h_tol, (Tank_B.h_p[N_f_B] - Tank_A.h_p[N_f_A]) );
    else //Cannot Assist
      f_disch_guess_1 = 0.0;
    end if;

    
    if Tank_C.h_p[N_f_C] > Tank_B.h_p[N_f_B] + h_tol then //100.0 then
      //f_disch_guess_2 = (h_PB_set - Tank_B.h_f[N_f_B]) / (Tank_C.h_f[N_f_C] - Tank_B.h_f[N_f_B]);
      f_disch_guess_2 = (h_PB_set - Tank_B.h_p[N_f_B]) / min(h_tol,  (Tank_C.h_p[N_f_C] - Tank_B.h_p[N_f_B]));
    else //Cannot Assist
      f_disch_guess_2 = 0.0;
    end if;
  ...
  //Charge
    ...
    if Tank_A.h_p[1] < Tank_B.h_p[1] - h_tol then //100.0 then
      //f_chg_guess_2 = (h_recv_set - Tank_B.h_f[1]) / (Tank_A.h_f[1] - Tank_B.h_f[1] );
      f_chg_guess_2 = (h_recv_set - Tank_B.h_p[1]) /  max(-1.0*h_tol, (Tank_A.h_p[1] - Tank_B.h_p[1] ));
    else
      f_chg_guess_2 = 0.0;
    end if;

    if Tank_B.h_p[1] < Tank_C.h_p[1] - h_tol then //100.0 then
      //f_chg_guess_1 = (h_recv_set - Tank_C.h_f[1]) / (Tank_B.h_f[1] - Tank_C.h_f[1] );
      f_chg_guess_1 = (h_recv_set - Tank_C.h_p[1]) /  max(-1.0*h_tol, (Tank_B.h_p[1] - Tank_C.h_p[1] ));
    else
      f_chg_guess_1 = 0.0;
    end if;
  ...
  */
  ...
  annotation(
    Icon(graphics = {Rectangle(origin = {9, 49}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-49, 11}, {31, -109}}), Text(origin = {-33, 33}, extent = {{-5, 5}, {5, -7}}, textString = "A"), Text(origin = {-7, 33}, extent = {{-5, 5}, {5, -7}}, textString = "B"), Rectangle(origin = {3, 11}, fillColor = {144, 144, 144}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {3, -1}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {-7, 17}, fillColor = {203, 203, 203}, fillPattern = FillPattern.Solid, extent = {{-29, 7}, {-9, 1}}), Rectangle(origin = {-3, -17}, fillColor = {24, 24, 24}, fillPattern = FillPattern.Solid, extent = {{-33, -1}, {-13, -7}}), Rectangle(origin = {1, -13}, fillColor = {71, 71, 71}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {3, 5}, fillColor = {113, 113, 113}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {-3, -11}, fillColor = {47, 47, 47}, fillPattern = FillPattern.Solid, extent = {{-33, -1}, {-13, -7}}), Rectangle(origin = {21, 17}, fillColor = {203, 203, 203}, fillPattern = FillPattern.Solid, extent = {{-31, 7}, {-11, 1}}), Rectangle(origin = {29, -13}, fillColor = {71, 71, 71}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {21, -17}, fillColor = {24, 24, 24}, fillPattern = FillPattern.Solid, extent = {{-31, -1}, {-11, -7}}), Rectangle(origin = {29, 11}, fillColor = {144, 144, 144}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {27, -1}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {29, 5}, fillColor = {113, 113, 113}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {23, -11}, fillColor = {47, 47, 47}, fillPattern = FillPattern.Solid, extent = {{-33, -1}, {-13, -7}}), Line(origin = {0, 53}, points = {{0, 9}, {0, -7}, {0, -7}}), Line(origin = {0, -53}, points = {{0, -7}, {0, 7}, {0, 7}}), Ellipse(origin = {-5, -41}, extent = {{1, -1}, {9, -9}}, endAngle = 360), Ellipse(origin = {-5, 51}, extent = {{1, -1}, {9, -9}}, endAngle = 360), Rectangle(origin = {53, 11}, fillColor = {144, 144, 144}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {53, 5}, fillColor = {113, 113, 113}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {53, -1}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {53, -13}, fillColor = {71, 71, 71}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {47, -11}, fillColor = {47, 47, 47}, fillPattern = FillPattern.Solid, extent = {{-31, -1}, {-11, -7}}), Rectangle(origin = {47, -17}, fillColor = {24, 24, 24}, fillPattern = FillPattern.Solid, extent = {{-31, -1}, {-11, -7}}), Text(origin = {19, 33}, extent = {{-5, 5}, {5, -7}}, textString = "C"), Line(origin = {-3, -31}, points = {{-23, 7}, {-23, -15}, {29, -15}}), Line(origin = {26, -32}, points = {{0, -14}, {0, 8}}), Line(origin = {0, -32}, points = {{0, -14}, {0, 14}}), Line(origin = {-1, 33.834}, points = {{-25, -11}, {-25, 11}, {27, 11}, {27, -11}}), Line(origin = {0, 35}, points = {{0, -11}, {0, 11}}), Rectangle(origin = {3, -7}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-39, 7}, {-19, 1}}), Rectangle(origin = {27, -7}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Rectangle(origin = {53, -7}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, extent = {{-37, 7}, {-17, 1}}), Polygon(origin = {-27, 6}, fillColor = {195, 195, 195}, fillPattern = FillPattern.Solid, points = {{-3, 20}, {5, 20}, {11, 18}, {11, -30}, {5, -32}, {-3, -32}, {-9, -30}, {-9, 18}, {-3, 20}}), Line(origin = {-26, 23.1708}, points = {{-10, 0.82918}, {-4, -1.17082}, {4, -1.17082}, {10, 0.82918}}), Ellipse(origin = {-26, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {-30, 24}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-30, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-24, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-24, 24}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-32, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {-20, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Line(origin = {-30, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {-22, -2}, points = {{0, 24}, {0, -24}}), Polygon(origin = {-1, 6}, fillColor = {195, 195, 195}, fillPattern = FillPattern.Solid, points = {{-3, 20}, {5, 20}, {11, 18}, {11, -30}, {5, -32}, {-3, -32}, {-9, -30}, {-9, 18}, {-3, 20}}), Line(origin = {0, 23.1325}, points = {{-10, 0.867544}, {-4, -1.13246}, {4, -1.13246}, {10, 0.867544}}), Line(origin = {-4, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {4, -2}, points = {{0, 24}, {0, -24}}), Ellipse(origin = {-4, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-6, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {2, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {-4, 24}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {2, 24}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {0, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {6, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Line(origin = {26, 23.1325}, points = {{-10, 0.867544}, {-4, -1.13246}, {4, -1.13246}, {10, 0.867544}, {10, 0.867544}}), Line(origin = {22, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {30, -2}, points = {{0, 24}, {0, -24}}), Ellipse(origin = {22, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {28, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Ellipse(origin = {26, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {32, 26}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-1.5, -1.5}, {1.5, -2.5}}, endAngle = 360), Ellipse(origin = {28, 24}, fillColor = {211, 211, 255}, fillPattern = FillPattern.Solid, extent = {{-0.5, -0.5}, {2.5, -1.5}}, endAngle = 360), Line(origin = {-26, 25}, points = {{0, 1}, {0, -1}}), Line(origin = {0, 25}, points = {{0, 1}, {0, -1}}), Line(origin = {26, 25}, points = {{0, 1}, {0, -1}}), Polygon(origin = {-26, 10}, fillColor = {156, 156, 156}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {-26, 2}, fillColor = {138, 138, 138}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {-26, -6}, fillColor = {108, 108, 108}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {-26, -14}, fillColor = {80, 80, 80}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {-26, -22}, fillColor = {61, 61, 61}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {-26, 18}, fillColor = {195, 195, 195}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Line(origin = {-30, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {-22, -2}, points = {{0, 24}, {0, -24}}), Polygon(origin = {0, 18}, fillColor = {195, 195, 195}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {0, 10}, fillColor = {156, 156, 156}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {26, 10}, fillColor = {156, 156, 156}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {0, 2}, fillColor = {138, 138, 138}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {26, 2}, fillColor = {138, 138, 138}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {0, -6}, fillColor = {108, 108, 108}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {26, -6}, fillColor = {108, 108, 108}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {0, -14}, fillColor = {80, 80, 80}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {26, -14}, fillColor = {80, 80, 80}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {0, -22}, fillColor = {61, 61, 61}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Polygon(origin = {26, -22}, fillColor = {61, 61, 61}, fillPattern = FillPattern.Solid, points = {{-10, 6}, {-4, 4}, {4, 4}, {10, 6}, {10, -2}, {4, -4}, {-4, -4}, {-10, -2}, {-10, 6}}), Line(origin = {-4, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {4, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {22, -2}, points = {{0, 24}, {0, -24}}), Line(origin = {30, -2}, points = {{0, 24}, {0, -24}})}, coordinateSystem(initialScale = 0.1)));
end LC_ThreeTank_MixedOutlet_FillerControl;