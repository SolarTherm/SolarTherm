...
model LC_Section_Final "Heat transfer model of thermocline tank with spherical (lumped capacitance) fillers"
  ...
  parameter Integer Correlation = 1 "1=WakaoKaguei, 2=MelissariArgyropolus, 3=Conservative, 4=Internal-flow-gas";
  ...
  for i in 1:N_f loop
    ...
    if abs(u_flow) > 1e-12 then //There is actually mass flowing
      ...
      elseif Correlation == 3 then
        ...
        Nu[i] = SolarTherm.Utilities.Nusselt.Internal_Flow.Nusselt_Gas(Re[i],Pr[i],T_f[i],T_p[i]);
      end if;
    else
      Re[i] = 0;
      Pr[i] = 0;
