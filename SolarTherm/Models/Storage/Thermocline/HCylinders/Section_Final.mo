within SolarTherm.Models.Storage.Thermocline.HCylinders;

model Section_Final "Heat transfer model of thermocline tank with horizontal-cylinder fillers"
  ...
  parameter Integer Correlation = 1; //Option 1: Nu[i] = (Re[i] * Pr[i]) ^ 0.5, 2(else) : Nu = 5.5 + 0.025*Pe (Borishanski 1967)"; //Option 1 is for horizontal cylinders
  ...
  parameter Real C_insulation = if U_loss_tank > 1e-3 then SolarTherm.Models.Storage.CpA_external_insulation(T_max,U_loss_tank)*A_loss_tank else 0.0;
  
  parameter SI.Density rho_bulk = max(rho_f_max,rho_f_min) "Bulk density used to calculate hydrostatic pressure"; //Only due to fluid because it is structured
  parameter Real C_tank = C_shell(rho_bulk,H_tank,D_tank,Tank_Package.sigma_yield(T_max),Tank_Package.rho_Tf(298.15, 0.0),Tank_Package.cost,4.0);
...
protected
  ...
  //Filler geometry
  parameter SI.Length r_p[N_p] = cat(1,SolarTherm.Models.Storage.Thermocline.Particle_Radii(d_p-2*t_e,N_p-1),{(d_p/2)-(t_e/2)}) "Radii of each cylinder element centre";
  parameter SI.Mass m_p[N_f, N_p] = fill( cat(1,SolarTherm.Models.Storage.Thermocline.Cylinder_Masses(d_p-2*t_e, N_p-1, rho_p, L_filler_cylinder), {CN.pi*(d_p-t_e)*(t_e)*(L_filler_cylinder)*rho_e} ) , N_f) "Masses of each filler cylinder element in each vertical tank slice";
...
*/
  ...
  for i in 1:N_f loop
    ...
    h_v[i] = 4.0 * (1.0 - eta) * Nu[i] * k_f[i] / (d_p * d_p);
  end for;
  //Particle Equations
  for i in 1:N_f loop
