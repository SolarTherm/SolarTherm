...
model Thermocline_Section
  ...
  Filler.State filler[N_f, N_p] "Filler object array";
  ...
  //Filler
  parameter SI.SpecificEnthalpy h_p_max = Filler.h_Tf(T_max, 1.0);
  parameter SI.SpecificEnthalpy h_p_min = Filler.h_Tf(T_min, 0.0);
  parameter SI.Density rho_p_min = Filler.rho_Tf(T_min, 0.0);
  parameter SI.Density rho_p_max = Filler.rho_Tf(T_max, 1.0);
  parameter SI.Density rho_p = (rho_p_min + rho_p_max) / 2.0 "kg/m3";
  //Discretization
