...
extends SolarTherm.Interfaces.Models.StorageFluid_Thermocline;
  ...
  Modelica.Blocks.Interfaces.RealOutput T_90_measured = Interpolate_Temperature(ZDH,T_f_degC,N_f,0.85) + 273.15 "Temperature at the 90% height of the tank as an output signal (K)"
                                          annotation (Placement(visible = true,transformation(
          extent = {{40, 36}, {60, 56}}, rotation = 0), iconTransformation(origin = {45, 41}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput T_10_measured = Interpolate_Temperature(ZDH,T_f_degC,N_f,0.15) + 273.15"Temperature at the 10% height of the tank as an output signal (K)"
                                          annotation (Placement(visible = true,transformation(
...
equation
  ...
  */
  ...
  if fluid_a.m_flow > 0.0 then //1e-6 then//mass is flowing into the top, direction is downwards so Tank_A.m_flow is (negative), charging
    Tank_A.m_flow = -1.0*fluid_a.m_flow;
    Tank_A.h_top = inStream(fluid_a.h_outflow);
    fluid_a.h_outflow = Tank_A.h_f[N_f];//fluid_top.h;
    fluid_b.h_outflow = Tank_A.h_f[1];//fluid_bot.h;
  else//if fluid_a.m_flow < 0.0 then// < -1e-6 then //discharging
    Tank_A.m_flow = -1.0*fluid_a.m_flow;
    Tank_A.h_bot = inStream(fluid_b.h_outflow);
    fluid_a.h_outflow = Tank_A.h_f[N_f];//fluid_top.h;
    fluid_b.h_outflow = Tank_A.h_f[1];//fluid_bot.h;
