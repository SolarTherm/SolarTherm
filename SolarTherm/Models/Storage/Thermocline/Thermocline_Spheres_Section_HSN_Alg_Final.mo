...
model Thermocline_Spheres_Section_HSN_Alg_Final
...
algorithm
  ...
  //Bottom Charging Fluid Node
    ...
    - 0.5*U_wall*CN.pi*D_tank*(T_f[1]-T_amb)/(eta*A) )/ ( 0.5*rho_f_avg );
  ...
  //Top Charging Fluid Node
    ...
    - U_top*(T_f[N_f]-T_amb)/(eta*dz) ) / (0.5*rho_f_avg);
...
end Thermocline_Spheres_Section_HSN_Alg_Final;