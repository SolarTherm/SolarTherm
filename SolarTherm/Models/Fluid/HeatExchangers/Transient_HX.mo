...
function Transient_HX
  ...
  SI.Temperature T_Na2_calc_prev=700+273.15 "Output Sodium Temperature calc";
...
algorithm
  ...
  while noEvent(condition>tol) loop
    ...
    //(U, h_s, h_t):=HTCs(...); //Function is called Heat Transfer Coefficients
    Q:=U*A*F*LMTD;
    m_flow_MS:=Q_d/(cp_MS*(T_MS2 - T_MS1));
    T_Na2_calc:=T_Na1-(Q/(m_flow_Na*cp_Na));
    condition:=abs(T_Na2_calc-T_Na2_calc_prev)/T_Na2_calc_prev;
