...
model HX_wi_noF_modified
  ...
  //Ports Variables
  SI.SpecificEnthalpy h_Na_in(start=h_Na_out_0, nominal=h_Na_in_0);
...
initial algorithm
  ...
  m_flow_MS_min_des := 0.20 * m_flow_MS_design;
...
//Problem
  ...
  m_flow_MS = /*if HF_on then max(1,Q/(h_MS_out - h_MS_in)) else*/ Q/(h_MS_out - h_MS_in);
