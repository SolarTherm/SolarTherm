...
//Problem
  Q = max(1, m_flow_Na) * (h_Na_in - h_Na_out);
  Q = max(1, m_flow_MS) * (h_MS_out - h_MS_in);
  ...
  when not HF_on then
    reinit(port_b_out.h_outflow, h_MS_out_0);
  end when;
  
end HX_wi_noF_modified_vrec;