...
model HX_H2H2O_H2_Capacitance
  ...
  //Array variables
  SI.Temperature T_H[N + 1](start = fill(T_H_in_des, N + 1)) "Temperature array of hot fluid (K)";
  SI.Temperature T_C[N + 1](start = fill(T_C_in_des, N + 1)) "Temperature array of cold fluid (K)";
  SI.Temperature T_H_plus[N](start = fill(T_H_in_des, N)) "Array of average temperature across hot element i to i+1 (K)";
  SI.Temperature T_C_plus[N](start = fill(T_C_in_des, N)) "Array of average temperature across cold element i to i+1 (K)";
...
algorithm
  for i in 1:N loop
    Q_flow[i] := UA / N * (T_H_plus[i] - T_C_plus[i]);
  end for;
equation
//Ports
