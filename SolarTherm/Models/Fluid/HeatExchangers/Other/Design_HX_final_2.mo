...
model Design_HX_final_2

  import SI = Modelica.SIunits;
  import CN = Modelica.Constants;
  import Modelica.Math;
  
  parameter SI.Length d_o "Outer Tube diameter";
  SI.Length L(start=2) "Tube length";
  Real N_p(start=4) "Number of passes";
  Integer layout(start=1) "Tube layout";
  // if layout=1(one) is square, while if layout=2(two) it is triangular //
  SI.Pressure P_shell(start=101325) "Shell-side pressure";
  SI.Pressure P_tubes(start=101325) "Tube-side pressure";
  SI.ThermalConductance UA(start=396826) "UA";
  SI.MassFlowRate m_flow_Na(start=400) "Sodium mass flow rate";
  SI.MassFlowRate m_flow_MS(start=85) "Molten-Salt mass flow rate";
  Real c_e(unit = "€/year",start=0.13) "Power cost";
  Real r(start=0.05) "Real interest rate";
  Real CEPCI_18(start=603.1) "CEPCI 2018";
  Real H_y(unit= "h",start=500) "Operating hours";
  Real M_conv(unit= "€/USD",start=0.9175) "Conversion factor";
  SI.ThermalConductivity k_wall(start=24) "Tube Thermal Conductivity";
  SI.ThermalConductivity k_Na(start=54.5113) "Sodium Conductivity @mean temperature";
  SI.ThermalConductivity k_MS(start=0.447185) "Molten Salts Conductivity @mean temperature";
  SI.Density rho_Na(start=782.473) "Sodium density @mean temperature";
  SI.Density rho_MS(start=1634.34) "Molten Salts density @mean temperature";
  SI.DynamicViscosity mu_Na(start=0.000182038) "Sodium dynamic viscosity @mean temperature";
  SI.DynamicViscosity mu_MS(start=0.00341597) "Molten Salts  dynamic viscosity @mean temperature";
  SI.DynamicViscosity mu_Na_wall(start=0.000182038) "Sodium dynamic viscosity @wall temperature";
  SI.DynamicViscosity mu_MS_wall(start=0.00260711) "Molten salts dynamic viscosity @wall temperature";
  SI.SpecificHeatCapacity cp_Na(start=1252.4) "Sodium specific heat capacity @mean temperature";
  ...
  parameter Real tol=0.05 "Heat transfer coefficient tollerance";
