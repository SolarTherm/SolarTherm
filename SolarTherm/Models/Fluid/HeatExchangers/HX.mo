...
model HX
  ...
  parameter Real H_y = 5600 "Operating hours";
  ...
  parameter SI.Temperature T_Na2_min = Modelica.SIunits.Conversions.from_degC(505) "Desing Sodium Hot Fluid Temperature";
  ...
  parameter SI.TemperatureDifference LMTD_start=(DT1_start - DT2_start) / Modelica.Math.log(DT1_start / DT2_start);
...
//Fluids Enthalpies
  h_Na_in = if Q_out_rec > CN.small then inStream(port_a_in.h_outflow) else h_Na_out_0;
...
//Problem
  ...
  if HF_on then
    if noEvent(DT1 > CN.small and DT2 > 0) then
      ...
      LMTD = (DT1 - DT2) / Modelica.Math.log(max(CN.small, DT1 / DT2));
    else
      LMTD = DT1;
    end if;
  else
