...
model StirlingSES_PB
  ...
  parameter Boolean Fixed_TH = true "fixed expansion temperature or set";
  ...
  // OK Stop here
  parameter SI.HeatFlowRate W_net_des = N*24.1181e3 "Maximum total net output of the engine array, anything above this will be curtailed";
  parameter SI.HeatFlowRate Q_des_each = 69.7471e3 "Design heat input when fully on, this is fixed" annotation(
...
if startup == false then
  ...
  if load > 0.1 then
    nu_cycle = (Beale * p_engine * V_swept * frequency * (1.0-(TC_internal/TH_internal)^0.5))/Q_flow_each; //determine efficiency
  else
    nu_cycle = 0.0;
  end if;
