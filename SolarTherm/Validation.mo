...
// Interpolate
  ...
  model test_st_surrogate
    ...
    SolarTherm.Utilities.SurrogateModels.STNeuralNetwork session = SolarTherm.Utilities.SurrogateModels.STNeuralNetwork(saved_model_dir);
  equation
    raw_input = {280e6 + time * 2e6, 268.15, 803.20, 23 + 0.1 * time};
    out = SolarTherm.Utilities.SurrogateModels.predict(session, raw_input, inputsize, time, X_max, X_min, out_max, out_min);
    der(marker) = time;
    annotation(
