...
// Interpolate
  ...
  model test_OTF_surrogate
    ...
    //******************** Kriging
    Kriging_properties Kriging_variables = Kriging_properties(P_net, T_in_ref_blk, p_high, PR, pinch_PHX, dTemp_HTF_PHX, load_base, T_amb_base, eta_gross_base, eta_Q_base, base_path, SolarTherm_path, inputsize, outputsize, tolerance_kriging, PB_model, htf_choice, dT_PHX_hot_approach, dT_PHX_cold_approach, eta_isen_mc, eta_isen_rc, eta_isen_t, dT_mc_approach, HTF_name);
    //******************** ANN PB
    parameter Real tolerance_ANN = 1;
    parameter Integer index_0 = 0;
    ANN_properties session_PB = ANN_properties(P_net, T_in_ref_blk, p_high, PR, pinch_PHX, dTemp_HTF_PHX, load_base, T_amb_base, eta_gross_base, eta_Q_base, index_0, base_path, SolarTherm_path, inputsize, outputsize, tolerance_ANN, PB_model, htf_choice, dT_PHX_hot_approach, dT_PHX_cold_approach, eta_isen_mc, eta_isen_rc, eta_isen_t, dT_mc_approach, HTF_name);
    //******************** ANN HX
    parameter Integer index_1 = 1;
    ANN_properties session_HX = ANN_properties(P_net, T_in_ref_blk, p_high, PR, pinch_PHX, dTemp_HTF_PHX, load_base, T_amb_base, eta_gross_base, eta_Q_base, index_1, base_path, SolarTherm_path, inputsize, outputsize, tolerance_ANN, PB_model, htf_choice, dT_PHX_hot_approach, dT_PHX_cold_approach, eta_isen_mc, eta_isen_rc, eta_isen_t, dT_mc_approach, HTF_name);
  ...
  initial equation
    NREL_PB_configurations = SolarTherm.Utilities.designNRELPB(P_net, T_in_ref_blk, p_high, dT_PHX_hot_approach, dT_PHX_cold_approach, eta_isen_mc, eta_isen_rc, eta_isen_t, dT_mc_approach, T_amb_base, HTF_name, SolarTherm_path);
  ...
  equation
    ...
    k = Medium.thermalConductivity(state);
    h = Medium.specificEnthalpy(state);
  end eval_sodium_props;

  model testCombiTable2D
    import SI = Modelica.SIunits;
    import nSI = Modelica.SIunits.Conversions.NonSIunits;
    parameter String tablefile = "./OELT_Solstice.motab";
    ...
    SI.Efficiency eta_opt[horizon] "Optical eff for the next forecast horizon";
    SI.Velocity Wspd_horizon[horizon];
    Real counter(start = -3600);
    Real time_simul;
    Real Wspd;
    Modelica.Blocks.Types.ExternalCombiTable2D opt_eff = Modelica.Blocks.Types.ExternalCombiTable2D(tableName = "optics", fileName = Modelica.Utilities.Files.loadResource("OELT_Solstice.motab"), table = fill(0.0, 0, 2), smoothness = Modelica.Blocks.Types.Smoothness.ContinuousDerivative);
    ////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////////
    replaceable function solarPosition = SolarTherm.Models.Sources.SolarFunctions.PSA_Algorithm;
    Modelica.Blocks.Sources.CombiTimeTable table(tableOnFile = true, tableName = "data", smoothness = Modelica.Blocks.Types.Smoothness.ContinuousDerivative, columns = 2:11, fileName = wea_file) annotation(
  ...
  equation
    ...
    when counter > 0 then
      ...
      for i in 1:horizon loop
        (dec_horizon[i], hra_horizon[i]) = SolarTherm.Models.Sources.SolarFunctions.PSA_Algorithm(time_simul + i * 1 * 3600, lon, lat, t_zone, year);
        eta_opt[i] = SolarTherm.Utilities.opt_eff_horizon(to_deg(dec_horizon[i]), to_deg(hra_horizon[i]), opt_eff);
        Wspd_horizon[i] = SolarTherm.Utilities.wind_speed_horizon(time_simul + i * 1 * 3600, wea_table);
      end for;
      reinit(counter, -3600);
    end when;
    Wspd = table.y[8];
  ...
  model ParticleGasHX_TestRig
    ...
    parameter Integer num_seg = 20 "Segmentation of heat exchanger";
    ...
    Modelica.Fluid.Sources.FixedBoundary source_pcl(redeclare package Medium = Med_PCL, T = T_in_PCL_DP, nPorts = 1, p = p_working, use_T = true, use_p = false) annotation(
    ...
    Modelica.Blocks.Sources.RealExpression realExpression(y = 1603.42) annotation(
      Placement(visible = true, transformation(origin = {-156, 96}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    SolarTherm.Models.Fluid.HeatExchangers.HeatExchanger_ParticleGas HX(D_particle = D_particle, H_HX = H_HX, T_in_AIR_DP = T_in_AIR_DP, T_in_PCL_DP = T_in_PCL_DP, T_out_AIR_DP = T_out_AIR_DP, T_out_PCL_DP = T_out_PCL_DP, W_HX = W_HX, num_seg = num_seg, p_working = p_working) annotation(
      Placement(visible = true, transformation(origin = {10, -24}, extent = {{-80, -80}, {80, 80}}, rotation = 0)));
    Modelica.Blocks.Sources.RealExpression realExpression1(y = 298.15) annotation(
  ...
  end ParticleGasHX_TestRig;

  model testDispOptDualTank
    Real c0 = 20;
    Real c1 = 10;
    parameter Integer n_horizon = 24;
    Real dt = 1.0;
    Real c_ratio = 7.58259;
    Real etaPB = 0.5;
    Real[n_horizon] etaSF = {0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6};
    Real etaRCV = 0.88;
    Real etaPV = 0.20;
    Real etaAEL = 0.55;
    Real etaBurner = 0.76;
    Real etaResistive = 1;
    Real dni[n_horizon] = {0, 0, 0, 0, 0, 0, 0, 176, 492, 862, 749, 761, 844, 676, 481, 501, 0, 0, 0, 0, 0, 0, 0, 0};
    Real ghi[n_horizon] = {0, 0, 0, 0, 0, 0, 0, 50, 201, 396, 461, 507, 522, 421, 286, 162, 21, 0, 0, 0, 0, 0, 0, 0};
    Real P_AEL_nameplate = 100;
    Real A_sf = 90000.0;
    Real A_pv = 1500000.0;
    Real PB_size = 100;
    Real TES_capacity = 1058.258;
    Real Q_TES_HX_max = 32;
    Real E_TES_init = 0.05 * TES_capacity;
    Real E_TES_min = 0.05 * TES_capacity;
    Real LHV = 120;
    Real mdot_H2_feedstock = 2.5755;
    Real Q_H2_reactor_max = mdot_H2_feedstock * LHV;
    Real delta_h_H2 = 1633200.0 / 1000000;
    Real t_storage_H2 = 720;
    Real H2stg_capacity = mdot_H2_feedstock * LHV * t_storage_H2 * 3600 * 0.0002777778;
    Real H2stg_init = 0.25 * H2stg_capacity;
    Real H2stg_min = 0.05 * H2stg_capacity;
    Real t0 = 0;
    Real optimalSolution[7];
  equation
    optimalSolution = SolarTherm.Utilities.LinProgFuncDualTank(c0, c1, n_horizon, dt, c_ratio, etaPB, etaSF, etaRCV, etaPV, etaAEL, etaBurner, etaResistive, dni, ghi, P_AEL_nameplate, A_sf, A_pv, PB_size, TES_capacity, Q_TES_HX_max, E_TES_init, E_TES_min, Q_H2_reactor_max, LHV, delta_h_H2, H2stg_capacity, H2stg_init, H2stg_min, t0);
    annotation(
      experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 1));
  end testDispOptDualTank;
  ...
  model test_PVFunc
    ...
    Real inc_dummy;
  equation
    (P_net, AM, Ib_beta_dummy, Id_beta_dummy, Ir_beta_dummy) = SolarTherm.Utilities.PVArray_func(N_parallel_final, N_series, DHI, albedo, Wspd, Tdry, PV_Target, azi_s, ele_s, lat, ele_min, dni, delta, hra, zen, azi, ele);
    annotation(
      experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 0.002));
  end test_PVFunc;
  ...
  model PVModelTest
    ...
    parameter Integer N_parallel_final = 136;
    parameter Integer N_series = 11;
    parameter Real DHI = 64.3242;
    parameter Real albedo = 0.216;
    parameter SI.Velocity Wspd = 5.08312;
    parameter SI.Temperature Tdry = 279.77;
    parameter SI.Power PV_Target = 2e8;
    parameter nSI.Angle_deg azi_s = 180;
    parameter nSI.Angle_deg ele_s = 30;
    parameter nSI.Angle_deg lat = 34.8667;
    parameter SI.Angle ele_min = 0.0872665;
    parameter Real dni = 875.96;
    parameter SI.Angle delta = -0.401805;
    parameter SI.Angle hra = -0.678581;
    parameter SI.Angle zen = 1.19794;
    parameter SI.Angle azi = 2.47244;
    parameter SI.Angle ele = 0.37286;
    parameter SI.Angle inc = 0.822074;
    Real P_net;
    // Plant location
    ...
    Real DC_AC_ratio_calc = N_module * P_module / (N_inv * P_ac_max_inv) "Calculated DC to AC ratio";
    //DC to AC ratio needs to be between 0.8 to 1.25 times inverter rated AC power
    Integer Nstr_max = integer(floor(I_Inv_max / Isc)) "Maximum number of strings based on I_Inv_max";
    Integer N_series_min = integer(floor(V_Inv_min / Voc)) "Minimum number of modules in series";
    Integer N_series_max = integer(floor(V_Inv_max / Voc)) "Maximum number of modules in series";
    Integer N_parallel_min = integer(N_module / N_series_max) "Minimum number of strings in parallel";
    Integer N_parallel_max = integer(N_module / N_series_min) "Maximum number of strings in parallel";
    ...
    Integer N_parallel_guess = if Nstr_max > N_parallel_max then N_parallel_max elseif Nstr_max > N_parallel_min then N_parallel_min else Nstr_max;
  ...
  model PVForecastTest
    ...
    Real t0 = 3600;
    SolarTherm.Models.PV.PVarray_forecasting pv_forecast(wea_file = wea_file, year = 1967, t_zone = -8);
  equation
/*
    for i in 1:24 loop
      pv_forecast.DHI[i] = DHI;
      pv_forecast.albedo[i] = albedo;
      pv_forecast.dni[i] = dni;
      pv_forecast.Wspd[i] = Wspd;
      pv_forecast.Tdry[i] = Tdry;
      pv_forecast.delta[i] = delta;
      pv_forecast.hra[i] = hra;
      pv_forecast.hra_intermediate[i] = hra;
      pv_forecast.zen[i] = zen;
      pv_forecast.ele[i] = ele;
    end for;
  */
    annotation(
      experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-06, Interval = 1));
