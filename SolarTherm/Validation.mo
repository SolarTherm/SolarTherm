...
// Interpolate
  ...
  model linprog
    parameter String filepathDNI = "/home/philgun/solartherm-particle/SolarTherm/Data/Weather/gen3p3_Daggett_TMY3_EES.motab";
    parameter String filepathPrice = "/home/philgun/solartherm-particle/SolarTherm/Data/Prices/aemo_vic_2014.motab";
    parameter Integer horison = 24;
    parameter Real dt = 1;
    //parameter Real time_simul = 0 * 3600;
    parameter Real etaC = 0.6;
    parameter Real etaG = 0.5;
    parameter Real t_stg = 14.;
    parameter Real DEmax = 100.43;
    parameter Real SLmax = 600.123;
    parameter Real SL0rel = 0.05;
    parameter Real SLminrel = 0.05;
    parameter Real Ahelio = 900000.0;
    Real optimalDispatch;
    Real counter(start = -dt * 3600);
    Real time_simul(start = 0);
  equation
    der(counter) = 1;
    when counter > 0 then
      time_simul = floor(time);
      optimalDispatch = SolarTherm.Utilities.LinProgFunc(filepathDNI, filepathPrice, horison, dt, time_simul, etaC, etaG, t_stg, DEmax, SLmax, SL0rel, SLminrel, Ahelio);
  ...
  model test_st_surrogate
    parameter Real[4] raw_input = {280e6, 268.15, 803.15, 23};
    Integer inputsize = 4;
    String saved_model_dir = "/home/philgun/solartherm-particle/SolarTherm/Resources/Include/neural-network/trained-model/my_model";
    Real out;
  equation
    out = SolarTherm.Utilities.st_surrogate(raw_input, inputsize, saved_model_dir);
    annotation(
