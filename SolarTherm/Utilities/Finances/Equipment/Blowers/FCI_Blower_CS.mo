...
function FCI_Blower_CS "Input = {P_c (W)}"
...
annotation(
    Documentation(info = "<html><head></head><body><p style=\"font-size: 12px;\"><span style=\"font-family: 'DejaVu Sans Mono';\">Fixed-Capital-Investment cost of centrifugal blowers provided by Seider for Carbon Steel with a reference CEPCI index of 500, indexed to USD_2022. When converted to SI units, the FOB cost relationship is:</span></p><div class=\"htmlDoc\" style=\"font-size: 12px; font-family: 'DejaVu Sans Mono';\"><div><br></div><div>FOB = (CEPCI/500.0) * F_m * exp{ 6.8929 + 0.79*[ln(P_c/745.7)]}</div><div><br></div><div>where:</div><div><br></div><div>F_m = 1.0 (Carbon Steel)</div><div><br></div><div>3729 &lt;= P_c [W] &lt;= 745700</div><div><br></div><div>CEPCI_2022 = 816.0</div><div><br></div><div><div>The FCI cost relationship is:</div><div><br></div><div>FCI = FOB * F_d * F_hand * F_adj</div><div><br></div><div>where:</div><div><br></div><div>F_d = 1.05 factor for delivery</div><div>F_hand = 4.0 for pumps</div><div>F_adj = 1.0 for carbon steel</div></div></div></body></html>"));
end FCI_Blower_CS;