...
function FOB_Insulation_Mullite_HBS "Free-on-board puchase cost of internal mullite insulation based on regression. Inputs are {max temp (K), U_loss (W/mK), A_surf (m2)}"
...
algorithm
  ...
  C_FOB := (816.0/795.4)*A_surf*sum(a.*x); //Regression was based on USD_2024, index it to 2022.
