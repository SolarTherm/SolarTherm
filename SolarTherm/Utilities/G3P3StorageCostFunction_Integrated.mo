...
input Real H_tower, D_tower_out, m_tot, D_outlet;
input Real c[:];
...
Real D_tower = D_tower_out - 1.2;
Real H_bin = 
((m_tot/rho_particle)-(tan_30*(D_tower-D_outlet)/2*pi/3*(D_tower^2/4+D_outlet^2/4+D_tower*D_outlet/2))-(pi/3*D_tower^2/4*tan_313*D_tower/2))/(pi*D_tower^2/4)+tan_313*D_tower/2+tan_30*(D_tower-D_outlet)/2;
Real A_internal_bin = pi*D_tower^2/4+2*pi*D_tower/2*H_bin;

Real C_thicker_walls = 55.01 * H_tower^2.321-681300;

Real V_refractory = 
pi*(D_tower+Th_RF)^2/4*Th_RF+pi*(H_bin)*((D_tower/2+Th_RF)^2-(D_tower/2)^2);

Real C_refractory = 
(pi*(D_tower+Th_RF)^2/4)*c_roof+(pi*(H_bin)*((D_tower/2+Th_RF)^2-(D_tower/2)^2))*c_RF;
...
Real V_HRC = 
pi*(H_bin-Modelica.Math.tan(Modelica.SIunits.Conversions.from_deg(31.3)*D_tower/2))*((D_tower/2+Th_RF+Th_HRC)^2-(D_tower/2+Th_RF)^2);

Real C_HRC = V_HRC*0.67*c_HRC;

Real V_floor = 
pi/3*tan_30*(2*(D_tower/2)^3-3*(D_outlet/2)*(D_tower/2)^2+(D_outlet/2)^3);
